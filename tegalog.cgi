...
# ================================================================== #
#  てがろぐ - Fumy Otegaru Memo Logger Ver 4.7.0      [tegalog.cgi]  #
# ================================================================== #
#  Copyright (C) Fumihiro Nishimura.(Nishishi) 2017-2025.            #
#                                                                    #
...
#                                                                    #
#  https://www.nishishi.com/                            [2025/09/27] #
# ================================================================== #
...
my %libdat = (
	urljqueryjs => 'https://code.jquery.com/jquery-3.7.1.min.js',										# jQuery本体のデフォルトURL
	urllightboxjs => 'https://cdnjs.cloudflare.com/ajax/libs/lightbox2/2.11.5/js/lightbox.min.js',		# Lightbox本体のデフォルトURL
	urllightboxcss => 'https://cdnjs.cloudflare.com/ajax/libs/lightbox2/2.11.5/css/lightbox.min.css',	# Lightbox用CSSのデフォルトURL
...
# -------------------- #
my $versionnum = '4.7.0';
my $reqfumycts = 2023000;	# 2.23.0 Required
...
my %subfile = (
	...
	subPHP =>  'tegup.php',			# 機能補助PHPプログラム
	clapPHP =>  './tegaclap/tegaclap.php',	# いいね拍手プログラム
	blackfile => './blackcheck.ini'	# 高頻度アクセス防止設定ファイル名
);
...
my %setdat = (
	...
	allowanylineinsecret => 1,				# 本文中の任意の位置に鍵入力フォームを配置可能に(1:する、0:しない)
	...
	galleryimgtolink => 1,					# ギャラリーモードで画像を(原寸画像への)リンクに(1:する/0:しない)
	...
	urlcardsizedef => 0,					# Twitter Cardの指定がない場合のデフォルトカードサイズ(0:S/1:L)
	...
	urlexpandcodepen => 0,					# URLの直前に [CodePen] ラベルがあればCodePen枠を埋め込み(1:する/0:しない)
	urlexpandtwtheme => 0,					# 埋め込まれるツイートのカラーテーマ(0:Light/1:Dark)

	urlexpandvideo => 1,					# URLの直前に [VIDEO] ラベルがあれば動画として埋め込み(1:する/0:しない)
	urlexpandvideolim => 0,					# VIDEO埋め込みのドメイン制限(0:なし/1:同一ドメイン下に限定)
	...
	imagesearchperpage => 500,				# 画像リスト(絞り込み検索時)1ページあたりに表示する画像個数
	...
	imageexpandjsmodule => 0,				# 画像拡大JavaScriptをモジュールとして読む(0:NO/1:YES)
	...
	cemojimaxlimits => 0,					# カスタム絵文字のUP容量に上限を(1:設ける/0:設けない)
	cemojimaxbytes => 102400,				# カスタム絵文字画像1枚あたりの最大アップロード可能サイズ(単位:Bytes／標準:102400＝100KB)
	cemojistoragelimit => 52428800,			# 保存可能な最大容量(単位:Bytes／標準:52428800＝50MB)

	alwaysshowquickpost => 1,				# QUICKPOSTを常時表示するかどうか(0:しない/1:する)
	checknowcatbeforehand => 0,				# 限定表示時中のカテゴリに事前にチェックを(0:入れない/1:入れておく) QUICKPOSTのみ
	...
	showLinkBtnCrd => 0,	linkBtnCrdLabel => 'OGPカード',			# リンクボタン[OGPカード]の表示とラベル
	...
	showDecoBtnOonA => 0,	showDecoBtnOonQ => 0,	decoBtnLabelO => '折畳',								# 装飾ボタン[O]の表示(編集画面,QUICKPOST)	※折り畳む	※デフォルトでは非表示
	...
	showDecoBtn1onA => 0,	showDecoBtn1onQ => 0,	decoBtnLabel1 => '①',		decoBtnElmnt1 => 'span',	# 装飾ボタン[1]の表示(編集画面,QUICKPOST)	※デフォルトでは非表示
	showDecoBtn2onA => 0,	showDecoBtn2onQ => 0,	decoBtnLabel2 => '②',		decoBtnElmnt2 => 'span',	# 装飾ボタン[2]
	showDecoBtn3onA => 0,	showDecoBtn3onQ => 0,	decoBtnLabel3 => '③',		decoBtnElmnt3 => 'span',	# 装飾ボタン[3]
	showDecoBtn4onA => 0,	showDecoBtn4onQ => 0,	decoBtnLabel4 => '④',		decoBtnElmnt4 => 'span',	# 装飾ボタン[4]
	showDecoBtn5onA => 0,	showDecoBtn5onQ => 0,	decoBtnLabel5 => '⑤',		decoBtnElmnt5 => 'span',	# 装飾ボタン[5]
	showDecoBtn6onA => 0,	showDecoBtn6onQ => 0,	decoBtnLabel6 => '⑥',		decoBtnElmnt6 => 'span',	# 装飾ボタン[6]
	showDecoBtn7onA => 0,	showDecoBtn7onQ => 0,	decoBtnLabel7 => '⑦',		decoBtnElmnt7 => 'span',	# 装飾ボタン[7]
	showDecoBtn8onA => 0,	showDecoBtn8onQ => 0,	decoBtnLabel8 => '⑧',		decoBtnElmnt8 => 'span',	# 装飾ボタン[8]
	showDecoBtn9onA => 0,	showDecoBtn9onQ => 0,	decoBtnLabel9 => '⑨',		decoBtnElmnt9 => 'span',	# 装飾ボタン[9]
	...
	showStateBtnStyle => 0,					# 公開状態ボタンの表示形態(2:不使用/1:常時表示/0:二段階表示)
	statebuttonlabel => '公開状態',			# 公開状態ボタンのラベル
	showFuncBtnStaytop => 1,	funcBtnStaytopLabel => '先頭に固定',		# 公開状態チェック[先頭に固定]の表示とラベル
	showFuncBtnDraft => 1,		funcBtnDraftLabel => '下書き(非公開)',		# 公開状態チェック[下書き]の表示とラベル
	showFuncBtnSecret => 1,		funcBtnSecretLabel => '鍵付き',				# 公開状態チェック[鍵付き]の表示とラベル
								funcBtnSecIndLabel => '個別鍵',				# 公開状態ボタン[個別鍵]のラベル
								funcBtnSecFormLabel => '鍵入力位置',		# 公開状態ボタン[鍵入力位置]のラベル
	showFuncBtnRear => 1,		funcBtnRearLabel => '下げる(一覧外)',		# 公開状態チェック[下げる]の表示とラベル
	showseckeystate => 1,					# 鍵入力窓の挿入前に現在設定を案内(1:する/0:しない)

	showFuncBtnStyle => 0,					# 機能ボタンの表示形態(2:不使用/1:常時表示/0:二段階表示)
	funcbuttonlabel => '機能',				# 機能ボタンのラベル
	showFuncBtnSpeech => 1,		funcBtnSpeechLabel => '読み上げ',			# 機能ボタン[読み上げ]の表示とラベル
								funcBtnSpStopLabel => '止',					# 機能ボタン[止]の表示とラベル
	showFuncBtnMaxEdit => 1,	funcBtnMaxEditLabel => '編集最大',			# 機能ボタン[編集最大]の表示とラベル

	easyinputUse => 1,						# 何でも簡単入力ボタンを(1:使う/0:使わない)
	easyinputs => '😋<BR />🎉<BR />✅<BR />',		# 何でも簡単入力の中身
	easyinputWay => 0,						# 何でも簡単入力ボタンの出力方法(0:ボタン/1:プルダウンメニュー)
	...
	freelines => '',						# 名前付き1行単位フリースペース用HTML
	...
	privopguidemsg => '<p>この「てがろぐ」は、<strong>ログイン者限定表示</strong>に設定されています。すべてのページは、<strong class="important">ログインしている状態でしか表示されません</strong>。</p><p>まずは、<a href="?mode=admin">ログインして下さい</a>。</p>',		# ログイン者限定表示時のログイン案内文言
	...
	faviconuse => 0,						# Faviconの選択(0:なし/1:独自/2:内蔵)
	faviconurl => '',						# 独自FaviconのURL
	faviconfor => 0,						# Faviconの適用先(0:管理画面内のみ/1:管理画面とスキン)
	...
	loginformgid => '',						# ログインフォームの上部に表示されるメッセージ
	loginnoindex => 1,						# ログインフォームにmeta noindexを追加(1:する/0:しない)
	...
	hideenvonerror => 1,					# エラー時に環境情報を省略(1:する/0:しない)
	allowservcalcres => 0,					# サーバ側の計算資源を使う機能を有効に(0:しない/1:する)
	allowhighloadonweb => 0,				# 高い負荷のかかる処理のWeb上での実行を許可(0:しない/1:する)
	outputjsconsolelog => 0,				# JavaScriptの処理ログをコンソールに(0:出さない/1:出す)
	...
	dateexpandshtml => '',					# 日付折り畳みリストのHTMLソース (保持用)
...
use constant {
	...
	DEFAULTFAVICON => 'data:image/png;base64,',
...
my %globalFlags = (
	...
	cardlinkcode => 0,		# カード型リンク用JSを既に出力したかどうか
...
# 	<?fumyxml version="1.0" ?><datum><cat:id>info</cat:id><cat:name>情報</cat:name><cat:dscrpt>カテゴリ例1</cat:dscrpt><cat:parent>-</cat:parent><cat:posts>0</cat:posts><cat:order>10</cat:order></datum><datum><cat:id>memo</cat:id><cat:name>メモ</cat:name><cat:dscrpt>カテゴリ例2</cat:dscrpt><cat:parent>-</cat:parent><cat:posts>0</cat:posts><cat:order>20</cat:order></datum><datum><cat:id>diary</cat:id><cat:name>日記</cat:name><cat:dscrpt>カテゴリ例3</cat:dscrpt><cat:parent>-</cat:parent><cat:posts>0</cat:posts><cat:order>30</cat:order></datum>
...

{
	...
	# カテゴリ情報の分解
	if( &fcts::isFumyxml( $setdat{'categorylist'} ) == 0 ) {
		# 旧データ形式なら新形式に変換
		$setdat{'categorylist'} = &fcts::datToFumyxml( $setdat{'categorylist'}, 'cat:id', 'cat:name', 'cat:dscrpt', 'cat:parent', 'cat:posts', 'cat:order', 'cat:iconsrc' );	# 旧形式は最大でも7項目
	}
	@fcts::catdata = &fcts::expandFumyxml( ( $setdat{'categorylist'} || $firstdat{'categories'} ) );	# カテゴリ情報が何もなければデフォルト値を設定(自らの意思で白紙化した場合は宣言(旧データなら区切り文字)が含まれるのでデフォルト値にはならない)
...
{
	if(( $cp{'mode'} =~ /^view|sitemap|gallery|picts|around|export/ ) && ( -f $subfile{'blackfile'} )) {
		# 指定モードの場合で、かつ、対策ファイルが存在する場合にだけ実行
		if(( !$cp{'postid'} ) && ( &islimited() == 0 )) {
			# ただし、VIEWモードで、表示条件が限定されていない場合は例外として、実行しない
				# 製作メモ：
				# ここではパラメータの正確性チェック前なので、postidに数値以外が指定されているとislimited関数の実行時点で警告が出る。
				# それを防ぐために、まず postid の中身が偽だと判定される場合(＝0)以外ではislimited関数を実行しないようにしておく。
			return;
		}

		# BlackCheckファイルを読む
		my %blackcheck = &fcts::INIin($subfile{'blackfile'});

		# 基本設定値を読む
		my $duration = 180;		# 規定値 (180秒間に)
		my $limitFreq = 8;		# 規定値 (8回まで)
		if( exists( $blackcheck{'Duration'} )) {	$duration = $blackcheck{'Duration'};	}
		if( exists( $blackcheck{'LimitFreq'} )) {	$limitFreq = $blackcheck{'LimitFreq'};	}
		if( $duration < 10 ) { $duration = 10; }	# 小さすぎる値を修正
		if( $limitFreq < 2 ) { $limitFreq = 2; }	# 小さすぎる値を修正
		if( $duration > 86400 ) { $duration = 86400; }	# 大きすぎる値を修正

		# ブロック時間が過ぎているレコードを一括削除
		foreach my $onerec ( keys(%blackcheck) ) {
			# 全キーをループして処理
			my $onedata = $blackcheck{$onerec};		# そのキーの値を取得
			if( $onedata =~ /^[PB]/ ) {
				# 値の先頭が P(Pass) か B(Block) の場合だけ処理する
				my @rec = split(/,/,$onedata);
				if( defined( $rec[2] ) ) {
					# 3番目の要素が存在する場合のみ
					if( $rec[2] < time ) {
						# 現在時刻を過ぎていたら、そのレコードを削除する
						delete( $blackcheck{$onerec} );
					}
				}
			}
			# ※値の先頭がF(Free)の場合やそれ以外のデータの場合は永続記録するのでここではスルー。
		}

# 	データファイルの仕様：
# 		(IPアドレス)=P,(1回目のアクセス時刻),(カウントリセット時刻),(これまでのアクセス回数)		←Pass:制限内アクセスの単なる記録でしかない時点
# 		(IPアドレス)=B,(1回目のアクセス時刻),(ブロック解除予定時刻),(これまでのアクセス回数)		←Block:制限超過でアクセス制限されている時点
# 		(IPアドレス)=F																				←Free:常時無制限

		# IPアドレスを得る
		my $nowip = &fcts::forsafety( $ENV{REMOTE_ADDR} );

		# ブロックフラグ・制限解除日時記憶用
		my $blockFlag = 0;
		my $liftingTime = '';

		# そのIPアドレスが記録されているかどうかを確認
		if( exists( $blackcheck{$nowip} )) {
			# あれば
			if( $blackcheck{$nowip} =~ /^[PB]/ ) {
				# 値の先頭が P(Pass) か B(Block) の場合だけ処理する
				my ($status, $firsttime, $resettime, $count) = split(/,/,$blackcheck{$nowip});
				if( $status eq 'P' ) {
					# 現在まではブロックされていない場合は、カウントを増やす
					$count++;
					if( $count > $limitFreq ) {
						# その結果、許容アクセス頻度の規定値を超えた場合は、ブロックする
						$status = 'B';
						$blockFlag = 1;
						# ブロック解除予定時刻を生成する
						$resettime = (time + $duration);
					}
					# 許容アクセス頻度の規定値を超えていない場合は、カウントリセット時刻は更新しない
				}
				else {
					# 既にブロックされている場合、カウントを増やし、ブロック解除予定時刻も延長する
					$count++;
					$resettime = time + $duration;
					$blockFlag = 1;
				}
				...
				# 記録を更新する
				$blackcheck{$nowip} = $status . ',' . $firsttime . ',' . $resettime . ',' . $count;
				$liftingTime = $resettime;
			}
			# ※値の先頭がF(Free)の場合やそれ以外のデータの場合はスルー。
		}
		else {
			# なければ新規にカウント
			$blackcheck{$nowip} = 'P,' . time . ',' . (time + $duration) . ',1';
		}

		# ------------------------------
		# BlackCheckファイルを出力(更新)
		# ------------------------------
		# 書き込み
		open(SETOUT, "+< $subfile{'blackfile'}") or &errormsg("$subfile{'blackfile'}ファイルへの出力に失敗しました。書き込み権限が正しく付与されているかどうか確認して下さい。");
		...
		for my $name (sort keys %blackcheck) {
			print SETOUT "$name=$blackcheck{$name}\n";
		}
		...
		if( $blockFlag == 1 ) {
			# 制限解除日時の文字列を作る
			my $kaijoDate = &fcts::getdatetimestring( $liftingTime );
			# HTMLを作る
			my $errHtml = &fcts::tooneline( q|
				<html>
					<head><title>429 Too Many Requests</title><meta name="viewport" content="initial-scale=1"></head>
					<body>
						<h1>429 Too Many Requests</h1>
						<p>Please reduce the frequency of your visits. This type of page cannot be accessed more than | . $limitFreq . q| times within any | . $duration . q|-second period.</p>
						<div style="background:#eee; margin:1em auto; max-width:1200px; padding:1em; border:1px solid #fcc; border-radius:1em;">
							<p><b>《高負荷対策のためのアクセス制限》</b></p>
							<p>
								同一IPアドレスからの高頻度アクセスによる負荷軽減のため、あなたのIPアドレスは、主なページを除くサブコンテンツに対してだけ、一時的に閲覧をブロックされています。
								現在の設定では、同一IPアドレスから<b> | . $duration . q|秒間に | . $limitFreq . q|回</b>を超えてサブコンテンツにアクセスすることはできません。（メインページやRSSなどの主なページに対しては制限はありません。）
								これは主にBotに対する制限ですが、設定値によっては人間でも該当してしまう可能性があります。
							</p>
							<p style="background:crimson;color:white; padding:0.25em;">➡ 制限が解除される<b> | . $kaijoDate . q| </b>以降に再度アクセスして下さい。</p>
							<p>上記の制限解除日時よりも前に再度アクセスすると、解除カウントがリセットされ、その時点からまた新たに | . $duration . q|秒間の制限が課されますのでご注意下さい。</p>
							<p style="margin-top:1em; padding-top:1em; border-top:1px dashed #ccc;">※もしあなたが人間の閲覧者で、あくまでも現実的な頻度でのアクセスだったと思えるなら、管理者までご一報下さい。制限値の見直しに役立ちます。</p>
						</div>
					</body>
				</html>
			|);
	...
	# -------------------------------------
	# UAにbotが含まれる場合の(簡易)強制対策
	# -------------------------------------
	if(( exists($ENV{'HTTP_USER_AGENT'}) ) && ( index($ENV{'HTTP_USER_AGENT'},'bot') >= 0 )) {
		...
		if( $targets ne '' ) {
			...
			print '<html><head><title>Forbidden</title><meta name="viewport" content="initial-scale=1"></head><body><h1>403 Forbidden: No need to crawl</h1><p>《高負荷対策》パラメータの組み合わせで同じ本文が何度も取得されるのを防止するための、クローラー向けアクセス制限です。このエラーで不都合な状況があれば、てがろぐ作者までお知らせ下さい。</p><p>※もしあなたが人間の閲覧者なら、クローラーだと誤解されにくいUA名で再度アクセスして下さい。</p></body></html>';
			exit;
		}
...
sub modeRss
{
	...
	$setdat{'urlexpandvideo'} = 0;		# 動画を埋め込まない
...
sub modeApi
{
	...
	if( $permittedid ne '' ) {
		# ログインしている場合
		$apiDat{'ISLOGGEDIN'}	= 'YES';													# ログイン中かどうか(YES/NO)
		$apiDat{'USERLEVEL'}	= &fcts::forsafety( &fcts::getUserDetail($permittedid,1) );	# ユーザの権限(非ログインなら0)
		$apiDat{'USERNAME'}		= &fcts::forsafety( &fcts::getUserDetail($permittedid,2) );	# ユーザ名(非ログインなら空文字列)
		$apiDat{'USERICON'}		= &fcts::forsafety( &fcts::getUserDetail($permittedid,4) );	# ユーザアイコン(非ログインなら空文字列)
		$apiDat{'USERID'}		= &fcts::forsafety( $permittedid );							# ユーザID(非ログインなら空文字列)
	}

	my $ret = '';
	my $headerType = 'text/plain';

	if( $apiParam{'item'} =~ m/([A-Z]+)/ ) {
		# 大文字で指定されていたら、それに対応するキーを出力する
	    my $key = $1;
	    if(exists $apiDat{$key}) {
			$ret = $apiDat{$key};
	    }
	    else {
	    	$ret = '*** UNDEFINED KEY';
    ...
    elsif( $apiParam{'item'} eq 'all' ) {
		# type別に出力
		if(( $apiParam{'type'} eq 'ini' ) || ( $apiParam{'type'} eq '' )) {
			$ret = &fcts::outputINI( \%apiDat );
		}
		elsif( $apiParam{'type'} eq 'json' ) {
			$ret = &fcts::outputJSON( \%apiDat );
			$headerType = 'application/json';
		}
		elsif( $apiParam{'type'} eq 'tag' ) {
			$ret = &fcts::outputTAG( \%apiDat );
		}
		elsif( $apiParam{'type'} eq 'yaml' ) {
			$ret = &fcts::outputYAML( \%apiDat );
		}
		else {
			$ret = '*** UNDEFINED TYPE';
		}
    }
    elsif( $apiParam{'item'} eq '' ) { $ret = '*** "item" PARAMETER REQUIRED'; }
	else { $ret = '*** UNDEFINED PARAMETER'; }
...
sub modeGallery
{
	...
	$setdat{'imageshowallow'} = 1;		# 画像表示は許可
	$setdat{'imagefullpath'} = 1;		# 画像は絶対URLで出力
	$setdat{'insertalttext'} = 0;		# 画像出力の省略時に「(画像省略)」とは出力しない
	$setdat{'outputlinkkeepskin'} = 0;	# 本文中のリンクで一時適用中のスキンを維持しない (skinパラメータを併用する場合のため)

	# 日付境界バーの設定を上書き (※使う場合は「ページの設定」側の設定値(1～3)に従うので、ここでは0の場合だけ上書きする)
	if( $setdat{'gallerydatebar'} == 0 ) {
		# 使わない場合
		$setdat{'separatepoint'} = 0;
	}

	# 画像を(原寸画像への)リンクにするかどうかの設定を上書き
	$setdat{'imagetolink'} = $setdat{'galleryimgtolink'};
...
sub loginCheckOnPrivateOperation
{
	...
	if( $permittedid eq '' ) {
		...
		my $guidemsg = &fcts::trim($setdat{'privopguidemsg'});
		if( $guidemsg eq '' ) {
			# 独自メッセージが設定されていなければ
			$guidemsg = '<!-- ログインを促すメッセージが何も指定されていません。 -->';
		}
		&showadminpage('LOGIN REQUIRED','', $guidemsg ,'BL');
		exit;
	}
}
...
sub modeView
{
	...
	# ▽第2段階
	elsif(( $cp{'mode'} eq 'view' ) && ( $cp{'skindir'} eq '' ) && ($cp{'cat'} ne '')) {
		# 3条件：(1)VIEW MODEで実行されている場合で、(2)スキンがパラメータでは指定されていない場合で、(3)カテゴリ限定表示時の場合	※カテゴリ以外の限定はあっても適用する
		my $catid = &fcts::forsafety($cp{'cat'});	# ※注：複数カテゴリの指定時にはここに複数カテゴリのIDが同時に入るため、何にもマッチしなくなるので、限定スキンは適用されない。
		if( &fcts::getCategoryDetail($catid, 'cat:useskin') ) {
			# カテゴリ限定時に、指定のスキンを適用する設定なら
			my $catskin = &fcts::getCategoryDetail($catid, 'cat:skin');
			if( &fcts::trim($catskin) ne '' ) {
				# スキン名が指定されているなら、それを採用する
				&overrideskins($catskin,'カテゴリ限定表示時のVIEW');
				$setdat{'skindirectory'} = "$catskin";
			}
		}
	}
	...
	# ----------------------------------------------
	# ▼何よりも先に事前処理しておく必要のある処理群
	# ----------------------------------------------
	# スキン用コメント記法
	$cvskin =~ s/\[\[!--(.+?)--\]\]//sg;	# 改行にもマッチさせたいので s修飾子を追加している
	...
	if( $safessi < 9 ) {
		...
		if( $count > 0 ) {
			# INCLUDEしたファイルがあるなら、コメント記法の解釈を再度する。
			$cvskin =~ s/\[\[!--(.+?)--\]\]//sg;	# 改行にもマッチさせたいので s修飾子を追加している
		}
	}
	...
	my %isExist = (
		...
		USERLIST		=> ( $cvskin =~ m/\[\[USERLIST/ ) || 0,			# ユーザリストの掲載がある場合（同上）
		...
		SYSINFO			=> ( $cvskin =~ m/\[\[SYS:/ ) || 0,				# システム情報の掲載がある場合（同上）
		...
		FREELINE		=> ( $cvskin =~ m/\[\[FREELINE:/ ) || 0,		# 名前付き1行単位フリースペースの掲載がある場合（同上）
	...
	# ▽第2段階：スキン側の設定で上書き
	# ---
	my $ignorePPPBySkin = 0;

	# ▽第1段階
	if(($cp{'cat'} ne '') && ( &islimited('cat') == 0 )) {
		# カテゴリ限定表示時で、カテゴリ以外の限定がない場合
		my $catid = &fcts::forsafety($cp{'cat'});
		if( &fcts::getCategoryDetail($catid, 'cat:useppp') ) {
			# カテゴリ限定時に、1ページ当たりの表示数を上書きする設定なら、それを採用する
			my $overridepagenum = &fcts::getCategoryDetail($catid, 'cat:ppp');
			if( $overridepagenum ) {
				# 1ページあたりの表示数が設定されているなら、
				$setdat{'entryperpage'} = &fcts::inrange($overridepagenum, 1, 10000);	# 安全のため1～10000の範囲に収める
				# スキン側に指定があるとき、上書きして良いのかどうかを確認しておく
				$ignorePPPBySkin = &fcts::getCategoryDetail($catid, 'cat:pppforce');	# 0=上書き許可／1=上書き拒否
			}
		}
	}
	# ▽第2段階
	if(( $ignorePPPBySkin == 0 ) && ( $setdat{'eppoverride'} == 1 ) && ( $cvskin =~ /\[\[TEGALOG:(\d+)\]\]/ )) {
		# スキンによる表示数の上書きが許可されている場合で、
		# かつ、スキン側に1ページ当たりの表示数が指定されている場合には、設定を上書き
	...
	# 名前付き1行単位フリースペースの挿入
	if( $isExist{'FREELINE'} ) {
		# 名前付き1行単位フリースペースのデータを分解して連想配列にしておく
		my %namedFreeLines = parseNamedFreelinesStrings($setdat{'freelines'});

		# 該当する名前付き1行単位フリースペースの中身を出力 (該当名がなければ何も出力しない：存在しないキーを指定してもundefが返るだけのハズ)
		$cvskin =~ s/\[\[FREELINE:(.+?)\]\]/&tagcheckforsafe($namedFreeLines{$1})/eg;
	}
	...
	if( $isExist{'DATEBOX'} ) {
		# 日付指定窓の掲載がある場合のみに処理：
		if( $setdat{'dateselecthtml'} eq '' || $setdat{'datelisthtml'} eq '' || $setdat{'dateexpandshtml'} eq '' ) {
		...
		$dateselectform	= &fcts::albaa( $dateselectform );
		my $datelinks 	= &fcts::albaa( $setdat{'datelisthtml'} );
		my $dateexpands = &fcts::albaa( $setdat{'dateexpandshtml'} );

		$cvskin =~ s/\[\[DATEBOX\]\]/$datelinks\n$dateselectform/g;	# 両方
		$cvskin =~ s/\[\[DATEBOX:PULL\]\]/$dateselectform/g;	# 日付検索窓
		$cvskin =~ s/\[\[DATEBOX:LIST\]\]/$datelinks/g;			# 日付リンクリスト
		$cvskin =~ s/\[\[DATEBOX:EXPAND\]\]/$dateexpands/g;		# 日付折り畳みリスト

		# 指定モード用リンクの出力
		$cvskin =~ s/\[\[DATEBOX:PULL:([A-Z]+)\]\]/&linkadjustbymode($1,$dateselectform)/eg;	# 日付検索窓
		$cvskin =~ s/\[\[DATEBOX:LIST:([A-Z]+)\]\]/&linkadjustbymode($1,$datelinks)/eg;			# 日付リンクリスト
		$cvskin =~ s/\[\[DATEBOX:EXPAND:([A-Z]+)\]\]/&linkadjustbymode($1,$dateexpands)/eg;		# 日付折り畳みリスト
	}
	...
	if( $isExist{'IMAGELIST'} ) {
		...
		if( $rescode > 0 ) {
			...
			# 読めなかったら
			$cvskin =~ s/\[\[IMAGELIST(.*?)\]\]/<span class="error">画像保存用ディレクトリが存在しないか、画像インデックスの生成に失敗したか、または読み取りに失敗しました。<\/span>/g;
		}
	}
	...
	if( $isExist{'LATESTLIST'} ) {
		...
		if( $setdat{'latestlisthtml'} eq '' ) {
			...
			&updatelatestlist( &fcts::albaa( &makelatestlist( $setdat{'latestlistup'} , $setdat{'latestlistparts'} , $setdat{'latesttitlecut'} )));
		}

		# 新着リストを挿入 (キャッシュがあればキャッシュから)
		$cvskin =~ s/\[\[LATESTLIST\]\]/&fcts::albaa( $setdat{'latestlisthtml'} )/eg;

		# カテゴリ限定新着リストを挿入 (常に動的生成)
		$cvskin =~ s/\[\[LATESTLIST:CAT\((.+?)\)\]\]/&fcts::albaa( &makelatestlist( $setdat{'latestlistup'} , $setdat{'latestlistparts'} , $setdat{'latesttitlecut'} , $1 ))/eg;

		# 新着リストを指定モード用リンクで挿入 (キャッシュがあればキャッシュをベースに加工)
		$cvskin =~ s/\[\[LATESTLIST:([A-Z]+)\]\]/&fcts::albaa( &linkadjustbymode($1,$setdat{'latestlisthtml'}))/eg;
	}
	...
	if( $isExist{'HASHTAGLIST'} ) {
		...
		$cvskin =~ s/\[\[HASHTAGLIST\]\]/[[HASHTAG:LIST]]/g;	# 旧記法を新記法に変換
		$cvskin =~ s/\[\[HASHTAG:(LIST|PULL)(\((\d+)\+\))?(:JS)?(:([A-Z]+))?\]\]/&makeHashtaglist($1,$3,$4,$6)/eg;
	}
	...
	if( $isExist{'USERLIST'} ) {
		# ユーザリストの掲載がある場合のみ
		my $permittedid = &fcts::checkpermission(1);		# 読み取り専用モードで認証を確認(ログイン中のIDを得る)
		my $linksuffix = '';	# &makeQueryString('');		# 恒常付加パラメータがあるようなら得ておく

		$cvskin =~ s/\[\[USERLIST\]\]/&fcts::getCustomUserList($linksuffix, '<N>', $permittedid, $cp{'userid'} )/eg;		# ユーザリストを挿入
		$cvskin =~ s/\[\[USERLIST:(.+?)\]\]/&fcts::getCustomUserList($linksuffix, $1, $permittedid, $cp{'userid'} )/eg;		# ユーザリストを挿入
	}
	...
	# -------------
	# Faviconの挿入
	if(( $setdat{'faviconuse'} != 0 ) && ( $setdat{'faviconfor'} == 1 )) {
		# 「指定しない」以外の設定で、管理画面だけでなくスキンにも出力する設定の場合
		$cvskin =~ s/\[\[FAVICON\]\]/&outputFaviconLink()/eg;	# Favicon用link要素を(あれば)挿入
		$cvskin =~ s/\[\[FAVICON:DEF\((.+?)\)\]\]/&outputFaviconLink($1)/eg;	# Favicon用link要素を挿入：ユーザ指定がなければスキンに記載されたデフォルト値を使う
	}
	else {
		# それ以外の設定の場合
		$cvskin =~ s/\[\[FAVICON\]\]//g;	# 記述を削除
		$cvskin =~ s/\[\[FAVICON:DEF\((.+?)\)\]\]/'<link rel="icon" href="' . &fcts::forsafety($1) . '">'/eg;	# スキン側に記載されたデフォルト値を使う
	}
	...
	if( $isExist{'SYSINFO'} ) {
		...
		if( $permittedid ne '' ) {
			# ログインしている場合
			$isloggedin = 'YES';
			$loginuserlevel	= &fcts::forsafety( &fcts::getUserDetail($permittedid,1) );
			$loginusername	= &fcts::forsafety( &fcts::getUserDetail($permittedid,2) );
			$loginusericon	= &fcts::forsafety( &fcts::getUserDetail($permittedid,4) );
			$permittedid = &fcts::forsafety( $permittedid );
		}
		$cvskin =~ s/\[\[SYS:ISLOGGEDIN\]\]/$isloggedin/g;		# ログイン中かどうか(YES/NO)
		$cvskin =~ s/\[\[SYS:USERLEVEL\]\]/$loginuserlevel/g;	# ユーザの権限(非ログインなら0)
		$cvskin =~ s/\[\[SYS:USERNAME\]\]/$loginusername/g;		# ユーザ名(非ログインなら空文字列)
		$cvskin =~ s/\[\[SYS:USERICON\]\]/$loginusericon/g;		# ユーザアイコン(非ログインなら空文字列)
		$cvskin =~ s/\[\[SYS:USERID\]\]/$permittedid/g;			# ユーザID(非ログインなら空文字列)
	}
	...
	$cvskin =~ s/\[\[GALLERY:NAME\]\]/&fcts::forsafety($setdat{'galleryname'})/eg;	# ギャラリーモードの名称
	...
	$cvskin =~ s/\[\[PICTS:NAME\]\]/&fcts::forsafety($setdat{'pictsmodename'})/eg;	# 画像一覧モードの名称
	...
	$cvskin =~ s/\[\[SITEMAP:NAME\]\]/&fcts::forsafety($setdat{'sitemappageyname'})/eg;	# サイトマップページモードの名称
	...
	# 最終投稿日時：
	if( $cvskin =~ m/\[\[INFO:LATESTPOSTDATE/ ) {
		# 最終投稿日時を取得する必要がある場合のみ取得
		if( ! $setdat{'latestpostdate'} ) {
			# 記録されていなければ生成する
			&reclatestpostdate( &getlatestpostdate() );
		}
		$cvskin =~ s/\[\[INFO:LATESTPOSTDATE\]\]/$setdat{'latestpostdate'}/g;
		$cvskin =~ s/\[\[INFO:LATESTPOSTDATE:(.+?)\]\]/&arrangeDateStr($1,$setdat{'latestpostdate'})/eg;

		if( $cvskin =~ m/\[\[INFO:LATESTPOSTDATE\(NOWCOND\)/ ) {
			# 現在の条件に限定した最終投稿日時を取得する必要がある場合のみ取得
			my $lpdInNowCond = &getlatestpostdate(@applydata);

			$cvskin =~ s/\[\[INFO:LATESTPOSTDATE\(NOWCOND\)\]\]/$lpdInNowCond/g;
			$cvskin =~ s/\[\[INFO:LATESTPOSTDATE\(NOWCOND\):(.+?)\]\]/&arrangeDateStr($1,$lpdInNowCond)/eg;
		}
	}
...
# -------------------------------------------------------------
# 名前付き1行単位フリースペースのデータを分解して連想配列にする
# -------------------------------------------------------------
sub parseNamedFreelinesStrings
{
	my $tagIni = shift @_ || '';

	my %tags;

	foreach my $line ( split( /<br>/, $tagIni ) ) {
		# 元データは改行コードではなく <br> で区切られているので、<br>で分解する
		# コロン記号が含まれていない行は無視する
		next unless $line =~ /:/;
...
# 		my $user	= &fcts::forsafety( &fcts::getcontent($onerec,'user') );
		...
		if( $count < $showimgs ) {
			...
			# 出力HTMLを作る
			$rethtml .= "<span \n" . 'class="imagelistitem">' . $linkhtmlA . $imghtml . $linkhtmlB . '</span>';
			$count++;
		}
		else {
			# 表示する分が過ぎたらループを終わる
...
sub outputUserIcon
{
	...
	if( $imgsource eq '' ) {
		...
		# 指定画像のURLを調整
		if( $imgsource =~ m/^http:\/\/.+/ ) {
			# もし指定画像が http:// で始まっているなら、ドメイン名を除いた絶対パス記法に変える (httpsの場合はあえて変える必要はない)
			if( &fcts::checkSameDomain( $cgifullurl, $imgsource ) == 1 ) {
				# CGI稼働URLと同じドメインなら
...
# ------------------------------------------
# 表示条件が限定されているかどうかのチェック	引数:判定に考慮しない要素
# ------------------------------------------	返値:0=何も限定されていない／1=限定されている
sub islimited
{
	my $ignore = shift @_ || '';

	my %ig = map { $_ => 0 } qw(postid datelim search hasgtag cat userid);		# 連想配列の初期化 (項目種類が増えた場合はqw内に空白区切りで追記すれば良い)
	$ig{$_} = 1 for grep { $ignore =~ /$_/ } keys %ig;							# 無視するキーワードの要素にだけ、フラグを立てる

	# 無視しない項目だけをチェック
	if(( $ig{'postid'} == 0 )	&& ($cp{'postid'} > 0))		{ return 1; }
	if(( $ig{'datelim'} == 0 )	&& ($cp{'datelim'} ne ''))	{ return 1; }
	if(( $ig{'search'} == 0 )	&& ($cp{'search'} ne ''))	{ return 1; }	# 注意：postsパラメータが付加されている場合も、searchとして扱われる。
	if(( $ig{'hasgtag'} == 0 )	&& ($cp{'hasgtag'} ne ''))	{ return 1; }
	if(( $ig{'cat'} == 0 )		&& ($cp{'cat'} ne ''))		{ return 1; }
	if(( $ig{'userid'} == 0 )	&& ($cp{'userid'} ne ''))	{ return 1; }
...
sub extractApplyData
{
	...
	# ▽第1段階
	if( $nowlimited == 0 ) {
		# 何も限定されていない場合で、指定投稿の先頭固定が指定されていれば、移動させておく（※モードがVIEWかSITEMAPの場合のみ／RSS等では無視する）
	...
	# ▽第2段階
	elsif(($cp{'cat'} ne '') && ( &islimited('cat') == 0 ) && ($cp{'mode'} ne 'rss')) {
		# カテゴリ限定表示時で、カテゴリ以外の限定がない場合で、RSSモードではない場合
		my $catid = &fcts::forsafety($cp{'cat'});
		if( &fcts::getCategoryDetail($catid, 'cat:useleads') ) {
			# カテゴリ限定時に、先頭固定する設定なら、移動させておく
			my $catleadposts = &fcts::getCategoryDetail($catid, 'cat:leads');
			&putpoststohead( $catleadposts );		# この関数で、グローバル変数 @xmldata の中身が書き換えられる点に注意。
		}
	}
...
sub expandDataBySkin
{
	...
	# -------------------------------------------------------
	# スキン用コメント記法
	$inskin =~ s/\[\[!--(.+?)--\]\]//sg;	# 改行にもマッチさせたいので s修飾子を追加している
	...
	# 名前付き1行単位フリースペースの挿入
	# -----------------------------------
	if( $inskin =~ m/\[\[FREELINE:/ ) {
		# 名前付き1行単位フリースペースの記述がある場合のみ
		# 名前付き1行単位フリースペースのデータを分解して連想配列にしておく
		my %namedFreeLines = parseNamedFreelinesStrings($setdat{'freelines'});

		# 該当する名前付き1行単位フリースペースの中身を出力 (該当名がなければ何も出力しない：存在しないキーを指定してもundefが返るだけのハズ)
		$inskin =~ s/\[\[FREELINE:(.+?)\]\]/&tagcheckforsafe($namedFreeLines{$1})/eg;
	}
	...
	foreach my $oneclip (@_) {
		...
		if( $setdat{'allowlinebreak'} == 1 ) {
			$comment =~ s|&lt;br /&gt;|<br />|g;	# 改行を改行として表示	(ここでソース上でも本当に改行を入れると文字装飾処理の一部が正しく動作しなくなるので注意)
		}
		else {
		...
		if( $setdat{'searchhighlight'} != 0 ) {
			...
			if( @searchwords ) {
				...
				# 事前準備：script要素を一時的に退避	※script要素があった場所は空文字列に置き換える
				my $evacuatedScripts = '';
				$comment =~ s{<script\b[^>]*?>.*?</script>}{
					$evacuatedScripts .= "\n" . $&;
					""
				}egis;
				...
				# 事後回復：退避したscript群があるならコメント本文の最後に連結する
				if( $evacuatedScripts ne '' ) {
					$comment .= "\n<!-- Evacuated Scripts: -->" . $evacuatedScripts;
				}
			}
		}
		...
		if( $flagLock && ( $setdat{'secretkeystatus'} > 0 )) {
			...
			my $line1 = '';
			if( $showKeyForm ) {
				# 残す範囲が指定されているかどうか(＝本文中に [SYS:KEYFORM] の記述があるかどうか)
				if(( $setdat{'allowanylineinsecret'} ) && ( $comment =~ m/&#91;SYS:KEYFORM&#93;/ )) {
					# KEYFORMの位置指定記述がある場合
					if( $comment =~ m/^((.*?)&#91;SYS:KEYFORM&#93;)/ ) {
						# 本文先頭から記法までを残す
						$line1 = $1;
						$line1 =~ s/&#91;SYS:KEYFORM&#93;//;		# ただし、記法そのものは消す
					}
				}
				# または 1行目を残すかどうか
				elsif( $setdat{'allowonelineinsecret'} ) {
				...
				# 本文中に [SYS:KEYFORM] の記述がある場合は消す
				$comment =~ s|&#91;SYS:KEYFORM&#93;|<span id="HiddenPoint"></span>|;	# 代わりにidを割り振っておく
			}
		...
		$mtemplate =~ s|\[\[ONEPICT:([0-9]+):IMG\]\]|&extractImageAttr(&pinpointarray($1,'',@imghtmls))|eg;
		$mtemplate =~ s|\[\[ONEPICT:([0-9]+):VALUE:([A-Z]+)\]\]|&extractImageAttr(&pinpointarray($1,'',@imghtmls),$2)|eg;
			...
			if( $comment =~ m/&lt;&gt;/ ) {
				...
				foreach my $sepcomment (@separatedcomments) {
					if( $comment eq '' ) {
						...
						my %opts = (
							'kind'		=> '続きを読む' ,					# 1：種別名
							'moreid'	=> $moreid ,						# 2：隠す領域1つ1つに割り当てるユニークID
							'morenum'	=> $morenum ,						# 3：隠す番号
							'opentag'	=> $readmorebuttontag ,				# 4：隠すボタンを作る要素名
							'hidetag'	=> $readmorehidetag ,				# 5：隠す範囲を出力する要素名
							'stylenum'	=> $setdat{'readmorestyle'} ,		# 6：展開する範囲の表示方法(0:inline/1:inline-block/2:block)
							'openlabel'	=> $setdat{'readmorebtnlabel'} ,	# 7：展開ボタンのラベル
							'hidelabel'	=> $setdat{'readmorecloselabel'} ,	# 8：畳むボタンのラベル
							'needhide'	=> $setdat{'readmorecloseuse'}		# 9：畳むボタンの出力が(1:必要/0:不要)
						);

						$comment .= &outputHiddenArea(
							\%opts,			# 各情報を格納した連想配列へのリファレンス
							$sepcomment		# 隠される本文(Separated Comment)
						);
					}
					$spcomcount++;
				}
...
sub makeSecretkeyInputForm
{
	...
	# 維持するパラメータを作る (※本来使われるのはinput要素のhiddenパラメータだけだが、URLにも反映されないと見た感じがおかしいのでURLパラメータも作って加える)
	my $hiddenParams = '';
	my $urlParams = '';
	if( $cp{'mode'} ne 'view' )	{ $hiddenParams .= '<input type="hidden" name="mode" value="' . &fcts::forsafety($cp{'mode'}) .			'">'; $urlParams .= '&amp;mode='	. &fcts::forsafety($cp{'mode'}) ; }
	if( $cp{'cat'} ne '' )		{ $hiddenParams .= '<input type="hidden" name="cat" value="' . &fcts::forsafety($cp{'cat'}) .			'">'; $urlParams .= '&amp;cat='		. &fcts::forsafety($cp{'cat'}) ; }
	if( $cp{'hasgtag'} ne '' )	{ $hiddenParams .= '<input type="hidden" name="hasgtag" value="' . &fcts::forsafety($cp{'hasgtag'}) .	'">'; $urlParams .= '&amp;tag='		. &fcts::forsafety($cp{'hasgtag'}) ; }
	if( $cp{'datelim'} ne '' )	{ $hiddenParams .= '<input type="hidden" name="datelim" value="' . &fcts::forsafety($cp{'datelim'}) .	'">'; $urlParams .= '&amp;date='	. &fcts::forsafety($cp{'datelim'}) ; }
	if( $cp{'search'} ne '' )	{ $hiddenParams .= '<input type="hidden" name="search" value="' . &fcts::forsafety($cp{'search'}) .		'">'; $urlParams .= '&amp;q='		. &fcts::forsafety($cp{'search'}) ; }
	if( $cp{'userid'} ne '' )	{ $hiddenParams .= '<input type="hidden" name="userid" value="' . &fcts::forsafety($cp{'userid'}) .		'">'; $urlParams .= '&amp;userid='	. &fcts::forsafety($cp{'userid'}) ; }
	if( $cp{'order'} ne 'straight' ) { $hiddenParams .= '<input type="hidden" name="order" value="' . &fcts::forsafety($cp{'order'}) .	'">'; $urlParams .= '&amp;order='	. &fcts::forsafety($cp{'order'}) ; }

	my $cgipath = &getCgiPath("postid=$postid" . $urlParams);	# URLにも反映させるために postid パラメータを加えているが、実際のpostidは hidden フォームからpostで送られている方が使われる点に注意。
	$ret .= qq|
	<!-- 鍵入力フォーム -->
	<form action="$cgipath" method="post" class="passkeyform">
 		$errmsg<input type="hidden" name="postid" value="$postid">$hiddenParams
...
sub cutsuperfluouspartsforrssogp
{
	...
	# 折り畳む領域は、ラベル部分だけを残して他は含めない
	$comment =~ s|<details.+<summary.*?>(.+?)</summary>.*?</details>| $1 |g;
...
sub makehighlightlist
{
	...
	my $searchstr = &fcts::trim( $cp{'search'} );	# パラメータから、前後の余分な空白は除外して得る
...
# -----------------------------------------		※「続きを読む」と「範囲を隠す」で共通
# 指定範囲を隠す部分(HTML/JavaScript)の出力		引数1：連想配列、引数2：隠される本文、引数3：展開ボタンのラベル(個別指定の場合)
# -----------------------------------------		返値：隠す全体のHTMLソース
sub outputHiddenArea
{
	my $optRef		= shift @_ || &errormsg('outputHiddenArea：指定範囲を隠す情報の連想配列リファレンスが必須');	# 引数1：オプション情報を格納した連想配列
	my $sepcomment	= shift @_ || '';	# 引数2:隠される本文(Separated Comment)
	my $argLabel	= shift @_ || '';	# 引数3:展開ボタンのラベル(個別指定がある場合用)

	# 各情報のデフォルト値 (※連想配列または引数2以降で上書きされる)
	my $kind		= '隠';						# 1：種別名
	my $moreid		= &fcts::getrandstr(7);		# 2：隠す領域1つ1つに割り当てるユニークID
	my $morenum		= '';						# 3：隠す番号
	my $opentag		= 'a';			# 4：隠すボタンを作る要素名
	my $hidetag		= 'span';		# 5：隠す範囲を出力する要素名
	my $stylenum	= 0;			# 6：展開する範囲の表示方法(0:inline/1:inline-block/2:block)
	my $openlabel	= '展開';		# 7：展開ボタンのラベル
	my $hidelabel	= '畳む';		# 8：畳むボタンのラベル
	my $needhide	= 0;			# 9：畳むボタンの出力が(1:必要/0:不要)

	my $markupset	= 0;			# マークアップ方法(0:レガシー/1:details+summary)

	# 送られてきた連想配列内に各情報があれば上書き (中身が空文字列でない場合のみ)
	if( exists $optRef->{'kind'} && $optRef->{'kind'} ne '' )			{	$kind		= $optRef->{'kind'};	}
	if( exists $optRef->{'moreid'} && $optRef->{'moreid'} ne '' )		{	$moreid 	= $optRef->{'moreid'};	}
	if( exists $optRef->{'morenum'} && $optRef->{'morenum'} ne '' )		{	$morenum 	= $optRef->{'morenum'};	}
	if( exists $optRef->{'opentag'} && $optRef->{'opentag'} ne '' )		{	$opentag	= $optRef->{'opentag'};	}
	if( exists $optRef->{'hidetag'} && $optRef->{'hidetag'} ne '' )		{	$hidetag	= $optRef->{'hidetag'};	}
	if( exists $optRef->{'stylenum'} && $optRef->{'stylenum'} ne '' )	{	$stylenum	= $optRef->{'stylenum'};	}
	if( exists $optRef->{'openlabel'} && $optRef->{'openlabel'} ne '' )	{	$openlabel	= $optRef->{'openlabel'};	}
	if( exists $optRef->{'hidelabel'} && $optRef->{'hidelabel'} ne '' )	{	$hidelabel	= $optRef->{'hidelabel'};	}
	if( exists $optRef->{'needhide'} && $optRef->{'needhide'} ne '' )	{	$needhide	= $optRef->{'needhide'};	}
	if( exists $optRef->{'markupset'} && $optRef->{'markupset'} ne '' )	{	$markupset	= $optRef->{'markupset'};	}

	# 個別用の展開ボタンラベルがあれば採用 (空白だけの場合は採用しない)
	if( &fcts::trim($argLabel) ne '' ) {
		$openlabel = $argLabel;
	}

	# ラベル名を安全化
	$openlabel  = &fcts::forsafety( $openlabel );
	$hidelabel = &fcts::forsafety( $hidelabel );

	# 展開する範囲の表示方法(0:inline/1:inline-block/2:block)
	my $readmoreblock = 'inline';
	if(		$stylenum == 1 ) { $readmoreblock = 'inline-block'; }
	elsif(	$stylenum == 2 ) { $readmoreblock = 'block'; }
	...
	if( $markupset == 1 ) {
		# ------------------------
		# details+summary構成の場合
		# ------------------------
		$ret .= qq|<!-- $kind --><details class="decorationO"><summary class="foldswitch"><span class="foldlabel">$openlabel$morenum</span></summary><div class="foldedarea">$sepcomment</div></details>|;
	}
	else {
		# ------------------
		# レガシー構成の場合
		...
		# 展開ボタンの出力(Button)
		$ret .= qq|<!-- $kind$morenum(B) --><$opentag href="#readmore" id="button4$moreid" style="display:none;" class="readmorebutton readmoreopen" onclick="document.getElementById('$moreid').style.display = '$readmoreblock'; this.style.display = 'none'; return false;">$openlabel$morenum</$opentag>|;

		# 本文の出力(Honbun)
		$ret .= qq|<!-- $kind$morenum(H) --><$hidetag id="$moreid" class="readmorearea">| . $sepcomment;
		if( $needhide != 0 ) {
			$ret .= qq|<$opentag href="#readclose" class="readmorebutton readmoreclose" onclick="document.getElementById('$moreid').style.display = 'none'; document.getElementById('button4$moreid').style.display = 'inline-block'; return false;">$hidelabel$morenum</$opentag>|;
		}
		$ret .= qq|</$hidetag>|;
...
sub getPartOfLines
{
	...
	# TITLE・BODY使用時の事前調整 (1行目に存在するとマズい記述を先送りする)
	if(( $want eq 'TITLE' ) || ( $want eq 'BODY' )) {
		if(defined $_[0] && $_[0] =~ /(<details)|(<ul)|(<[^>]+class="decorationH")/) {
			# タイトルとして存在するとマズいHTMLタグがある場合は、先頭に空行を挿入する
			unshift( @_, '' );
		}
	}
	...
	if( $limit > 0 ) {
		...
		$ret =~ s/<img .+?>/$imgomit/g;		# 画像省略
		$ret =~ s/<span class="cemoji.+?<\/span>/(絵文字)/g;		# カスタム絵文字省略
		$ret =~ s/<script>.+?<\/script>//g;	# スクリプト削除(カスタム絵文字コピー用)
		$ret =~ s/<.+?>//g;
		if( $rdec == 1 ) {
			# てがろぐ記法の数値文字参照をデコード
...
sub linkize
{
	...
	# ▼汎用動画リンク処理
	if( $setdat{'urlexpandvideo'} != 0 ) {
		# 動画ラベル付きURLなら動画として掲載
		$ts =~ s|\[[Vv][Ii][Dd][Ee][Oo](:(\S*?))?\](https?:\/\/[-_.!~*\'()a-zA-Z0-9;\/?:@&=+\$,%#]+)|&embedvideo($3,$2)|eg;
	}
...
sub makelabeledlink
{
	...
	# ▽CARDコマンド付きラベル
	my $cardjs = '';
	if( $linklabel =~ m/:CARD(-[LS])?(:|$)/ ) {
		# ラベルからコマンド部分を消す
		$linklabel =~ s/(.*):CARD(-[LS])?/$1/;
		if( $linklabel eq '' ) {
			# ラベルが指定されていない場合は仮ラベルを出す
			$linklabel = '(Loading...)';
		}
		# カードサイズ指定用のclassを作る
		my $cardsize = $2 || '';
		...
		# カードリンク用のclassを加える
		$addclass .= ' cardlink';
		# labeledlinkのclassは外す
		$lbclass = '';
		# リンク用カードを作る
		$linklabel = &fcts::tooneline(q|
			<span class="cardlinkbox| . $cardsize . q|">
				<span class="cardlinkimage"></span>
				<span class="cardlinktextbox">
					<span class="cardlinktitle">| . $linklabel . q|</span>
					<span class="cardlinkdescription">...</span>
					<span class="cardlinkurl"></span>
				</span>
			</span>
		|);
		# グローバルフラグを参照して、カード用JavaScriptが未出力なら出力する
		if( $globalFlags{'cardlinkcode'} == 0 ) {
			# 連携プログラムの存在を確認する
			if( -f $subfile{'subPHP'} ) {
				# ある
				# 連携用トークンを発行する
				my $token = &fcts::maketoken();
				# カード型リンク用のOGP読み込みJavaScriptを生成する
				$cardjs = &outputCardlinkCode($token);
				# 既出力フラグを立てる
				$globalFlags{'cardlinkcode'} = 1;
			}
			else {
				# ない：エラーを返す
				return q|<i style="| . &styleforembeddederror() . q|">カード型リンクの表示には、てがろぐ補助ツール | . &fcts::forsafety($subfile{'subPHP'}) . q| の設置とその正常稼働が必要です。</i>|;
			}
		}
	}

	return qq|<a class="url$lbclass$addclass" href="$linkurl"$addatt>$linklabel</a>$cardjs|;
}

# ---------------------------------------------
# カード型リンク用のOGP読み込みJavaScriptを返す		引数:連携用トークン
# ---------------------------------------------
sub outputCardlinkCode
{
	my $token = shift @_ || '';		# このトークンをPHPに渡す

	my $ogpGetter = $subfile{'subPHP'} . '?token=' . $token;	# このPHPにJavaScriptから取得先URLを加えてリクエストを送る
	my $defCardSize = 'cardsize-S';
	if( $setdat{'urlcardsizedef'} == 1 ) {
		# 大サイズが指定されている場合
		$defCardSize = 'cardsize-L';
	}
	my $outputLogToConsole = '0';
	if( $setdat{'outputjsconsolelog'} == 1 ) {
		# JavaScriptのログをコンソールに出力する場合
		$outputLogToConsole = '1';
	}

	# --------------------------
	# JavaScriptソースを生成する (変数ogpGetterの中身を含めるために、前半と後半に分割している)
	# --------------------------
	my $jssource = << 'EOM';
...
EOM
# --------------------------- JSソース動的生成部分:
	$jssource .= q|const defCardSize = '| . $defCardSize . q|';|;
	$jssource .= q|const outputLog = '| . $outputLogToConsole . q|';|;
# ---------------------------
	$jssource .= << 'EOM';
	const totalFets = document.querySelectorAll("a.cardlink").length;	/* 総個数を得ておく */
	document.querySelectorAll("a.cardlink").forEach(async (card) => {
		const pageUrl = card.getAttribute("href");
EOM
# --------------------------- JSソース動的生成部分:
	$jssource .= q|const apiUrl = `| . $ogpGetter . q|&order=${fetCount}&total=${totalFets}&fetch=${encodeURIComponent(pageUrl)}`;|;
# ---------------------------
	$jssource .= << 'EOM';

		/* 処理連番 */
		let waitms = (fetCount - 1) * 50;			/* 待機時間 */
		let fetLabel = '{Tegalog-TegUp:' + fetCount++ + '} ';	/* コンソール報告用番号表記 */

		/* ちょっと待機してリクエストをずらす */
		await wait(waitms);

		/* リクエスト */
		outputLog == 1 && console.log( fetLabel + "Request:", apiUrl,'wait:', waitms, '(ms)');

		try {
			const response = await fetch(apiUrl);
			const text = await response.text();

			let jsonData, jsonPart = "", extraPart = "";
			try {
				const jsonEndIndex = text.indexOf("}") + 1;
				jsonPart = text.substring(0, jsonEndIndex);
				extraPart = text.substring(jsonEndIndex);
				jsonData = JSON.parse(jsonPart);
			}
			catch (e) {
				/* JSONが返ってこなかった場合 */
				if((text.startsWith('<!DOCTYPE html>')) && (text.includes('TegUp 1'))) {
					/* TegUp Ver.1 の画面が返ってきた場合は、TegUpのバージョンが古いと判断：エラーを表示して終わる */
					const emsg = '(tegup.phpのバージョンが古い可能性があります。TegUp Ver.2以降が設置されているか確認して下さい。)';
					card.querySelector(".cardlinkdescription").textContent = emsg;
					console.warn(fetLabel + "Invalid response:", emsg);
				}
				else {
					/* JSONではない返値はコンソールに出力して、処理を終わる */
					console.warn(fetLabel + "JSON parse error or invalid response:", text);
				}
				return;
			}

			if(extraPart.trim()) {
				console.info(fetLabel + "Extra data received:", extraPart);
			}

			const ogTitle = jsonData.title;
			const ogDescription = jsonData.description;
			const ogImage = jsonData.image;
			const cardSize = jsonData.card;
			const recDate = jsonData.recorded_date;

			/* 出力したOGP情報がキャッシュされた日時の報告 */
			outputLog == 1 && console.log(fetLabel + "Cached Date:", recDate);

			/* ------------------ */
			/* ▼タイトルと概要文 */
			/* ------------------ */
			if( ogTitle !== '' ) {
				/* タイトル文字列が得られた場合にだけ置き換える (得られなかった場合は投稿時のラベルのままにする) */
				card.querySelector(".cardlinktitle").textContent = ogTitle;
			}
			card.querySelector(".cardlinkdescription").textContent = ogDescription;

			if(( ogTitle === '' ) && ( ogDescription === '' )) {
				/* タイトルと概要文の両方がない場合は、情報を取得できなかったと解釈してアラートを出す (タイトルには投稿時のラベルが入っているので、概要文にだけ) :ただ、基本的にはTegUp側で概要部分に何らかの情報を出すので、この機能はフェイルセーフ的なもの。 */
				card.querySelector(".cardlinkdescription").textContent = '(このリンク先には取得可能なテキスト情報がありませんでした)';
			}
			...
			card.querySelector(".cardlinkurl").textContent = urlObj.hostname;

			/* ------ */
			/* ▼画像 */
			/* ------ */
			const imageElement = card.querySelector(".cardlinkimage");
			if( ogImage !== '' ) {
				/* 画像がある場合にだけ背景画像に指定 */
				imageElement.style.backgroundImage = `url('${ogImage}')`;
			}

			/* -------------- */
			/* ▼カードサイズ */
			/* -------------- */
			const cardBox = card.querySelector(".cardlinkbox");
			if(!cardBox.classList.contains("cardsize-S") && !cardBox.classList.contains("cardsize-L")) {
				/* 既にカードサイズが指定されている場合は何もしない */
				cardBox.classList.remove("cardsize-None");
				if(cardSize === "summary") {
					cardBox.classList.add("cardsize-S");
				}
				else if(cardSize === "summary_large_image") {
					cardBox.classList.add("cardsize-L");
				}
				else {
					cardBox.classList.add(defCardSize);
				}
			}
		}
		catch (error) {
			console.warn(fetLabel + "Error fetching OGP data:", error);
			card.querySelector(".cardlinkdescription").textContent = '(連携ツール tegup.php が正常に実行されなかった可能性があります。TegUp側のバージョンや正常稼働を確認して下さい。コンソールも参考にして下さい。)';
			warnCount++;
			if(( outputLog == 0 ) && ( warnCount > 0 )) {
				console.log('《参考情報》警告対象のリクエストを知るには、てがろぐ管理画面の設定→システム設定→【高度な設定】で「OGP取得時のリクエストログをConsoleに出力する」項目をONにして下さい。');
				warnCount -= 10;
			}
		}
	});
...
EOM
	...
	return &fcts::tooneline( $jssource );
}
...
sub comdecorate
{
	...
	if( $setdat{'allowdecorate'} != 0 ) {
		...
		$ts =~ s|\[([A-Z0-9]):([a-zA-Z0-9().,]+):|顗$1靍$2魵|g;	# 装飾の開始部分『 [英数:英数等: 』の文字を仮エスケープする1	※装飾記法の開始仕様を変更した際には、ここも併せて書き換える必要がある。
		$ts =~ s|\[([A-Z0-9]):|鑈b靏$1隝e髜|g;					# 装飾の開始部分『 [英数: 』の3文字を仮エスケープする2		※装飾記法の開始仕様を変更した際には、ここも併せて書き換える必要がある。
		$ts =~ s|:|&#58;|g;										# すべてのコロン記号を &#58; に置き換える
		$ts =~ s|鑈b靏([A-Z0-9])隝e髜|[$1:|g;					# 仮エスケープを元に戻す2
		$ts =~ s|顗([A-Z0-9])靍([a-zA-Z0-9().,]+)魵|[$1:$2:|g;	# 仮エスケープを元に戻す1
		...
		my @elmNums = (
			'NONE',
			$setdat{'decoBtnElmnt1'} || 'SPAN',
			$setdat{'decoBtnElmnt2'} || 'SPAN',
			$setdat{'decoBtnElmnt3'} || 'SPAN',
			$setdat{'decoBtnElmnt4'} || 'SPAN',
			$setdat{'decoBtnElmnt5'} || 'SPAN',
			$setdat{'decoBtnElmnt6'} || 'SPAN',
			$setdat{'decoBtnElmnt7'} || 'SPAN',
			$setdat{'decoBtnElmnt8'} || 'SPAN',
			$setdat{'decoBtnElmnt9'} || 'SPAN'
		);

		# 続きを読む機能(隠す機能)の使用可否を事前判定
		my $canUseHideDeco = &canUseReadmoreFunc('H');		# 1=使用可能、0=使用不可
		# 隠す機能用の要素を作成
		my $hideOpen = qq|<$elmH class="decorationH">|;
		my $hideClose = qq|</$elmH>|;
		# 続きを読む用の共通オプション情報をまとめておく
		my %hideopts = (
			'kind'		=> '範囲を隠す' ,			# 1：種別名
			'moreid'	=> '' ,						# 2：隠す領域1つ1つに割り当てるユニークID
			'morenum'	=> '' ,						# 3：隠す番号
			'opentag'	=> '' ,								# 4：隠すボタンを作る要素名
			'hidetag'	=> $elmH ,							# 5：隠す範囲を出力する要素名
			'stylenum'	=> $setdat{'readmorestyle'} ,		# 6：展開する範囲の表示方法(0:inline/1:inline-block/2:block)
			'openlabel'	=> $setdat{'readmorebtnlabel'} ,	# 7：展開ボタンのラベル
			'hidelabel'	=> $setdat{'readmorecloselabel'} ,	# 8：畳むボタンのラベル
			'needhide'	=> $setdat{'readmorecloseuse'}		# 9：畳むボタンの出力が(1:必要/0:不要)
		);
		# 折り畳む用のオプション情報をまとめておく
		my %foldopts = (
			'kind'		=> '折り畳む' ,						# 1：種別名
			'openlabel'	=> $setdat{'readmorebtnlabel'} ,	# 7：展開ボタンのラベル
			'markupset'	=> 1								# 10：マークアップ方法(0:レガシー/1:details+summary)
		);
		...
		for( my $i = 0; $i < $count ; $i++ ) {
			...
			# 汎用装飾があれば(1～9)																											# 正規表現の区切り記号として s||| の代わりに s{}{} を使用。＋『 [英:英数: 』に該当しない場合は2個目のコロンはエスケープされているので。
	# ×	$ts =~ s{\[([1-9]):(([-\w]+)(&#58;|:))?([^\[\]]+)\]}{<$elmNums[$1] class="decoration$1 deco-@{[$3 // '']}">$5</$elmNums[$1]>}g;		# @{[$3 // '']} は $3 が undef の場合に ''（空文字列）を代入する。
			$ts =~ s{\[([1-9]):(([-\w]+)(&#58;|:))?([^\[\]]+)\]}{<$elmNums[$1] class="decoration$1 deco-@{[defined $3 ? $3 : '']}">$5</$elmNums[$1]>}g;		# (4.6.2：上記だと Perl 5.8.9 では // 演算子が未サポートのため文法エラーになるので修正)
			...
			if( $canUseHideDeco == 1 ) {
				...
				if( $setdat{'readmoredynalabel'} == 1 ) {
					...
					$ts =~ s{\[H:(([^\r\n\[\]<>\t\&]{1,100}?)(&#58;|:))?([^\[\]]+)\]}{$hideOpen . &outputHiddenArea(\%hideopts,$4,$2) . $hideClose}eg;		# ラベル文字列の安全化は送り先関数の方でしている # 正規表現の区切り記号として s||| の代わりに s{}{} を使用。
				}
				else {
	 				# ▽[H:隠す本文]記法だけを適用する場合
	 				$ts =~ s|\[H:([^\[\]]+)\]|$hideOpen . &outputHiddenArea(\%hideopts,$1) . $hideClose|eg;
				}
			}
			else {
				# 使えない状況なら全文を表示
			...
			# 折り畳む記述があれば (ラベル文字列は省略可)
			$ts =~ s{\[O:(([^\r\n\[\]<>\t\&]+?)(&#58;|:))?([^\[\]]+)\]}{&outputHiddenArea(\%foldopts,$4,$2)}eg;		# 正規表現の区切り記号として s||| の代わりに s{}{} を使用。
...
sub decoratelist
{
	...
	if( $str =~ m/(.*?)(:|&#58;)(.+)/ ) {
		...
		if( $opt =~ m/^(-?[0-9]+)$/ ) {
			...
			# 丸囲み記号なら、番号付きリストの記号版だと解釈
			$elem = 'ol';
			if   ( $opt eq '①' ) { $startunicode = 9312;  }	# Unicode:2460 &#9312;
			elsif( $opt eq '➊' ) { $startunicode = 10122; }	# Unicode:278A &#10122;
			elsif( $opt eq '⓵' ) { $startunicode = 9461;  }	# Unicode:24F5 &#9461;
			elsif( $opt eq '㊀' ) { $startunicode = 12928; }	# Unicode:3280 &#12928;
			elsif( $opt eq '㋐' ) { $startunicode = 13008; }	# Unicode:32D0 &#13008;
			elsif( $opt eq 'ⓐ' ) { $startunicode = 9424;  }	# Unicode:24D0 &#9424;
			elsif( $opt eq 'Ⓐ' ) { $startunicode = 9398;  }	# Unicode:24B6 &#9398;
		}
		elsif( &fcts::mbLength($opt) <= 2 ) {
			# 上記以外の2文字以下だけだったらリストの先頭記号だと解釈	※一部の絵文字がPerlで2文字だと解釈されるのに簡単に(強引に)対応させるため。
	...
	foreach my $one (@listitems) {
		if( $one ne '' ) {
			...
			elsif( $startunicode > 0 ) {
				# 数値文字参照用の番号が指定されている場合
				my $ncrNum = $startunicode + $count - 1;
				$output .= qq|<li style="list-style-type: '&#$ncrNum';">| . $one . '</li>';
			}
			else {
	...
	if( $targetfile eq '' ) {
		...
		if( $setdat{'imageoutdir'} == 1 ) {
			...
			if(!( -f $servpath )) {
				...
				my $errdetail = '';
				if( !$setdat{'hideenvonerror'} ) { $errdetail = qq|◆なお、サーバ内の $servpath の位置を調べた結果、存在しないと判断しました。◆もしサーバ内のフルパスが正しくないなら、システム設定の「ドキュメントルートの位置」項目を編集して下さい。|; }
...
# ----------------------------------------
# 埋め込み画像HTMLから指定属性値だけを抽出		引数1:HTMLソース(embeddedimage)、引数2:属性名(または空文字)
# ----------------------------------------		返値:指定属性の値(※引数2の指定がない場合はimg要素全体を返す)、該当属性がなければ空文字
sub extractImageAttr
{
	my $html = shift @_ || '';
	my $attName = shift @_ || '';
	...
	# 第2引数がなければ、抽出したimg要素だけを返す
	if( $attName eq '' ) {
		return $imgTag;
	}

	# 属性名を小文字で統一（大文字小文字を無視して比較するため）
	my $att = lc($attName);
	my $value = '';

	# 属性を抽出（ダブルクオートまたはシングルクオートに対応）
	if($imgTag =~ /\b$att\s*=\s*(['"])(.*?)\1/i) {
		$value = $2;
	}

	# 属性値に「embeddedimage」が含まれる場合に限って、それを除外する
	$value =~ s/embeddedimage//g;

	# 前後の空白は除外して返す (※属性が見つからなかった場合は空文字列)
	return &fcts::trim($value);
}
...
sub utilitylinksforonepost
{
	...
	# ========================================================================================== #
	# ★-★-★ このへんのソースを修正することで、余計なリンク群を消そうと考えた方々へ！ ★-★-★ #
	# ========================================================================================== #
	# → そんな手間をかけなくても、管理画面の「設定」から非表示に設定できます！！！ ★-★-★-★  #
	# 　 管理画面の [設定]→「ページの表示」タブ→【ページの表示／投稿単独】枠の中に含まれる　   #
	# 　 『本文の下に、ユーティリティリンク枠を表示する』という項目をOFFにすると全部消せますし、 #
	# 　 必要な項目だけを見せるように取捨選択することもできます。そちらの設定をお試し下さい。　  #
	# ========================================================================================== #
...
# ----------------------
# 動画ファイルの埋め込み		引数1：URL、引数2：オプション文字列
# ----------------------
sub embedvideo
{
	my $url = shift @_ || '';
	...
	# 同一ドメインに限定する場合の制限処理
	if( $setdat{'urlexpandvideolim'} != 0 ) {
		# 同一ドメインに限定する設定なら
		my $baseurl = $cgi->url(-base, 1);
		if( index($url, $baseurl) != 0 ) {
			# 同一ドメインではない場合は
			return qq|<a class="url urlimage" href="$url"><i style="| . &styleforembeddederror() . q|">URLを指定した外部動画の埋め込みは、現在の設定では同一ドメイン下の動画ファイルだけに制限されています。</i></a>|;
		}
	}
	...
	if( $opt eq 'NOCTRL' ) {
		...
		# キャプション
		$caption = &fcts::forsafety($opt);
	}

	# 出力
	$url = &fcts::forsafety($url);
	my $ret = qq|<video$ctrls preload="metadata" class="embeddedvideo"><source src="$url"><p class="fallbackmessage">この動画を再生できない場合は、<a href="$url">ダウンロード</a>できます。</p></video>|;
	if( $caption ne '' ) {
		$ret = qq|<figure class="embeddedvideo"><figcaption class="videotitle">$caption</figcaption>| . $ret . '</figure>';
	}
...
# ===========================
# ★EDIT MODE
# ===========================	引数1:編集領域に入れておく文字列、引数2:画面上部に表示するメッセージ
sub modeEdit
{
	...
	my $cssform = q|
		...
		/* ▼システムボタン(削除ボタン) */
		.deleteButton { background-color:#aaa; color:white; border-color:gray; }
...
sub modeWrite
{
	...
	# コメントがなくて、でも編集No.が指定されている場合は、「再編集画面で本文なしで再投稿された」場合なので削除扱いにする
	if(( $existnewdata == 0 ) && ( $newid ne '' )) {
		push(@ids,$newid);
	}
	...
	if( $existnewdata == 1 ) {
		...
		if( defined($ids[0]) ) {
			...
			# 新規投稿画面へ移動する (第1引数は空にする必要がある)
			&modeEdit('','<p style="font-size:0.9em; color:crimson; margin:0;  padding:0.7em 1em; background:#fee; border-radius:2em; line-height:1.1;">本文も画像もなかったので投稿処理はされていません。投稿するには何かお書き下さい。</p>');
			exit;
		}
	}
	...
	if( $existnewdata == 1 || defined($ids[0]) ) {
		...
		# ▼新着情報(新着リスト・最新投稿日時)の再生成
		&updateLatestInfos();
		push(@accompanies,'新着情報を再生成しました。');
...
sub modeCEmojiup
{
	...
	if( $flagDemo{'RefuseToChangeSettings'} == 1 ) {
		&demomodemsg('カスタム絵文字のアップロード機能は使用できません。');
	}
	...
	# --------
	if( $plv <= 7 ) {
		# カスタム絵文字UPに必要な権限がなければ拒否
		$msg = '<p class="important"><strong>アップロードできません</strong></p><p>カスタム絵文字画像のアップロードに必要な権限がありません。</p>';
		&showadminpage('NO PERMISSION','',$msg,'CEA','');
		exit;
	}
	...
	# ファイルの転送のチェック
	my $error;
	if (!@tryfilenames and $error = $form->cgi_error){
		&errormsg("ファイルが転送できませんでした：$error");
		exit;
	}
	...
	foreach my $filename (@tryfilenames) {
		...
		if( $filename eq '' ) {
		...
		elsif( $filename !~ m/\.(svg|gif|png|jpg|webp)$/i ) {
			# ……………………………
			# ▼許可拡張子でなければ
			# ……………………………
			$msg .= '<li class="important "><b>エラー：</b>カスタム絵文字として使えない形式のファイルが送信されました。送信されたデータは保存されませんでした。</li>';
			push(@ret,'OUTEXT');
			next;
		}
		else {
			...
			if( $ceservdir eq '' ) {
				# 絵文字用ディレクトリが存在しない場合
				&errormsg('カスタム絵文字用のディレクトリが見つかりません。');
				exit;
			}
			...
			if( $filename =~ m/([A-Za-z0-9._-]+)\.(.+)$/ ) {
				# ファイル名が[英数字・ドット・アンダーバー・ハイフン]だけで構成されていれば
				$imagename = $1 . '.' . $fileext;										# ファイル名は一旦そのまま保持(拡張子は小文字に変換されたもの)
				$newfilepath = &fcts::makelastcharslash($ceservdir) . $imagename;		# ファイルパスを作って保持

				# ※同名のファイルが存在した場合は上書きするので、既存かどうかはチェックしない
			}
			else {
				$msg .= '<li class="important "><b>エラー：</b>カスタム絵文字用の画像ファイルに使えるファイル名は、英数字・半角記号3種(ドット・アンダーバー・ハイフン)だけです。条件に該当しない名称のファイルは保存されませんでした。</li>';
				push(@ret,'OUTEXT');
				next;
			}
			...
			# ‥‥‥‥‥‥‥‥‥‥‥‥‥‥
			unless( open (OUTFILE,'>',"$newfilepath") ){
				&errormsg("サーバに画像ファイルを保存できませんでした。CGIの設定を見直すか、出力先ディレクトリの書き込み権限や空き容量を確認して下さい。(※保存先ディレクトリを作成していない状態で画像をアップロードしようとした際にもこのエラーが表示されます。)<br>→エラー詳細: [ $! ]\n");
				exit;
			}
			...
			# サイズオーバーならファイルを消してエラーを表示する
			if(( $setdat{'cemojimaxlimits'} == 1 ) && ( $setdat{'cemojimaxbytes'} < $fsize )) {
				# UP上限が設定されていて、かつ、オーバーしているなら消す
				unlink($newfilepath);
				$msg .= '<li class="important">【画像アップロードエラー】送信されたファイルは、許容サイズを超えています。アップロードは拒否されました。現在の設定では、1ファイルあたりの最大サイズは <b>' . &fcts::byteswithunit($setdat{'cemojimaxbytes'}) . '</b> に制限されています。</li>';
				push(@ret,'OVER');
			}
			else {
				# サイズ内なら完了報告
				$loopcount++;
				$msg .= '<li><span class="sizeinfo"><img src="' . $newfilepath . '" alt="Custom Emoji" class="cemojiPreview">：約 ' . &fcts::byteswithunit($fsize) . 'のカスタム絵文字</a>をアップロードしました。</span> <!-- ' . $imagename . ' --></li>';
				push(@ret,$imagename);
			}
		}
	}
	...
	if( $loopcount == 0 ) {
		...
		if( $msg ne '' ) {
			# 何らかのエラーが既に報告されている場合は整形する
			$msg = qq|<p class="msgtitle">【カスタム絵文字アップロードエラー】</p><ul>$msg</ul><p>何もアップロードされませんでした。</p>\n|;
		}
		else {
			$msg .= '<p>アップロードするファイルが指定されていません。</p>';
		}
		...
		$msg = '<p class="msgtitle">【カスタム絵文字用画像のアップロード完了】</p><ol>' . $msg . '</ol><p class="msgsum">計' . $loopcount . '個のカスタム絵文字をアップロードしました。</p>';
	}

	my $css = q|<style>
		.cemojiPreview { max-height:2em; width:auto; vertical-align:middle; }
		</style>
	|;

	# 結果報告画面を表示する
	&showadminpage('CUSTOM EMOJI UPLOADED','',$msg,'CEA',$css);
}
...
sub modeAdmin
{
	...
	elsif(	$awn eq 'unusedimages'	) { &accesslevelcheck(1,$plv); &adminUnusedImages(); }
	elsif(	$awn eq 'unusedimgres'	) { &accesslevelcheck(1,$plv); &adminUnusedImgRes(); }
	...
	elsif(	$awn eq 'namacatedit'	) { &accesslevelcheck(7,$plv); &adminNamaCatEditor(); }
	elsif(	$awn eq 'trybulkeditcat') { &accesslevelcheck(7,$plv); &adminTryBulkEditCat(); }
	...
	elsif(	$awn eq 'filetrydels'	) { &accesslevelcheck(3,$plv); &adminFileTrydels(); }
	elsif(	$awn eq 'deletefiles'	) { &accesslevelcheck(3,$plv); &adminDeleteFiles(); }
	elsif(	$awn =~ m/^imgflagedit-(\w*)$/ ) { &accesslevelcheck(7,$plv); &adminImgBlukFlagEdit($1); }
	...
	elsif(	$awn eq 'tegaclap'		) { &accesslevelcheck(9,$plv); &adminExternalTool($awn); }
...
sub adminMenu
{
	...
	$smenu[1][2] = &genSysMenuItem('ユーザIDを管理',		'Manage User IDs',	'ユーザIDを新規作成/編集/削除します。',			&makeQueryString('mode=admin','work=userlist'),		&requiredlevelcheck(9,$permitlevel));
	$smenu[1][3] = &genSysMenuItem('自分のIDを設定',		'Edit Own User ID',	'自分のIDの設定情報を編集します。',				&makeQueryString('mode=admin','work=changepass',"userid=$permittedid"),	&requiredlevelcheck(5,$permitlevel));
	...
	$smenu[1][6] = &genSysMenuItem('スキンの切替',			'Change Skin',		'スキンを一覧表示し、切り替えます。',			&makeQueryString('mode=admin','work=skinlist'),		&requiredlevelcheck(9,$permitlevel));
	$smenu[2][0] = '▼バックアップ：';
	...
	$smenu[3][2] = &genSysMenuItem('全員を強制ログアウト',	'Force Logout for All',	'全てのログインユーザのアクセスを終了させます。',	&makeQueryString('mode=admin','work=panic'),	&requiredlevelcheck(9,$permitlevel), '本当に全員を強制ログアウトさせてよろしいですか？');

	# 条件に応じて追加する項目を準備
	if( -f $subfile{'clapPHP'} ) {
		# てがくらっぷがあれば
		$smenu[1][7] = &genSysMenuItem('てがくらっぷ',		'Tegaclap',			'連携ツール「てがくらっぷ」へ移動します。',		&makeQueryString('mode=admin','work=tegaclap'),		&requiredlevelcheck(9,$permitlevel));
	}
	...
	<div class="systemmenubox">
		...
		<div class="systemmenucolumn">
			<div class="systemhelpbox"><span class="helpboxtitle">このCGIについて$rentalsign</span>
				...
				<a href="$aif{'puburl'}#feedback">機能要望/質問の送り方</a><br>
...
sub adminCategories {
	...
	# 生編集リンクを表示
	# ………………………
	my $nequery = &makeQueryString('mode=admin','work=namacatedit');
	$msg .= qq|\n<div class="namacateditLink"><a href="$nequery" class="btnlink">複数カテゴリの一括生編集</a></div>\n|;
	...
	my $css = q|<style>
		...
		.namacateditLink { margin:0.5em 0 0 0; text-align:right; }
...
sub adminCatEditor
{
	...
	my %catSets = (
		...
		cprnt => '',
		chits => '',
		coder => int(&fcts::getCategoryLastOrder() / 10) * 10 + 10,
		cicon => '',
	...
	if( $catid ne '' ) {
		# カテゴリIDの指定があれば情報を得る	(※番号との互換性のために、表示順序とは異なる順で記述しているので注意)
		$catSets{'cname'} = &fcts::forsafety( &fcts::getCategoryDetail($catid, 1));	# 編集対象カテゴリの表示名
		$catSets{'cdscr'} = &fcts::forsafety( &fcts::getCategoryDetail($catid, 2));	# 編集対象カテゴリの概要文
		$catSets{'cprnt'} = &fcts::forsafety( &fcts::getCategoryDetail($catid, 3));	# 編集対象カテゴリの親カテゴリID
		$catSets{'chits'} = &fcts::forsafety( &fcts::getCategoryDetail($catid, 4));	# 編集対象カテゴリの該当個数
		$catSets{'coder'} = &fcts::forsafety( &fcts::getCategoryDetail($catid, 5));	# 編集対象カテゴリの掲載順序
		$catSets{'cicon'} = &fcts::forsafety( &fcts::getCategoryDetail($catid, 6));	# 編集対象カテゴリのアイコン
		if( $catSets{'cname'} eq '' ) {
			# 名称が空文字列の場合はIDが存在しない
		...
		# オプション群の情報を得る
		$catSets{'useppp'}	 = &fcts::forsafety( &fcts::getCategoryDetail($catid, 'cat:useppp'));
		$catSets{'ppp'}		 = &fcts::forsafety( &fcts::getCategoryDetail($catid, 'cat:ppp'));
		$catSets{'pppforce'} = &fcts::forsafety( &fcts::getCategoryDetail($catid, 'cat:pppforce'));
		$catSets{'useskin'}	 = &fcts::forsafety( &fcts::getCategoryDetail($catid, 'cat:useskin'));
		$catSets{'skin'}	 = &fcts::forsafety( &fcts::getCategoryDetail($catid, 'cat:skin'));
		$catSets{'useleads'} = &fcts::forsafety( &fcts::getCategoryDetail($catid, 'cat:useleads'));
		$catSets{'leads'}	 = &fcts::forsafety( &fcts::getCategoryDetail($catid, 'cat:leads'));
	}
	...
	if( $catSets{'useppp'} )	{ $cBox{'useppp'} = ' checked'; }
	if( $catSets{'pppforce'} )	{ $cBox{'pppforce'} = ' checked'; }
	if( $catSets{'useskin'} )	{ $cBox{'useskin'} = ' checked'; }
	if( $catSets{'useleads'} )	{ $cBox{'useleads'} = ' checked'; }
	...
	my $catselectlist = &fcts::getCategorySelectList('tryparentid',1,$catSets{'cprnt'},$catid);	# カテゴリ一覧をHTMLのselect要素で得る（引数：1=name属性値、2=先頭空白、3=デフォルト選択ID、4=自分自身のIDは除外対象）
	if( $catselectlist ne '' ) {
		# カテゴリが1つ以上ある場合の注釈
		...
		<form action="$cgipath" method="post"><input type="hidden" name="mode" value="admin"><input type="hidden" value="trychangecat" name="work"><input type="hidden" name="idwork" value="$work">
			<fieldset>
				...
				<ul class="inputs">
					...
					<li><label><span class="itemhead">カテゴリ名：</span> <input type="text" value="$catSets{'cname'}" name="tryname"></label> <span class="notice">※いつでも変更できます。</span></li>
					<li><label><span class="itemhead">概要文：    </span> <input type="text" value="$catSets{'cdscr'}" name="trydesc"></label> <span class="notice">※いつでも変更できます。(省略可) $allowhtmlsign</span></li>
					<li><label><span class="itemhead">アイコン：  </span> <input type="text" value="$catSets{'cicon'}" name="tryicon" placeholder="https://"></label> <span class="notice">※いつでも変更できます。(省略可)</span></li>
					<li><label><span class="itemhead">親カテゴリ：</span> </label>$catselectlist</li>
					<li><label><span class="itemhead">掲載順序：  </span> <input type="text" value="$catSets{'coder'}" name="tryorder" class="num"></label> <span class="notice">※小さい順に並びます。連番でなくて構いません。</span></li>
				</ul>
			</fieldset>
			<fieldset>
				<legend>カテゴリオプション</legend>
				<div id="highsetbtn"><input type="button" value="オプション設定を表示する" onclick="document.getElementById('highsetbtn').style.display='none'; document.getElementById('highsets').style.display='block'; document.getElementById('highsetsGuide').style.display='inline-block';"></div>
				<div id="highsets">
					<p class="opthead">▼このカテゴリ単独の限定表示時にだけ適用する設定：</p>
					<ul class="checkinputs">
						<li><input type="checkbox" name="useppp" id="useppp" value="1"$cBox{'useppp'}><label for="useppp">1ページあたりの表示投稿数を指定する</label>
							<ul>
								<li><label>表示件数： <input type="text" value="$catSets{'ppp'}" name="ppp" class="num">件</label></li>
								<li class="addeditem"><label><input type="checkbox" name="pppforce" value="1"$cBox{'pppforce'}>スキン側で表示数が指定されていてもこの値を強制する</label></li>
							</ul>
						</li>
						<li><input type="checkbox" name="useskin" id="useskin" value="1"$cBox{'useskin'}><label for="useskin">表示に使うスキンを指定する</label>
							<ul>
								<li><label>スキン格納ディレクトリ名： <input type="text" value="$catSets{'skin'}" name="skin"></label> <span class="notice">(※1)</span></li>
							</ul>
						</li>
						<li><input type="checkbox" name="useleads" id="useleads" value="1"$cBox{'useleads'}><label for="useleads">指定の投稿を先頭に固定する</label>
							<ul>
								<li><label>固定表示する投稿番号： <input type="text" value="$catSets{'leads'}" name="leads"></label> <span class="notice">(※2)</span></li>
							</ul>
						</li>
					</ul>
				</div><!-- #highsets -->
			...
			<p>
				<input type="submit" value="カテゴリ情報を$submitLabelする" class="sendui">
			</p>
		</form>
		<p class="noticebox">
			※表示名の重複チェックはしませんので、複数のIDで同じカテゴリ名を使うこともできます。<br>
			※カテゴリ名が画面に表示されるかどうかは、表示に使うスキン次第です。<br>
			※親カテゴリを指定する際に、階層構造が無限ループになるような指定はしないで下さい（それらのカテゴリ設定が一括して消滅してしまいます）。<br>
			※掲載順序は「<strong class="important">同一階層の中での</strong>最も小さい値」から順に並びます。小数や負数も使えます。<br>
			<span id="highsetsGuide">
				<br>
				※1：[設定]→[システム設定]→【スキンの適用制限】で不許可な位置にあるスキンは(指定しても)使えません。<br>
				※2：投稿番号を半角カンマ記号「,」で区切れば複数指定できます。上限なし。先頭に固定されるのは、このカテゴリが<strong class="important">単独で</strong>限定表示されている場合、かつ、他の条件では絞り込まれていない場合だけです。<br>
			</span>
		</p>
		<script>document.getElementById('highsets').style.display='none'; document.getElementById('highsetsGuide').style.display='none';</script>
	|;
	...
	my $css = '<style>
		.opthead { margin:0.5rem 0; font-weight:bold; }
		.addeditem { list-style-type:none; }
		.itemhead { min-width: 6.15em; display: inline-block; }
		.inputs { margin: 0.5em 0; padding: 0 0 0 20px; }
		.checkinputs { list-style-type:none; margin: 0.5em 0; padding: 0; }
		.checkinputs > li:not(:first-child) { margin-top:0.5rem; }
		.checkinputs ul { list-style-type:disc; }
		.deletions { color: gray; font-weight:bold; border: 2px dotted gray; padding: 5px; margin: 1em 0 1em; display: inline-block; }
		...
		.num { width: 3em; }
		/* ▼チェックボックス連動サブ項目の表示 */
		input:not(:checked) ~ ul { opacity: 0.42; }
		#highsetbtn { text-align:center; }
	</style>';
...
sub adminTrychangecategory {
	...
	# カテゴリ情報の作成処理(TRY)
	my %try = (
		id			=> &fcts::deleteseparators( $cgi->param('tryid') ) || '',
		name		=> &fcts::deleteseparators( &fcts::safetycutter( scalar $cgi->param('tryname') )) || '',
		desc		=> &fcts::deleteseparators( $cgi->param('trydesc') )  || '',
		icon		=> &fcts::deleteseparators( $cgi->param('tryicon') )  || '',
		parentid	=> &fcts::deleteseparators( $cgi->param('tryparentid') )  || '',
		order		=> &fcts::deleteseparators( $cgi->param('tryorder') )  || '',

		useppp		=> &fcts::deleteseparators( $cgi->param('useppp') )  || 0,
		ppp			=> &fcts::deleteseparators( $cgi->param('ppp') )  || '',
		pppforce	=> &fcts::deleteseparators( $cgi->param('pppforce') )  || 0,
		useskin		=> &fcts::deleteseparators( $cgi->param('useskin') )  || 0,
		skin		=> &fcts::deleteseparators( $cgi->param('skin') )  || '',
		useleads	=> &fcts::deleteseparators( $cgi->param('useleads') )  || 0,
		leads		=> &fcts::deleteseparators( $cgi->param('leads') )  || ''
	);
	...
	if( $try{'order'} ne '' && $try{'order'} =~ /[^-\d\.０１２３４５６７８９]/ ) {
		$errMsg .= '<li>掲載順序には数字だけが使えます。</li>';
	}
	else {
		$try{'order'} = &fcts::nozenkaku($try{'order'});	# 全角数字を(あれば)半角にする（数字以外は消える）
	}

	# PPPに数字以外があったら
	if( $try{'ppp'} ne '' && $try{'ppp'} =~ /[^\d０１２３４５６７８９]/ ) {
		$errMsg .= '<li>表示件数には数字だけが使えます。</li>';
	}
	else {
		$try{'ppp'} = &fcts::nozenkaku($try{'ppp'}) || '';	# 全角数字を(あれば)半角にする（数字以外は消える）※0が返ってきた場合は空文字にする
	}

	# 先頭固定群に数字とカンマ以外があったら
	if( $try{'leads'} ne '' && $try{'leads'} !~ /^[\d,\s]+$/ ) {
		$errMsg .= '<li>先頭固定投稿の指定には数字と「区切るための半角カンマ記号」だけが使えます。</li>';	# 空白の入力も許容(後で強制削除するから)
	}
	else {
		$try{'leads'} =~ s/\s//g;			# 空白文字を削除
		$try{'leads'} =~ s/^,+//;		# 先頭のカンマを削除
		$try{'leads'} =~ s/,+$//;		# 末尾のカンマを削除
		$try{'leads'} =~ s/,+/,/g;		# 連続するカンマを1つに置換
	}
	...
	if( $flagDemo{'RefuseToChangeSettings'} != 1 ) {
		...
		my %catOpts = (
			...
			'cat:name'		=> $try{'name'} ,
			'cat:dscrpt'	=> $try{'desc'} ,
			'cat:parent'	=> $try{'parentid'} ,
			'cat:posts'		=> '＾維' ,
			'cat:order'		=> $try{'order'} ,
			'cat:iconsrc'	=> $try{'icon'},

			'cat:useppp'	=> $try{'useppp'},
			'cat:ppp'		=> $try{'ppp'},
			'cat:pppforce'	=> $try{'pppforce'},
			'cat:useskin'	=> $try{'useskin'},
			'cat:skin'		=> $try{'skin'},
			'cat:useleads'	=> $try{'useleads'},
			'cat:leads'		=> $try{'leads'}
		);
		my $newidline = &fcts::makeLineForCategoryDat( $try{'id'}, \%catOpts );		# カテゴリID：tryid、それ以外の情報を含めた連想配列のリファレンス	(各値に「＾維」を指定すると、既存IDなら前の値を維持する)
		...
		# 報告用文字列
		$result .= '<li>カテゴリID「 ' . &fcts::forsafety($try{'id'}) . ' 」の情報を' . $wlabel . 'しました。</li>';
...
sub categorycounter
{
	...
	# カテゴリが1つでもあれば
	if( $done >= 1 ) {
		# カテゴリ該当件数の更新関数にカウント値データ(連想配列のリファレンス)を送る (返値は結果表示用文字列)
		$msg .= &fcts::updateCatApplyPostsNum( \%counter );

		# カテゴリ情報を保存用形式に文字列化
		my $catsDataForSave = &fcts::getCompFumyxmlForCats();

		# 更新したカテゴリ情報をiniに保存する
		my @trywrites;
		push(@trywrites,"categorylist=$catsDataForSave");
		&savesettings( @trywrites );
	}
	else {
...
sub adminNamaCatEditor
{
	...
	# カテゴリ生データを取得する
	my $catsource = $setdat{'categorylist'};
	if( $catsource eq '' ) {
		# カテゴリデータが存在しない場合は、デフォルト値を使う
		$catsource = $firstdat{'categories'};
	}

	# 1件ずつ行に分解する
	my @rawCats = $catsource =~ /<datum>(.*?)<\/datum>/g;
	$catsource = join("\n",@rawCats);

	my $cgipath = &getCgiPath();
	my $catmanagepage = &makeQueryString('mode=admin','work=categories');

	# カテゴリページ上部
	my $msg = '<p>カテゴリの生データを一括編集します。<br>複数のカテゴリの情報を一括で変更したり、新規に追加したり、削除したりできます。ただし、<strong class="important">記法に沿って記述しないと正しく更新できない</strong>のでご注意下さい。（この編集方法に自信がない場合は、<a href="' .
				$catmanagepage . '">カテゴリ管理画面</a>から1件ずつ操作することをお勧め致します。）</p>';

	$msg .= '<h2>▼カテゴリ生データ</h2>';

	$msg .= qq|
	<div class="bulkcatEditCover">
		<script>
			function blankCheck() {
				const textarea = document.getElementById('BulkCats');
				if(textarea.value.trim() === '') {
					const isConfirmed = confirm('カテゴリを全消去しようとしています。本当に既存のカテゴリをすべて削除して良いですか？');
					if (!isConfirmed) { return false; }
				}
			}
		</script>
		<form action="$cgipath" method="post" onsubmit="return blankCheck();"><input type="hidden" value="admin" name="mode"><input type="hidden" value="trybulkeditcat" name="work">
			<div class="bulkcatEditBox"><textarea name="bulkcat" id="BulkCats">$catsource</textarea></div>
			<p><input type="submit" value="上記の生データを保存する"></p>
		</form>
	</div>\n|;

	$msg .= q|
		<div class="editGuideBox">
			<h3>編集方法</h3><ul class="editGuide">
			<li>1行1件で記述します。</li>
			<li>データ項目は <code>&lt;cat:xxx&gt;～&lt;/cat:xxx&gt;</code> の独自タグで区切られています。順不同です。</li>
			<li>カテゴリを新規に増やしたい場合は、行を追加して下さい。</li>
			<li>カテゴリを削除したい場合は、当該カテゴリの行をまるごと削除して下さい。</li>
			<li>行の並びを入れ替えてもカテゴリの表示順序は<strong class="important">変わりません</strong>。（掲載順序の数字を書き換える必要があります。）</li>
			<li>ここでカテゴリID<code>&lt;cat:id&gt;～&lt;/cat:id&gt;</code>を変更しても、既存投稿の所属カテゴリIDは変わりません。（その結果、それらの投稿は当該カテゴリから外れてしまいますのでご注意下さい。一度作ったカテゴリIDは変更しない前提の仕様です。）</li>
			<li>該当件数<code>&lt;cat:order&gt;～&lt;/cat:order&gt;</code>にはどのような値を入れても再集計された際に正しい値に自動修正されます。</li>
			<li>親カテゴリIDがない場合は<code>&lt;cat:parent&gt;-&lt;/cat:parent&gt;</code>のように「-」記号だけを記述して下さい。存在しないカテゴリIDを書いてしまわないようご注意下さい（存在しないIDを親にすると、そのカテゴリは消えます）。</li>
			<li>カテゴリの親子関係で無限ループを作らないようご注意下さい。（その場合は、無警告で当該カテゴリの情報が消えます。）</li>
			</ul>
		</div>
	|;

	$msg .= q|<p class="noticebox">
		※カテゴリを削除しても、当該カテゴリに属している投稿データは消えません。ただし、当該カテゴリの名前(カテゴリ名)は表示されなくなります。<br>
		※削除したカテゴリを復活させたい場合は、同じID名でカテゴリを新規作成した後、<a href="| . $catmanagepage . q|">カテゴリ管理画面</a>から再集計すれば良いです。<br>
		※この画面でデータを編集する場合は、<strong class="important">編集前の状態のバックアップコピーをテキストエディタで取っておく</strong>ことをお勧め致します。
		</p>|;

	# CSS
	my $css = q|<style>
		.bulkcatEditCover { }
		.bulkcatEditBox textarea {
			font-size: 1rem;
			max-width: 100%;
			width: 100%;
			height: min(18rem, 50vh);
			box-sizing: border-box;
			overflow-x: auto; overflow-y: auto; white-space: pre; word-wrap: normal;
			line-height: 1.5;
		}
		.editGuideBox { font-size: 0.9em; background-color:#eee; border: 2px solid #ccc; border-radius: 10px; padding:0.5rem; }
		.editGuideBox h3 { margin: 0.25rem 0; border-bottom: 3px double #aaa; line-height: 1; }
		.editGuideBox code { background-color:#ffc; border:1px solid #ccc; display:inline-block; padding:1px 3px; }
		.editGuide { margin:0; padding:0 1rem; }
		</style>
	|;

	&showadminpage('CATEGORY：BULK EDIT','',$msg,'CA',$css);
}

# -----------------------------
# ADMIN：カテゴリ一括修正の試行
# -----------------------------
sub adminTryBulkEditCat
{
	# デモモードなら実行を拒否する
	if( $flagDemo{'RefuseToChangeSettings'} == 1 ) {
		&demomodemsg('カテゴリ情報の変更はできませんでした。');
	...
	# 不正送信の確認
	&fcts::postsecuritycheck('work=trybulkeditcat');
	...
	# ▼送信された情報の取得
	# ……………………………
	my $trybulkcat	= $cgi->param('bulkcat') || '';
	my @newcatdata = ();

	# 行単位で配列に格納
	my @lines = split(/\r|\n/, $trybulkcat);	# CR,CR+LF,LFの各パターンに対応するため、CRとLF両方で分割する(一時的に空行ができる)

	# 共通エラーメッセージを用意しておく
	my $catmanagepage = &makeQueryString('mode=admin','work=categories');
	my $commonErrMsg = '<p>保存はできませんでした。<br>前の画面に戻って適切に訂正して下さい。</p><p class="noticebox">※修正方法が分からない場合は、<a href="' . $catmanagepage . '">カテゴリ管理画面</a>から1件ずつ操作することをお勧め致します。</p>';
	my $afterErrPoint = 'が、カテゴリ生データの記法に沿っていません。';

	# 正しい記法に沿っていそうかどうかをざっくり確認する
	my $count = 0;
	my @cids = ();	# 重複チェック用の記録用
	my @pids = ();	# 親確認用の記録用
	foreach my $one ( @lines ) {
		...
		# 1項目ずつ分解(タグをパースしてハッシュに格納)して、指定されている文字列の種類を確認
		my %items;
		while( $one =~ /<([\w_:-]+)>(.*?)<\/\1>/g ) {
			$items{$1} = $2;
		}
		...
		my $epoint = '';	# エラー報告格納用
		if( $items{'cat:id'} !~ m/^[a-zA-Z0-9]+$/ ) { $epoint .= ' 第1項目(カテゴリID) '; }			# カテゴリID
		if( $items{'cat:parent'} !~ m/^[a-zA-Z0-9-]+$/ ) { $epoint .= ' 第4項目(親カテゴリID) '; }	# 親カテゴリID or ハイフン
		if( $items{'cat:posts'} !~ m/^[0-9.]+$|^未集計$/ ) { $epoint .= ' 第5項目(該当件数) '; }	# 該当件数
		if( $items{'cat:order'} !~ m/^-?[0-9.]+$/ ) { $epoint .= ' 第6項目(掲載順序) '; }			# 掲載順序

		# カテゴリIDの重複を確認する
		foreach my $ocat ( @cids ) {
			if( lc($ocat) eq lc($items{'cat:id'}) ) {
				# 一致していたら同じカテゴリIDが2度使われているのでエラー
				$epoint = 'カテゴリID「' . &fcts::forsafety($items{'cat:id'}) . '」が他の(既存の)カテゴリIDと重複しています。';
				$afterErrPoint = '重複しないカテゴリIDを指定して下さい。';
				last;
			}
		}

		if( $epoint eq '' ) {
			# 沿っているなら追加 (エラー報告がないなら条件に沿っている)
			push( @newcatdata, $one );	# 追加
			$count++;					# カウントする
			push( @cids, $items{'cat:id'} );		# カテゴリIDを個別に記録しておく
			push( @pids, $items{'cat:parent'} );	# 親カテゴリIDも個別に記録しておく
		}
		else {
			# 沿っていないならエラーで止める
			my $emsg = '<h2>記法エラー</h2><p style="color:crimson;">➡ <b>' . ( $count + 1 ) . '件目のデータの' . $epoint . '</b>' . $afterErrPoint . '</p>' . $commonErrMsg;
			&showadminpage('SYNTAX ERROR','',$emsg,'BGA','');
			exit;
		}
	}

	# 存在しない「親カテゴリID」が指定されていないかどうかを確認する
	push( @cids , '-' );	# 親カテゴリIDリストに「-」を追加しておく
	my %idsC = map { $_ => 1 } @cids;	# @cids の要素をハッシュのキーとして格納
	# @pids の要素を順番に確認
	foreach my $element (@pids) {
		# 要素が %idsC に存在しない場合はそこでエラー終了
		unless (exists $idsC{$element}) {
			my $emsg = '<h2>親カテゴリIDの記述ミス</h2><p style="color:crimson;">➡ <b>存在しない親カテゴリID「' . $element . '」が指定されています。</b><br>階層構造を作りたい場合、親カテゴリIDには、<b>既存の</b>カテゴリIDを指定して下さい。<br>階層構造にしない（＝第1階層に置きたい）場合は、親カテゴリID欄には半角「-」記号1つだけを書いて下さい。</p>' . $commonErrMsg;
			&showadminpage('WRONG PARENT CATEGORY ID','',$emsg,'BGA','');
			exit;
		}
	}
	...
	# 更新データを作成
	my @catDatumList = ();
	foreach my $onecat (@newcatdata) {
		push(@catDatumList, '<datum>' . $onecat .'</datum>' );
	}
	my $categorylistStr = &fcts::makeCompleteFumyxml(@catDatumList);

	# 保存処理へ渡す
	my @trywrites;
	push(@trywrites,'categorylist='. $categorylistStr);
	&savesettings( @trywrites );

	# ※製作メモ
	# カテゴリの再集計は、『てがろぐ起動時に $setdat{'categorylist'} の値を専用グローバル変数に展開したときの情報』を使うことが前提なので、ここで実行するのは面倒だから、しない。
	# (単に $setdat{'categorylist'} の値を更新するだけでは、再集計できないので。)

	# 結果報告画面を作る
	my $msg = '<h2>カテゴリ一括編集完了</h2><p>カテゴリ生データを保存しました。カテゴリを新設した場合や、該当数データを弄った場合は、再集計して下さい。</p>';
	my $rcquery = &makeQueryString('mode=admin','work=recount','target=cat');	# 再カウントリンク
	my $nequery = &makeQueryString('mode=admin','work=namacatedit');			# 生データ再編集リンク
	$msg .= qq|<p><a href="$rcquery" class="btnlink">カテゴリ該当数を再集計する</a> <a href="$nequery" class="btnlink">生データを再編集する</a></p>|;
	my $css = '';

	&showadminpage('CATEGORY UPDATED','',$msg,'CGA',$css);
}
...
sub adminCemoji
{
	...
	# ▼デフォルト上部案内
	# …………………………
	my $topguide =  '<div id="defaultTopMsg">';
	$topguide .= "<p>$usernameさん、あなたのIDでは『カスタム絵文字リストの閲覧・選択』";
	if( $plv < 7	) { $topguide .= 'だけ'; }
	if( $plv >= 7 	) { $topguide .= 'と『新規アップロード・削除』'; }
	$topguide .= "ができます。(ユーザID「$permittedid」でログイン中)</p>";
	$topguide .= "</div>\n";

	# ………………………
	# 初期選択タブの設定
	my @checkedtab = ('','','','','','');
	if(( $cp{'page'} > 1 ) && ( $cp{'page'} <= 2 )) { $checkedtab[$cp{'page'}] = 'checked'; }
	...
	foreach my $onefile (@ifiles) {
		...
		# HTMLを生成(テーブル)
		my $usize = &fcts::byteswithunit($os);
		$cetable .= qq|\n<tr><td class="cecheck"><input type="checkbox" name="filename" value="$of"></td><td class="cepreview"><img src="$cefpath" alt="$of"></td><td class="insstr">[:$fname:]</td><td class="filename">$of</td><td class="filetime">$ot</td><td class="filesize$sizeAlert">$usize</td><td class="usesearch"><a href="?q=[:$fname:]\|[:$of:]" class="btnlink uselink">投稿を探す</a><a href="?mode=admin&amp;work=manage&amp;q=[:$fname:]\|[:$of:]" class="btnlink uselink">一覧で探す</a></td></tr>|;

		# 引用符に囲まれたファイル名の配列も作る(※後からJavaScriptでの重複確認用配列を作る用)
		push(@cearray,'"'.$of.'"');
	}
	...
	if( $imgtempid == 0 ) {
		# 1つもない場合
		$totalinfo .= q|<p style="color:gray;">（※1つもアップロードされていません。）</p>|;
	}
	else {
		# 1つ以上ある場合
	...
	# JavaScriptでの重複確認用配列を作る
	my $cejsarray = '[' . join(',',@cearray) . ']';

	# --------------------
	# 一覧タブページの生成
	# --------------------
	my $tab1html = qq|<p>アップロードされているカスタム絵文字の一覧：</p>\n<div id="CemojiList">$celist\n</div>| . $totalinfo;

	# JavaScriptを追加：Cemojiコードをクリップボードにコピーするために、一時的なTextareaを生成して範囲選択し、クリップボードに格納する。その際、対象コードの表示色を500ミリ秒だけ変更する。
	$tab1html .= &fcts::tooneline( q|
<script>
function copyCode( fname ) {
	...
	if( $plv >= 7 ) {
		# 画像をUPする権限があれば、画像アップロードフォームを生成
		my $upform = '';
		my $cgipath = &getCgiPath();
		$upform .= '<div class="imageuploadarea">';
		if(( $setdat{'cemojimaxlimits'} == 0 ) || ( $setdat{'cemojistoragelimit'} < $totalsize )) {
			# 上限が設定されていないか、または上限に到達していなければUPフォームを表示
			$upform .= qq|
				<form action="$cgipath" method="post" class="postform" enctype="multipart/form-data" id="CEmojiUp">
					<fieldset>
						<legend>新規アップロード</legend>
						ファイル：<input type="file" name="upload_file" class="imageinput" multiple>
						<input type="hidden" value="cemojiup" name="mode">
						<input type="submit" value="アップロード" class="largebtn">
						<p class="uploadformguide">
							※複数ファイルの同時選択も可能です。<a href="#" onclick="alert('Windowsの場合は、[Ctrl]キーを押しっぱなしにした状態で複数のファイルをクリックすれば、クリックしたすべてのファイルを同時に選択できます。Macの場合は[Command]キーを使います。iOSやAndroidでは、何もせずにそのままタップするだけで複数選択できます。'); return false;">複数選択するには？</a><br>
							※ファイル名には、英数字と半角記号3種「.」「-」「_」のみが使えます。(→詳細<a href="$aif{'puburl'}usage/#howtouse-customemoji-file">ヘルプ</a>)<br>
							※同名ファイルが既に存在する場合は上書きされます。<br>
						</p>
					</fieldset>
				</form>
			|;
			...
			$upform .= '<p class="important"><strong>保存可能容量が最大値に達しているため、これ以上のアップロードはできません。</strong><br>さらにアップロードするには、上限設定を変更するか、または既存のカスタム絵文字画像ファイルを削除する必要があります。</p>';
		}

		# 上限の案内
		$upform .= '<div class="maxguidebox">';
		my $maxBytes = '無制限です';
		my $totalLimit = '無制限です';
		my $addAlert = '';
		if( $setdat{'cemojimaxlimits'} == 1 ) {
			$maxBytes = '最大 <b>' . &fcts::byteswithunit($setdat{'cemojimaxbytes'}) . ' </b>です';
			$totalLimit = '<b>' . &fcts::byteswithunit($setdat{'cemojistoragelimit'}) . ' </b>です<small>（※制限を超過しても新規UPができなくなるだけで自動削除はされません）</small>';
		}
		else {
			$addAlert = '<small>（参考警告サイズ：' . &fcts::byteswithunit($setdat{'cemojimaxbytes'}) . '）</small>';
		}
		$upform .= '<p>●現在の設定では、1ファイルあたりのサイズは、' . $maxBytes . '。' . $addAlert . '<br>●アップロード可能な総容量は、' . $totalLimit . '。</p>';

		$upform .= '</div><!-- /.maxguidebox -->';

		$upform .= '</div><!-- /.imageuploadarea -->';

		$tab2html = $upform;
		$cetable = '<table class="ceTable"><thead><tr><th>＼</th><th>絵文字</th><th>挿入記法</th><th>ファイル名</th><th>UP日時</th><th>サイズ</th><th>使用場所</th></tr></thead><tbody>' . $cetable . '</tbody></table>';
		$tab2html .= qq|<form action="$cgipath" method="post">|;
		$tab2html .= $cetable . "\n";
		$tab2html .= q|<p>
			<input type="submit" value="選択したカスタム絵文字を削除する">
			<input type="hidden" value="admin" name="mode">
			<input type="hidden" value="filetrydels" name="work">
			<input type="hidden" value="cemoji" name="type">
		</p>|;
		$tab2html .= q|</form>|. $totalinfo;

		# 既存ファイルとの重複確認スクリプトを出力しておく
		$tab2html .= &fcts::tooneline( qq|
<script>
const existfiles = $cejsarray;	/* 既存のファイルリスト */
const fileInput = document.querySelector('input[type="file"][name="upload_file"]');	/* UPフォームを取得 */
const uploadForm = document.querySelector('#CEmojiUp');	/* フォームを取得 */

/* フォーム送信時のイベントリスナーを作る */
uploadForm.addEventListener('submit', function(event) {
	const selectedFiles = Array.from(fileInput.files);	/* 選択されたファイルのリストを配列に変換 */
	const duplicateFiles = [];

	/* 選択されたファイル名を既存のファイルリストと比較 */
	selectedFiles.forEach(file => {
		if(existfiles.includes(file.name)) {
			duplicateFiles.push(file.name);
		}
	});

	/* 重複ファイルがある場合に、確認ダイアログを表示 */
	if(duplicateFiles.length > 0) {
		const dpfilelist = duplicateFiles.join(", ");
		const confirmation = confirm("次のファイル名は既にカスタム絵文字として存在しています。\\n" + dpfilelist + "\\n\\nアップロードを続行しますか？(※同名ファイルは上書きされます。)");
		if(!confirmation) {
			/* キャンセルされた場合は送信を中止 */
		...
		$tab2html .= '<p>あなたの権限では、カスタム絵文字のアップロードや削除はできません。<br>アップロードや削除をするには、Lv.7以上の権限のあるIDでログインし直して下さい。</p><p>※既存のカスタム絵文字を投稿本文中に使うことはできます。</p>';
	}
	...
	my $msg = qq|
		$topguide
		<div class="cemojiTab">
			<input type="radio" name="tabset" class="tabcheck" id="tabcheck1" $checkedtab[1]><label for="tabcheck1" class="tab" id="tab1"><span class="stdtabname">一覧</span></label>
			<input type="radio" name="tabset" class="tabcheck" id="tabcheck2" $checkedtab[2]><label for="tabcheck2" class="tab" id="tab2"><span class="stdtabname">追加/削除</span></label>
...
$tab2html
			...
			<label for="tabcheck1" class="tab tabbtm" id="tabbtm1"><span class="stdtabname">一覧</span></label>
			<label for="tabcheck2" class="tab tabbtm" id="tabbtm2"><span class="stdtabname">追加/削除</span></label>
			<p id="bottom">
			</p>
		</div>
	|;
	...
	my $css = q|<style>
		...
		.oneCemoji { text-align:center; border:1px solid #cec; padding:1em; border-radius:0.5em; background-color:#fff; }
		...
		.ceTable { border-collapse:collapse; border:1px solid green; font-size:0.9rem; line-height:1.1; }
		.ceTable th { background-color:green; color:white; padding:0.25rem; }
		.ceTable td { background-color:#fff; border-width:1px; border-style:solid dashed; border-color:green; padding:0.25rem 0.5rem; }
		.ceTable td.cepreview { padding: 3px; }
		.cepreview img { width: auto; height:2rem; display:block; margin:auto; }
		.filesize { text-align:right; }
		.usesearch { white-space:nowrap; }
		.btnlink.uselink { margin:0; }
		.btnlink.uselink:first-child { margin-right:3px; }

		.imageuploadarea { border-width: 1px 0; border-style: dashed; border-color: gray; margin: 1em 0; padding: 1rem 0.75rem; background-color: #ffd; }
		.uploadformguide { margin: 1em 0 0 0; font-size: 0.75em; color: #888; line-height:1.3; }
		.maxguidebox { margin:1rem 0 0 0; font-size:0.9rem; }
		.maxguidebox p { margin:0; }
		.alert { color:crimson; }
		...
		input:checked + #tab1 , #tabcheck1:checked ~ #tabbtm1 { background-color: #ffd; color: #d00; 	position:relative; }
		input:checked + #tab2 , #tabcheck2:checked ~ #tabbtm2 { background-color: #efe; color: #080; position:relative; }
		/* ▼タブの中身(共通装飾＋非選択状態の装飾) */
		.tabcontent {
			display: none; border: 1px solid gray; margin-top: -1px; position: relative; z-index: 0; padding: 0.25rem 1rem;
		}
		/* ▼チェックが入っているラジオボタンに対応するタブの中身を表示する */
		#tabcheck1:checked ~ #tabcontent1,
		#tabcheck2:checked ~ #tabcontent2 { display: block; }
		/* ▼タブ背景 */
		#tabcontent1 { background-color: #ffd; }
		#tabcontent2 { background-color: #efe; }
	</style>|;
...
sub adminImages
{
	...
	# ▼絞り込み検索時の事前調整
	# …………………………………
	# 絞り込み検索中の場合は、新規アップロードフォームが畳まれるようにする
	if(( $cp{'search'} ne '' ) && ( $resmsg eq '' )) {
		$resmsg = '絞り込み検索結果を表示しています。';
	}

	# unused で絞り込まれている場合は、「画像インデックス」と「投稿データ」のタイムスタンプを比較して、画像インデックスの方が古い場合には、再度「未使用画像を探す」機能を実行するよう促す。
	if( index($cp{'search'},'unused') >= 0 ) {
		# 検索語の中に unused があるなら
		my $lasttimeUnusedImgFound = $setdat{'unusedimgfounddate'} || 0;	# 最後に未使用画像を探した日時
		my $bmod = &fcts::getModTimeStamp($bmsdata);						# 投稿データの更新日時

		if( $lasttimeUnusedImgFound < $bmod ) {
			# 画像インデックスファイルの更新日時が、投稿データの更新日時より古いなら、画像インデックスの再生成を促す
			my $emsg = q|<p><b>未使用画像を探す処理を先にして下さい。</b></p>|;
			if( $lasttimeUnusedImgFound == 0 ) {
				# 未使用画像を探した日時を記録がない場合
			}
			else {
				# ある場合
				$emsg .= q|
					<p>画像一覧を未使用画像で絞り込む操作がされました。<br>
					しかし、前回に未使用画像を探して以後、投稿データが更新されているため、現状では「正確な未使用画像リスト」を把握できていない可能性があります。</p>
				|;
			}
			$emsg .= q|
				<p>この操作を続行するには、未使用画像を新たに探し直す必要があります。下記のボタンを押して、未使用画像を探して下さい。</p>
				<p><a href="?mode=admin&amp;work=unusedimages" class="btnlink">未使用画像を探す(JS版)</a></p>
			|;
			&showadminpage('MANAGE IMAGES','',$emsg,'IA','');
			exit;
		}
	}
	...
	my $ifilelist = &listupImagesFromIndex($plv);	# listupImageFilesを使うのは廃止(>3.8.7)
	...
	# ------------------------
	# 画像ファイルリストの表示
	$msg .= '<div class="imagetablearea">' . $ifilelist . '</div><p class="noticebox ">※このページに掲載される「1ページあたりの画像個数」は<a href="?mode=admin&amp;work=setting&amp;page=4">システム設定</a>→【<a href="?mode=admin&amp;work=setting&amp;page=4#sysCtrlpanel">管理画面内の表示</a>】で変更できます。<small>（通常の一覧時と、絞り込み表示時とで、別々に個数を指定できます。）</small><br>※<b>誰でも使用可：</b>どのIDでUPされた画像でも、誰でも何度でも投稿文中に埋め込んで掲載できます。<br>※<b>削除の条件：</b>管理者・編集者権限のあるIDなら、どの画像でも削除できます。それ未満の権限では、自分でUPした画像だけを削除できます。ただし、ゲスト権限では一切削除できません。<br>※<b>一覧表示対象</b>：設定で許可された拡張子以外のファイルは、この一覧には表示されません。FTP等の別手段でUPされたファイルも表示されますが、サブディレクトリにあるファイルは表示されません。<br>※設定で投稿可能総数を無制限にしても、サーバ側のディスクスペースに上限がある点にご注意下さい。<br>※アップロードはできるのに<strong class="important">画像が表示されない</strong>場合は、<a href="' . $aif{'puburl'} . 'setup/#imagenotshow">トラブルシューティング</a>をご参照下さい。</p>';
	...
	my $css = q|<style>
		...
		.ctrlboxCover { display:flex; flex-wrap:wrap; gap:0.5em 2em; align-items:center; justify-content:flex-start; margin:1em 0; background-color:#f0fff0; padding:0.75em; border:1px solid #ded; }
		.ctrlbox { display:flex; flex-wrap:wrap; gap:0.5em 0; align-items:end; justify-content:flex-start; }
		.acthead { font-size:0.75em; display:block; text-align:left; }
		.searchCover { min-width:300px; }
		.filteringBox { width: 240px; }
		.filteringBox::placeholder { color:#888; }
		.filterReset { font-size: 0.67em; }
		.filtered { color:crimson; font-weight:bold; }
		.addFilter { display:inline-block; font-size:0.75em; margin-left:1em; }
		.foundcount { font-weight:bold; }
		...
		.imgflag { font-size:0.75em; display:block; border:1px solid green; background-color:green; color:white; line-height:1; padding:0.45em; margin-bottom:3px; text-decoration:none; }
		.imgflag:hover { background-color:white; color:green; }
		...
		@media (max-width: 919px) {
			...
			select.actcheck, .filteringBox { font-size:1rem; }
...
sub adminImageMeta
{
	...
	# 編集可能な項目を安全化
	$cap	= &fcts::forsafety($cap);	# キャプション
	$date	= &fcts::forsafety($date);	# 投稿日時 (データが正しければ安全化の必要はないが念のため)
	...
	$msg .= qq|
		...
		<div class="imagecover">
			...
			<div class="metacol">
				<form action="$cgipath" method="post" id="imagemetaEditform">
					<table class="imtable">
						<tr><th>ファイルパス：</th><td class="imgpath">$fp</td></tr>
						$thumbbox
						<tr><th>投稿者(ID)：</th><td class="imgupid" >$imgupname $imgupid</td></tr>
						<tr><th>投稿日時 <span class="kome">(※1)</span>：</th><td class="imguptime"><input type="text" value="$date" name="tstamp" id="tstamp" class="textinput" required></td></tr>
						<tr><th>キャプション：</th><td class="cap"><input type="text" value="$cap" name="caption" class="textinput"></td></tr>
						<tr><th>フラグ <span class="kome">(※2)</span>：</th><td class="flags">
							...
							<label style="display:none;"><input type="checkbox" name="flag_unused" value="1"$flagcheck{'unused'}>未使用</label>
						</td></tr>
						<tr><th>縦横サイズ：</th><td class="size">
							<ul class="sizechosebox">
								<li><label><input type="radio" name="sizechose" value="A"$sizechose[0]>自動取得</label>：$imgwh <span class="kome">(※3)</span></li>
								<li><label><input type="radio" name="sizechose" value="M"$sizechose[1]>手動指定</label>：<span class="sizeinputs">横<input type="number" value="$sizewh[0]" name="sizew" min="1" max="10000" class="numinput">×縦<input type="number" value="$sizewh[1]" name="sizeh" min="1" max="10000" class="numinput"> <span class="kome">(※4)</span></span></li>
							</ul>
						</td></tr>
					...
					<p class="btnbox">
						...
						<input type="hidden" value="$fp" name="imgpath">
						<input type="hidden" value="$cp{'page'}" name="page">
						<input type="hidden" value="$cp{'search'}" name="q">
						<input type="hidden" value="admin" name="mode">
		...
		<p class="noticebox">
			※1：この日時を修正することで、画像ファイルのタイムスタンプを変更できます。変更すると、画像一覧モードでの並び順、新着投稿画像の並び順、画像管理画面での並び順が変わります。<br>
			※2：フラグの動作については、公式ヘルプの「<a href="$aif{'puburl'}usage/#howtouse-imagemanager-flags">画像に付与できるフラグの種類と仕様</a>」項目や、「<a href="$aif{'puburl'}usage/#howtouse-newpostflagrule">鍵付き投稿・下書き投稿と同時にアップロードされた画像には、最初から「一覧外」フラグが立つ</a>」項目をご覧下さい。<br>
			※3：縦横サイズを自動取得できるのは、GIF、PNG、JPEG、SVG形式の一部のみです。<br>
			※4：ここで手動指定できる縦横サイズは、あくまでもHTMLのimg要素の出力時にwidth属性値やheight属性値に使われるだけです。実際にどんなサイズで画像が表示されるかは、CSSでどのように装飾されているか次第です。なので、ここで縦横サイズを指定したからといって、<strong>そのサイズで表示されるとは限りません</strong>。詳しくは、公式ヘルプの「<a href="$aif{'puburl'}usage/#howtouse-imagemanager-imgedit">画像情報の編集画面</a>」項目をご覧下さい。
		</p>
	| . q|
		<script>
		document.getElementById('imagemetaEditform').addEventListener('submit', function(event) {
			const datetimeInput = document.getElementById('tstamp').value.trim();
			const datetimePattern = /^\\d{4}\\/\\d{2}\\/\\d{2}\\s+\\d{2}:\\d{2}:\\d{2}$/;
			if(!datetimePattern.test(datetimeInput)) {
				alert('投稿日時を修正する際は、必ず YYYY/MM/DD hh:mm:ss 形式で入力して下さい。西暦は4桁、その他は必ず2桁必要です。時刻も秒まで必須です。');
				event.preventDefault();
			}
		});
		</script>
	...
	my $css = q|<style>
		...
		.imtable th { text-align:left; color:darkgreen; width:7.5em; }
		.imtable td { }
		.textinput { font-size:1rem; width:100%; box-sizing: border-box; }
...
sub adminUpdateImageMeta
{
	...
	# デモモードでの実行は拒否
	if( $flagDemo{'RefuseToChangeSettings'} == 1 ) {
		&demomodemsg('画像メタデータの更新機能は使用できません。(※キャプションやフラグを指定したい場合は一度画像を削除して、アップロード時に望みのキャプションやフラグを指定して下さい。)');
	}
	...
	# 更新画像のファイルパスを受信
	my $fp =  $cgi->param('imgpath') || "$imagefolder/$file";	# 得られなければ、ファイル名に画像保存用ディレクトリ名を加えて作る
	...
	my $tstamp = $cgi->param('tstamp') || '';	# timestamp (YYYY/MM/DD hh:mm:ss)

	my $flag_nolisted = $cgi->param('flag_nolisted') || 0;	# 1 or (undefined→0)
	my $flag_nsfw = $cgi->param('flag_nsfw') || 0;	# 1 or (undefined→0)
	my $flag_unused = $cgi->param('flag_unused') || 0;	# 1 or (undefined→0)
	...
	# --------------------------------------------------
	# ▼画像ファイルのタイムスタンプを(必要なら)更新する
	# --------------------------------------------------
	# 実ファイルのタイムスタンプを取得しておく
	my $origindate = $imeta_ref->{'date'};

	# タイムスタンプが指定されていれば
	if( $tstamp ne '' ) {
		# タイムスタンプ変更文字列の正当性を確認
		$tstamp = &fcts::trim($tstamp);	# 前後の空白を取り除く
		$tstamp =~ s/[\s　]+/ /g;		# 連続する空白(全角の空白文字も含む)を半角スペース1つにする
		if( $tstamp =~ m/^(\d{4})\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}$/ ) {
			# YYYY/MM/DD hh:mm:ss 形式に沿っている場合
			if( $1 >= 1970 ) {
				# 西暦が1970年以上の場合のみ、指定日時をエポック秒に戻す
				my $epoch = &fcts::getepochtime($tstamp);
				if( $epoch > 0 ) {
					# 値が1以上ならエポック秒に変換できているので、タイムスタンプの更新操作が必要かどうかを判断
					if( $tstamp ne $origindate ) {
						# 元のタイムスタンプとは異なる日時がしていされているなら、その値を使ってファイルのタイムスタンプを更新する
						if( utime $epoch, $epoch, $fp ) {
							# タイムスタンプの変更に成功した場合
							$addmsg = '※画像ファイルのタイムスタンプを ' . $tstamp . ' に変更しました。<small>(タイムスタンプが変わると画像の並び順も変わるため、日時を大きく変更した場合は当該画像が現在のページからは見えなくなっている可能性がある点にご注意下さい。)</small>';
						}
						else {
							# タイムスタンプの変更に失敗した場合
							$tstamp = $origindate;
							$addmsg = '※画像ファイルのタイムスタンプの変更処理(utime)に失敗しました。';
						}
					}
					else {
						# 更新の必要が無い
					...
					# エポック秒が得られなければ
					$tstamp = $origindate;
					$addmsg = '※日付の変換に失敗したため、画像ファイルのタイムスタンプは修正されませんでした（実在しない日時が指定された可能性があります）。';
				}
			}
			else {
				# 1970年より前なら拒否
				$tstamp = $origindate;
				$addmsg = '※投稿日付(画像ファイルのタイムスタンプ)は、1970年より後の日付しか指定できません。日付は修正されませんでした。';
			}
		}
		else {
			# 形式が正しくない場合は、タイムスタンプの変更はしない (オリジナルのタイムスタンプをそのまま使う)
			$tstamp = $origindate;
			$addmsg = '※投稿日付(画像ファイルのタイムスタンプ)の日付形式が正しくなかったので、日付は修正されませんでした。';
		}
	}
	else {
		# タイムスタンプが指定されていなければ、オリジナルのタイムスタンプを使う
		$tstamp = $origindate;
	}
	# ---------▲ここまで▲-----------------------------------------
	...
	if( $flag_nsfw		) { push(@flags,'nsfw'); }
	if( $flag_unused	) { push(@flags,'unused'); }
	$flagstar = join(',',@flags);
	...
	my %newimeta = (
		...
		flag  => $flagstar,
		cap   => $caption,
		date  => $tstamp		# ←メモ：この情報は送らなくても、実ファイルのタイムスタンプで更新される
	);
	...
	&adminImages("画像 $file の情報を更新しました。<br>$addmsg");
}

# -----------------------------------
# ADMIN：未使用画像を探す用の管理画面
# -----------------------------------
sub adminUnusedImages
{
	# ログイン確認・権限取得
	my $permittedid = &fcts::checkpermission();		# ログイン中ユーザのID名が得られる
	if( !$permittedid ) {
	...
	if( $plv < 7 ) {
		# ユーザ権限がLv.7未満の場合は実行を拒否
		my $err .= '<p>この機能を実行するには権限が足りません。Lv.7以上のIDでログインし直してから実行して下さい。</p>';
		&showadminpage('PERMISSION DENIED','',$err,'CA','');
		exit;
	}
	...
	if(!( -d $imagefolder )) {
		# 画像UP用ディレクトリがなければ
		&errormsg('画像保存用ディレクトリが見つかりません。on adminUnusedImages');	# この関数の実行段階でここが実行されることは普通ないハズなのでエラー画面を出す。
		exit;
	}

	# 画像インデックスを更新(または生成)して読み込む
	my $iires = &loadImageIndex(1);
	if( $iires < 0 ) { &errormsg('画像インデックスを生成できませんでした。on adminUnusedImages'); }	# エラー時

	# ワンタイムトークンを発行しておく
	my $token = &onetimeTokenPublish();

	# --------------------------------------------------
	# JavaScriptで未使用画像を探すためのデータを出力する
	# --------------------------------------------------
	my $fileNameList = '';
	# 画像インデックスをループしてファイル一覧を取得
	foreach my $onerec (@imgindex) {
		# その画像の情報を得る
		$fileNameList .= '"' . &fcts::forsafety( &fcts::getcontent($onerec,'file') ) . '",' . "\n";
	}
	# JS用に出力
	my $fileNamesForJSArray = 'let imgfiles = [' . $fileNameList . ']';

	my $allPostList = '';
	# 全投稿をループ
	foreach my $oneclip (@xmldata) {
		# 本文だけを得る
		my $escdcom = &fcts::forsafety( &fcts::getcontent($oneclip,'comment') );		# HTML系記号を安全化した状態の本文
		$escdcom =~ s|\\|\\\\|g;		# 無用なエスケープシーケンスがJavaScript側で認識されないようにエスケープしておく
		$allPostList .= '"' . $escdcom . '",' . "\n";
	}
	# JS用に出力
	my $allPostsForJSArray = 'let posts = [' . $allPostList . ']';
	...
	$msg .= &fcts::tooneline( qq|
		<div id="fuiHead">
			<p>未使用画像を探しています...</p>
			<p class="fuiNote">※総データ数が多い場合には待ち時間が長くなる可能性があります。<br>※ローカルマシンパワーを使っているため、CPU使用率が上がる可能性があります。<br>➡ 自動で画面が切り替わるまで、何も触れずにお待ち下さい。</p>
		</div>
		<div id="fuiSubmit" style="display:none;">
			<p><span class="fuiResTitle">【未使用画像の検索結果】</span><br>総画像数 <b id="res_totalimgs">x</b>個のうち、未使用画像は <b id="res_unusedimgs">x</b>個でした。</p>
			<p>自動的に処理が継続されます。そのまま何も触れずにお待ち下さい。<i class="loader"></i></p>
			<p id="manualSubmit" style="display:none;">
				もし、画面がなかなか切り替わらない場合は、以下のボタンを押して下さい。<br>
				<input type="button" value="続行する" onclick="document.unusedimgs.submit();">
			</p>
		</div>
		<div class="resBoxes">
			<div class="imglistcover" id="ResultList"><p class="waitmsg"><span id="nowprocess2"></span><br>お待ち下さい...</p></div>
			<div class="imglistcover" id="ImgFileList"><p class="waitmsg"><span id="nowprocess1">対象データを準備しています。<i class="loader"></i></span><br>お待ち下さい...</p></div>
		</div>
		<form action="$cgipath" method="post" name="unusedimgs" id="unusedimgs" style="display:none;">
			<textarea name="uifilelist" id="uifilelist"></textarea>
			<input type="hidden" value="$token" name="token">
			<input type="hidden" value="admin" name="mode">
			<input type="hidden" value="unusedimgres" name="work">
		</form>
	| );
	...
	$msg .= &fcts::tooneline( qq|
		...
		/* この後の自動画面遷移後に、ブラウザの「戻る」ボタンが扱われても、この処理ページには戻らないように(現在ページの)履歴を書き換えておく。 */
		history.replaceState(null, "", "?mode=admin&work=images");

		let result;
		/* -------------------- */
		/* ▼メッセージを再描画 */
		/* -------------------- */
		function announceShow(target,msg) {
			document.getElementById(target).innerHTML = msg;
		}
		/* ------------------ */
		/* ▼画像リストを描画 */
		/* ------------------ */
		function showNowImgList(targetarray, showfor) {
			...
			const imgFileList = document.getElementById(showfor);
			imgFileList.innerHTML = '';
			imgFileList.appendChild(listelement);
		}
		/* ------------------------------------------------------------ */
		/* ▼imgfilesの中で postsのどの要素にも含まれていない要素を抽出 */
		/* ------------------------------------------------------------ */
		function findUnusedItems() {
			result = imgfiles.filter(imgfn => 
				!posts.some(comment => comment.includes(imgfn))
			);
		}
		/* ----------------------------------------- */
		/* ▼抽出結果をCGIに戻すためのフォーム用処理 */
		/* ----------------------------------------- */
		function makeResultForm() {
			document.getElementById("res_totalimgs").innerText = imgfiles.length;
			document.getElementById("res_unusedimgs").innerText = result.length;
			document.getElementById("uifilelist").value = result.join("\|");
			document.getElementById("fuiSubmit").style.display = 'block';
			document.getElementById("fuiHead").style.display = 'none';
			setTimeout(() => {
				document.getElementById("manualSubmit").style.display = 'block';
			}, 60000);
			document.unusedimgs.submit();
		}
	| );

	$msg .= $fileNamesForJSArray . ";\n\n";
	$msg .= $allPostsForJSArray . ";\n\n";

	$msg .= &fcts::tooneline( qq|
		/* ------------------------------------------ */
		/* その都度画面を更新するよう、順番に実行する */
		/* ------------------------------------------ */
		announceShow('nowprocess1','生成中です。');
		announceShow('nowprocess2','探索中です。<i class="loader"></i>');
		requestAnimationFrame(() => {
			/* 1フレーム目 */
			showNowImgList(imgfiles,'ImgFileList');
			requestAnimationFrame(() => {
				/* 2フレーム目 */
				findUnusedItems();
				showNowImgList(result,'ResultList');
				requestAnimationFrame(() => {
					/* 3フレーム目 */
					makeResultForm();
				});
	...
	# CSS
	my $css = q|<style>
		.resBoxes { display: flex; align-items: start; gap: 1rem; flex-wrap:wrap; flex-direction: row-reverse; width:fit-content; }
		.imglistcover { border:1px solid gray; font-size:0.75em; max-width:100%; overflow:auto; }
		.imglistcover::before { content: ''; display:block; background-color:gray; color:white; padding:0.25em; }
		#ImgFileList { border-color: royalblue; }
		#ResultList  { border-color: hotpink; }
		#ImgFileList::before { content: '全画像リスト';	background-color: royalblue; }
		#ResultList::before  { content: '未使用画像';	background-color: hotpink; }
		.waitmsg { width:fit-content; margin:1em; padding:1em 1.5em; border:1px solid darkorange; border-radius:2em; background-color:#ffc; }
		.fuiResTitle { display:inline-block; color:crimson; }
		#manualSubmit { border:1px solid hotpink; background-color:#fee; border-radius:0.75em; width:fit-content; padding:0.75em; margin:1em 0; }
		@media all and (max-width: 599px) {
			.fuiNote { font-size:0.8em; }
		}
		.loader {
			display: inline-block;
			width: 25px;
	...
	&showadminpage('FINDING UNUSED IMAGES','',$msg,'CA',$css);
}

# -------------------------------------------------------
# ADMIN：ローカルマシンで探した未使用画像にフラグを付ける
# -------------------------------------------------------
sub adminUnusedImgRes
{
	# ログイン確認・権限取得
	my $permittedid = &fcts::checkpermission();		# ログイン中ユーザのID名が得られる
	if( !$permittedid ) {
	...
	# 画像インデックスを読み込む
	my $iires = &preLoadImageIndex();
	if( $iires < 0 ) { &errormsg('画像インデックスが読めませんでした。on adminUnusedImages'); }	# エラー時

	# ------------------
	# データ受信内容確認
	# ------------------
	my $uifilelist = $cgi->param('uifilelist') || '';	# 縦棒区切りで未使用画像ファイル名が列挙されている仕様
	my $token = $cgi->param('token') || '';				# ワンタイムトークン (偽装フォームからの送信を防ぐため)

	# ワンタイムトークンを確認する
	&onetimeTokenCheck($token,1);

	# ------------------------------------
	# ▼未使用画像に unused フラグを付ける
	# ------------------------------------
	# 未使用画像を配列にする
	my @unusedfiles = split(/\|/,$uifilelist);	# 注：縦棒で分解する際はエスケープが必要！

	# 一旦、画像インデックス内の全項目から未使用フラグを下ろす (※「既に未使用フラグが付いているけども、今は使われている」という場合にはフラグを外さないといけないので。)
	&delallunusedflags();

	# 画像インデックスから当該ファイル名を探してフラグを立てる
	my $upcount = &imgflaggingup( 'unused', @unusedfiles );

	# 処理が完了したら、ワンタイムトークンを抹消する
	&onetimeTokenClear();

	# 未使用画像を探した日時を記録しておく
	my @trywrites;
	push(@trywrites,"unusedimgfounddate=" . time);
	&savesettings( @trywrites );

	# すべての処理が終わったら、未使用画像の検索状態で画像管理画面を表示する
			# ↓この方法で表示すると遷移後の画面でリロードができなくなるので、URLを作ってリダイレクトする。
			# $cp{'search'} = 'unused';
			# &adminImages();
	my $cgipath = &getCgiPath('mode=admin&work=images&q=unused');
	print "Location: $cgipath\n\n\n";
}

# ----------------------------------------------------
# 画像インデックス内から未使用画像フラグを一括削除する
# ----------------------------------------------------
sub delallunusedflags
{
	# 既存インデックス内をループしてフラグを書き換える。データは元の @imgindex 内の要素を直接書き換える。
	for my $i (0 .. $#imgindex) {
		# 画像インデックスのフラグ部分だけをアップデート
		$imgindex[$i] =~ s|<flag>(.*?),?unused(.*?)</flag>|<flag>$1$2</flag>|;	# unused の文字列だけを消す(unusedの直前にカンマがある場合はそれも同時に消す)
		$imgindex[$i] =~ s|<flag>,(.*?)</flag>|<flag>$1</flag>|;				# 編集の結果として先頭にカンマが1つ残った場合はそれも消す
	}
}

# ----------------------------------------------
# 指定画像ファイル群に一括して指定フラグを立てる		引数1：立てるフラグ名(ただし「-」なら削除の意味)、引数2以降：対象画像ファイル名の配列
# ----------------------------------------------		返値：更新した件数
sub imgflaggingup
{
	my $targetflag = shift @_ || return 0;	# 引数1がなければ何も処理しない
	my @targetfiles = @_;
	...
	if( $ret < 0 ) {
		# エラーなら中止
		&errormsg('画像インデックスが読めませんでした。on imgflaggingup');
	}

	# 作業数カウンタ
	my $workcount = 0;

	# 既存インデックス内をループして、指定画像ならフラグを書き換える。なければそのまま。データは元の @imgindex 内の要素を直接書き換える。
	for my $i (0 .. $#imgindex) {
		my $operate = 0;	# フラグを操作するかどうかのフラグ( 1=フラグを立てる / 0=何もしない )

		# ファイル名を得て比較する
		my $onefilename = &fcts::getcontent($imgindex[$i],'file');
		if( $#targetfiles >= 0 ) {
			# 対象画像ファイル群が残っている場合にだけ一致を確認する
			foreach my $tfname (@targetfiles) {
				if( $onefilename eq $tfname ) {
					$operate = 1;
					@targetfiles = grep { $_ ne $tfname } @targetfiles;		# 対象ファイルリストから、いま見つけたファイル名を消しておく (※同じファイル名を二度チェックさせないために。)
					last;
				}
			}
		}
		else {
			# 対象画像ファイル群が残っていなければ、ループを終わる
			last;
		}
		...
		# フラグデータを作る
		my $newflags = '';
		my $oldflags = &fcts::getcontent($imgindex[$i],'flag');
		if( $targetflag eq '-' ) {
			# 指定フラグが「削除」の意味なら全部消す
			$newflags = '';
		}
		elsif( $oldflags eq '' ) {
			# フラグが何もないなら、シンプルに代入する
			$newflags = $targetflag;
		}
		elsif( index($oldflags,$targetflag) < 0 ) {
			# 何らかのフラグは立っているが、指定フラグはまだ立っていないなら、追加で立てる
			$newflags = &fcts::commaseparateadd( $targetflag, $oldflags );
		}
		else {
			# 指定フラグが既に立っているならそのまま採用する
			$newflags = $oldflags;
		}

		# 画像インデックスのフラグ部分だけをアップデート
		$imgindex[$i] =~ s|<flag>.*?</flag>|<flag>$newflags</flag>|;
		$workcount++;
	}

	# 書き換えたデータがあるなら、画像インデックスファイルを上書き更新する
	if( $workcount > 0 ) {
		if( &saveImageIndex() <= 0 ) {
			# 書き込みに失敗した場合はエラーコードを返す
			return -1;
		}
	}
...
{
	# --- 注意 ---
	# 画像インデックスを読み込みたい場合は、この関数ではなく preLoadImageIndex の方を呼ぶこと。
	# この関数だと読み込み済みであることを示すグローバルフラグが立たないので。
	# (※何らかの理由で読み込み済みフラグを立てたくない場合には、この関数を直接呼べば良いが。)
...
# ----------------------------------------------------
# 現状の画像インデックスをそのままファイルに上書き保存
# ----------------------------------------------------		返値：1=保存成功、エラー値(0=書き込み失敗、-1=ディレクトリがない、-2=ファイルがない)
sub saveImageIndex
{
	# 存在を前提とするGlobal変数: @imgindex			# 画像インデックスのメインデータ配列
	# 存在を前提とするGlobal変数: $imgidxmeta		# 画像インデックスのメタデータ
	# これらのデータをそのままファイルに出力する (必要な変更は別途上記のGlobal変数に実施済みであることが前提)
	...
	my $iifile = "$imagefolder/$subfile{'indexXML'}";
	if(!( -f $iifile )) {
		# 存在しなければエラー (※この関数では既存の画像インデックスファイルがあることが前提)
		return -2;
	}

	my @newxmldata = ();
	@newxmldata = @imgindex;				# 既存の画像インデックスデータをコピー
	unshift(@newxmldata , $imgidxmeta);		# 既存のメタデータを先頭に追加
...
# ----------------------------
# 既存の画像インデックスを更新	引数1：生成先ファイルパス、引数2：存在しない画像のデータを(1=消す/0=残す)
# ----------------------------	返値：実行結果フラグ(1:生成成功/0:書き込み失敗/-1:画像用ディレクトリがない)
sub updateImageIndex
{
	my $iifile = shift @_ || &errormsg("No file path on updateImageIndex.");
	my $delNofileDat = shift @_ || 0;
	...
	foreach my $onerec (@imgindex) {
		...
		# 記録されているファイル名が、実際の画像ファイル群リスト内に見つからない場合は、画像が直接削除されている ( $workflag の値は -1 )
		...
		if( $workflag == 0 ) {
			...
			if(( $delNofileDat != 1 ) || ( $newsize > 0 )) {
				# 存在しない画像のデータを残す設定か、またはファイルサイズが1Byte以上あるなら記録
				my $newxmlline = &fcts::makerecord( 'image',
...
sub outputLightboxLoader
{
	...
	if(( $setdat{'isuselightbox'} == 2 ) && ( $forceLightbox != 1 )) {
		...
		if( $setdat{'imageexpandingjs'} ne '' ) {
			...
			$loadjs  = $setdat{'imageexpandingjs'};
			$loadcss = $setdat{'imageexpandingcss'};
		}
	}

	# Lightbox本体JS＋CSSの読み込み
	$ret .= outputDynamicCssLoadHtml( $loadcss );
	$ret .= outputDynamicJSLoadHtml( $loadjs );

# 	return $ret;
	return &fcts::tooneline($ret);
}

# JavaScriptの動的読み込みソースを返す		引数:JavaScriptファイルのURL(空白記号区切りで複数個指定可)
sub outputDynamicJSLoadHtml
{
	my $str = shift @_ || '';

	$str = &fcts::trim($str);		# 前後の空白を削除
	my @urls = split(/\s+/, $str);	# 1つ以上の半角空白を区切りとして分割

	# モジュールとして読み込むかどうか
	my $module = '';
	if( $setdat{'imageexpandjsmodule'} == 1 ) {
		$module = ' type="module"';
	}

	my $ret = '<!-- 画像拡大表示用JavaScript： -->' . "\n";
	foreach my $oneUrl (@urls) {
		$ret .= '<script' . $module . ' src="' . &fcts::forsafety($oneUrl) . '"></script>' . "\n";
	}

	return $ret;
}

# CSSの動的読み込みソースを返す		引数:CSSファイルのURL(空白記号区切りで複数個指定可)
sub outputDynamicCssLoadHtml
{
	my $str = shift @_ || '';
	...
	foreach my $oneUrl (@urls) {
		$ret .= q|
			<script>
				...
				delaycss.href = '| . &fcts::forsafety($oneUrl)  . q|';
...
# --------------------------------------------------------
# 指定の文字列がどこかの投稿に含まれているかどうかを調べる		引数:調べる文字列
# --------------------------------------------------------		返値:1=ある、0=ない
sub checkWrittenInAnyPost
{
	my $searchword = shift @_ || return 0;	# 検索語の指定が無ければ0を返す
...
sub listupImagesFromIndex
{
	...
	# メタデータを得ておく (※これはページネーションだけでなく情報表示にも使われる)
	my $totalfiles	= &fcts::getcontent($imgidxmeta,'files') || 0;		# 画像ファイル個数
	my $totalsize	= &fcts::getcontent($imgidxmeta,'bytes') || 0;		# 総データサイズ

	# 実際に表示する画像データ
	my @extractedimgindex = ();			# 表示する画像だけのインデックス
	my $showtotalfiles = $totalfiles;	# 表示するファイル総数

	# 絞り込み検索語の事前調整
	$cp{'search'} =~ s/　/ /g;		# 全角空白を半角空白にする
	$cp{'search'} =~ s/\s+/ /g;		# 空白系文字の連続を半角空白1つにする
	$cp{'search'} =~ s/\*/ /g;		# アスタリスクを半角空白にする(ファイル名検索時に慣習的に使われそうなアスタリスクをAND検索として扱うため)
	...
	if( $searchInput ne '' ) {
		...
		# そうでないなら未使用画像の削除リンクを出力する
		if( $plv >= 7 ) {
			# ユーザ権限がLv.7以上の場合だけ

			# ----------------------------------------------------
			# ▼サーバリソースを使う版の未使用画像を探す機能用(旧)	※総データ数が少ない場合はこっちの方が画面遷移が少ない分だけ速い
			# ----------------------------------------------------
			if( $setdat{'allowservcalcres'} == 1 ) {
				# 投稿総数と画像総数を乗算して一定数を超えている場合にだけconfirmダイアログを出す準備
				my $power = $totalfiles * ( $#xmldata + 1 );
				my $confirmSource = '';
				if(( $power > 1000000 ) && ( $power <= 66000000 )) {
					# 百万超過の場合は事前警告
					$confirmSource = q|if( confirm('この機能は負荷が高いので、現在の画像総数×投稿総数だと、処理の完了までに数秒～十数秒程度かかる可能性があります。何も操作せずにお待ち下さい。') == false ) { return false; }|;
				}
				elsif( $power > 66000000 ) {
					# 6600万超過(だいたい50秒かかる目安)の場合はローカル推奨
					my $roughMin = '1分以上';
					if( $power > 100000000 ) { $roughMin = '数分（少なく見積もっても2分以上）'; }
					elsif( $power > 80000000 ) { $roughMin = '1分半以上'; }
					# 警告を作る
					if( &fcts::isRunOnLocalhost() ) {
						# Localhostで稼働している場合
						$confirmSource = q|if( confirm('この機能は負荷が高いので、現在の画像総数×投稿総数だと、処理の完了までに| . $roughMin . q|かかる可能性があります。しばらく何も操作せず、画面が変わるまでお待ち下さい。本当に実行しますか？') == false ) { return false; }|;
					}
					else {
						# おそらくWeb上で稼働している場合
						if( $setdat{'allowhighloadonweb'} == 1 ) {
							# 高負荷処理の実行が許可されている場合
							$confirmSource = q|if( confirm('現在の画像総数×投稿総数だと極めて大きな負荷がかかると予想されます。処理には| . $roughMin . q|かかる可能性があり、共用サーバで稼働している場合はサーバ側から途中で実行の継続を拒否される可能性があります。また、繰り返し実行すると当CGIの使用を禁止される危険もあります。ローカル環境(Localhost)での実行を強く推奨しますが、本当にここで実行しますか？') == false ) { return false; }|;
						}
						else {
							$confirmSource = q|alert('現在の画像総数×投稿総数だと極めて大きな負荷がかかると予想されるため、Web上ではこの機能を実行できません。ローカル環境(Localhost)での実行をお勧め致します。'); return false;|;
						}
					}
				}
				# 出力
				$addFiltering .= q|<a href="#" class="addFilter" onclick="| . $confirmSource . q| var filteringBox = document.getElementById('filteringBox'); var filterSubmit = document.getElementById('filterSubmit'); filteringBox.value = '$$notuse'; filterSubmit.value = 'お待ち下さい...'; this.style.display = 'none'; filterSubmit.disabled = true; filteringBox.form.submit(); return false;">未使用画像を探す(Server版)</a>|;
			}
			# ----------------------------------------------------
			# ▲サーバリソースを使う版の未使用画像を探す機能用(旧)
			# ----------------------------------------------------

			# 両方の機能が表示される場合の区分用文字列
			my $kubun = '';
			if( $setdat{'allowservcalcres'} == 1 ) { $kubun = '(JS版)'; }

			my $cgipath = &getCgiPath();
			my $findunusedimageslink = &makeQueryString('mode=admin','work=unusedimages');
			$addFiltering .= q|<a href="| . $findunusedimageslink . q|" class="addFilter">未使用画像を探す| . $kubun . q|</a>|;
		}
	}

	# 絞り込みが必要なら先に画像インデックスから絞り込んでおく
	if( $searchInput ne '' ) {
		# 絞り込み検索語が指定されていれば検索する
		my $condNotUse = 0;
		if(( index($cp{'search'},'$$notuse' ) >= 0 ) && ( $plv >= 7 )) {
			# 検索語に $$notuse がある場合は「使われていない画像」を探すフラグを立てる (※ユーザ権限値がLv.7以上の場合のみ)
			$condNotUse = 1;
			# 検索語に指定された $$notuse は検索コマンドに置き換える
			$cp{'search'} =~ s/\$\$notuse//;	# U+E195 U+E196 U+E2A8
		}

		# 1ページ当たりの画像掲載個数を、絞り込み検索時用の数にする (※絞り込み検索時用の方が小さい場合は変えない)
		if( $setdat{'imageperpage'} < $setdat{'imagesearchperpage'} ) {
			$setdat{'imageperpage'} = $setdat{'imagesearchperpage'};
		}

		# 画像インデックスをループして表示対象を検索
		foreach my $onerec (@imgindex) {
			# その画像の情報を得る
			...
			# 日付を確認して削除済みファイルなら無視する (※ファイルサイズは常に取得可能とは限らないので存在判定には使えない点に注意)
			if( $date =~ /^0/ ) {
				# 西暦が0で始まっていたら削除済みファイル (eq '0001/01/01 00:00:00'で調べても良いが先頭1文字だけを見る方が速いだろうから)
				next;
			}
			...
			if( $condNotUse ) {
				...
				my $searchword = ':' . $file;
				# 使われていないことを確認
				if( &checkWrittenInAnyPost( $searchword ) == 0 ) {
					# その画像がどの投稿でも使われていない場合には、flagに検索コマンド用フラグを加える
					$flag .= ' ';
				}
				# 使われていた場合には何もしない
			}
			...
			# フラグ検索拡張
			$flag =~ s/nolisted/nolisted 一覧外/;

			# 検索を実行（フラグでも検索可能にする）
			if( &fcts::wordsearch( "$file $date $user $flag $cap" , $cp{'search'} ) == 0 ) {
				# 非該当なら次を探す
				next;
			}

			# 該当したら表示用リストに加える
			push( @extractedimgindex, $onerec );
		}
		# ページネーション用に総数を数える
		$showtotalfiles = $#extractedimgindex + 1;
	}
	else {
		# 絞り込みがないなら、画像インデックスの全量を対象にする
		@extractedimgindex = @imgindex;
	}

	# ページネーション計算
	my( $startid, $endid, $endpage );
	( $startid, $endid, $endpage, $cp{'page'} ) = &fcts::calcpagenation( $showtotalfiles, $setdat{'imageperpage'}, $cp{'page'} );
	...
	# テーブルHTML格納用
	my $restable = qq|<form action="$cgipath" method="post" name="imageform" id="imageform">\n| . q|<table class="images standard"><tr><th>＼</th><th>画像</th><th>投稿ID</th><th>UP日時</th><th>サイズ</th><th>キャプション</th><th>フラグ</th><th>使用場所</th></tr>| . "\n";

	# フラグカウンタ
	my $fcnolisted = 0;
	my $fcnsfw = 0;

	# フラグリンク用の文字列を先に作っておく (ループの内側で何度も生成すると無駄だから)
	my $qNolisted = &makeQueryString('mode=admin','work=images','q=nolisted');	# 一覧外フラグ付き画像だけを一覧するリンク
	my $flaglinknolisted = qq|<a href="$qNolisted" class="imgflag flag-nolisted">一覧外</a>|;
	my $qNsfw = &makeQueryString('mode=admin','work=images','q=nsfw');	# NSFWフラグ付き画像だけを一覧するリンク
	my $flaglinknsfw = qq|<a href="$qNsfw" class="imgflag flag-nsfw">NSFW</a>|;

	# インデックスのファイル情報から表示用のテーブル中身を作る
	my $count = 0;
	foreach my $onerec (@extractedimgindex) {
		# 情報を得る
		my $file	= &fcts::forsafety( &fcts::getcontent($onerec,'file') );	# $of
		...
		if( $count >= $startid && $count <= $endid ) {
			...
			# フラグ解読
			my $flagbtns = '';	# フラグごとにマークアップした文字列の格納用
			my @imgflags = split(/,/, $flag);	# カンマで分解
			foreach my $one ( @imgflags ) {
				# 一致を確認
				if( $one eq 'nolisted' ) {
					$flagbtns .= $flaglinknolisted;	# 一覧外フラグ表示(リンク付き)を加える
					$fcnolisted++;	# カウントする
				}
				if( $one eq 'nsfw' ) {
					$flagbtns .= $flaglinknsfw;		# NSFWフラグ表示(リンク付き)を加える
					$fcnsfw++;	# カウントする
				}
			}
			...
			my $usepostlink = &makeQueryString("q=$file");	# その画像を使用している投稿を探して見るためのURLを作る		# v4.6.5まで「q=PICT: $file」
			my $uselistlink = &makeQueryString('mode=admin','work=manage',"q=$file");	# その画像を使用している投稿を管理画面の一覧で見るためのURLを作る		# v4.6.5まで「q=PICT: $file」
			my $uesearch = &fcts::urlEncode( $cp{'search'} );	# #記号をエンコードしておく
			my $metaeditlink = &makeQueryString('mode=admin','work=imagemeta',"img=$file","q=$uesearch","page=$cp{'page'}");		# その画像のメタデータを編集する画面へのURLを作る
			...
			# 表示しない分に対しては、フラグのカウントだけをする
			my $flag	= &fcts::forsafety( &fcts::getcontent($onerec,'flag') );
			if( index($flag,'nolisted') >= 0 ) { $fcnolisted++;	}
			if( index($flag,'nsfw') >= 0 ) { $fcnsfw++;	}
		}
...
# 		}
	...
	elsif( $showtotalfiles == 0 ) {
		$restable .= '<tr><td colspan="8">※指定の条件では、表示できる画像がありません。</td></tr>';
	}
	...
	# 追加フィルタの準備 (※当該フィルタの該当件数が1件以上あって、当該フィルタがまだ適用されていない場合に限って出力)
	if( index($searchInput,'$$notuse' ) == -1 ) {
		# 検索語として「 $$notuse 」が指定されていない場合にだけ表示する
		if(( $fcnsfw > 0 )	&& ( index($searchInput,'nsfw' ) == -1 ))			{ $addFiltering = &makeFilterLinkForImgList( 'nsfw' , 'NSFW', $fcnsfw , length($searchInput) ) . $addFiltering;	}
		if(( $fcnolisted > 0 )	&& ( index($searchInput,'nolisted') == -1 ))	{ $addFiltering = &makeFilterLinkForImgList( 'nolisted', '一覧外', $fcnolisted, length($searchInput) ) . $addFiltering;	}
	}
	...
	| . "\n" .
	qq|
		<div class="ctrlboxCover">
			<div class="ctrlbox">
				<span class="ctbchecks">
					<input type="button" value="全部選択" onclick="allcheck(true);">
					<input type="button" value="全部解除" onclick="allcheck(false);">
				</span>
				<span>：</span>
				<span class="ctbacts">
					<span class="acthead">選択した画像を...</span>
					<select name="work" form="imageform" class="actcheck">
						...
						<option value="imgflagedit-nolisted">フラグ『一覧外』付きにする</option>
						<option value="imgflagedit-nsfw">フラグ『NSFW』付きにする</option>
						<option value="imgflagedit-_clear">フラグなしにする</option>
					</select>
					<input type="hidden" value="admin" name="mode" form="imageform">
					<input type="hidden" value="$cp{'search'}" name="bef_q" form="imageform">
					<input type="hidden" value="$cp{'page'}" name="bef_page" form="imageform">
					<input type="button" value="実行" onclick="document.imageform.submit();">
				</span>
			</div>
			<div class="searchCover">
				<form action="$cgipath" method="get">
					<span class="acthead">▼$searchTitle：</span><input type="search" name="q" value="$searchInput" placeholder="ファイル名・キャプション・フラグ等" class="filteringBox" id="filteringBox">
					<input type="submit" value="検索" id="filterSubmit"> $resetFiltering$addFiltering
					<input type="hidden" value="$cp{'skindir'}" name="skin">
					<input type="hidden" value="admin" name="mode">
					<input type="hidden" value="images" name="work">
				</form>
			</div>
		</div>
	| . "\n";
	...
	if( $searchInput ne '' ) {
		...
		$baselink = &makeQueryString('mode=admin', 'work=images', "q=$uesearch", 'page=');
	}
	else {
		# 絞り込み時でなければ
		$baselink = &makeQueryString('mode=admin', 'work=images', 'page=');
	}
	...
	my $addstatus = '';
	if( $searchInput ne '' ) {
		$addstatus .= '※現在の<strong class="important">絞り込み</strong>表示対象: <b>' . $showtotalfiles . '</b>個';
		$resctrl .= qq|<p class="foundcount">▼$showtotalfiles個の画像ファイルが該当しました。</p>\n|;
		$resctrl .= q|
<script>
document.addEventListener('DOMContentLoaded', function() {
	// .btnlink クラスを持つ全てのa要素を取得
	const links = document.querySelectorAll('a.btnlink[href*="%24%24notuse"], a.btnlink[href*="$$notuse"]');
	// 各リンクにクリックイベントを追加
	links.forEach(function(link) {
		link.addEventListener('click', function(event) {
			// confirmダイアログがfalseを返したら、ページ移動をキャンセル
			if(!confirm('未使用画像の検索結果を維持した状態でページを移動する場合は、ページ移動に合わせて「未使用画像を探す処理」が再度実行されます。初回に未使用画像を探したときと同じだけの負荷と待ち時間が発生する点にご注意下さい。本当に移動しますか？')) {
				event.preventDefault();
			}
		});
	});
	...
	my $resstatus = '<p class="storageStatus">(使用容量: <b>' . &fcts::byteswithunit($totalsize) . "</b> / 全 $totalfiles 個) <small>$addstatus</small></p>";

	return $respagelist . $resctrl . $restable . $pagelink . $resstatus;
...
# 一覧外・NSFW検索リンクを出力
# ----------------------------
sub makeFilterLinkForImgList
{
	my $searchCommand = shift @_ || '';
	my $linklabel = shift @_ || '';
	my $existNum = shift @_ || 0;
	my $alreadyfiltered = shift @_ || 0;

	my $addSign = '';
	if( $alreadyfiltered ) { $addSign = '+'; }

	return q|<a href="#" class="addFilter" onclick="var filteringBox = document.getElementById('filteringBox'); var filterSubmit = document.getElementById('filterSubmit'); filteringBox.value += ' | . $searchCommand . q|'; filteringBox.form.submit(); return false;">| . $addSign . $linklabel . '(' . $existNum . ')</a>';
}

# ------------------------
# 汎用ファイル削除確認画面
# ------------------------
sub adminFileTrydels
{
	# 画像の削除(TRY)
	my $deltype = $cgi->param('type');		# 削除対象種別
	my @trydels = $cgi->param('filename');	# 指定されたファイル名一覧
	my $msg = '';

	# 不正送信の確認
	&fcts::postsecuritycheck('work=filetrydels');

	# ファイル削除時の共通事前確認(権限確認)処理
	my ($placedir, $targettypename, $backoptions) = &commonPrepForDeleteFile($deltype);
	...
	if( $trydels[0] ne '' ) {
		...
		|;
		my $listupItems = 0;
		foreach my $fn (@trydels) {
			# 削除対象をリストアップ
			my $preview = '';
			if( $deltype eq 'cemoji' ) {
				# カスタム絵文字の場合はその画像を表示
				$preview = qq|<img src="$placedir/$fn" class="thumbnail" alt="">|;
			}
			$msg .= qq|<tr><td>$fn</td><td>$preview<input type="hidden" value="$fn" name="trydelete"></td></tr>|;
			$listupItems++;
		}

		# 冒頭の確認メッセージの作成
		$msg = qq|<p>$targettypenameの中から、以下の $listupItems 個のファイルを削除します。よろしいですか？（削除すると取り消せません。）</p>| . $msg;

		# 確認ダイアログの作成
		my $lotscheck = '';
		if( $listupItems >= 10 ) {
			# 削除個数が10個以上ある場合は確認ダイアログを挟む
			$lotscheck = qq|onclick="return confirm('削除対象が $listupItems 個もあります。本当にこれらのファイルを一括削除してよろしいですか？');"|;
		}

		$msg .= qq|
				</table>
				<input type="hidden" value="admin" name="mode">
				<input type="hidden" value="deletefiles" name="work">
				<input type="hidden" value="cemoji" name="type">
				<input type="submit" value="上記のファイルを削除する"$lotscheck>
...
sub adminDeleteFiles
{
	...
	# 不正送信の確認
	&fcts::postsecuritycheck('work=deletefiles');

	# ファイル削除時の共通事前確認(権限確認)処理
	my ($placedir, $targettypename, $backoptions) = &commonPrepForDeleteFile($deltype);

	# 報告の準備
	my $msg = '<p>' . $targettypename . 'の削除処理結果：</p><ul class="deletelist">';

	# ファイルが存在していれば消す
	my $count = 0;
	foreach my $onefile ( @trydels ) {
		if( $onefile ne '' ) {
			# 文字列があれば、指定ディレクトリ名を加えてファイルパスを作る
			my $of = $placedir . $onefile;
			# そのファイルの存在を確認
			if( -f $of ) {
				# あれば消す
				unlink( $of );
	...
	$msg .= '</ul><p>計 ' . $count . '個のファイルを削除しました。</p>';
	...
	&showadminpage('DELETED','',$msg,$backoptions,$css);
}

# ------------------------------------------
# ファイル削除(確認＋本番)共通の事前確認処理	※sub adminFileTrydels と adminDeleteFiles から呼ばれる。
# ------------------------------------------	引数:削除する種別(cemoji)	返値：ファイル格納ディレクトリ名(最後はスラッシュ記号であることを保証)
sub commonPrepForDeleteFile
{
	my $deltype = shift @_ || '';
	...
	# 種別名を作る
	# ------------
	my $targettypename = '(不明)';
	if( $deltype eq 'cemoji' ) {
		# カスタム絵文字の場合
		$targettypename = 'カスタム絵文字';
	}
	...
	# デモモードでの実行は拒否
	if( $flagDemo{'RefuseToChangeSettings'} == 1 ) {
		&demomodemsg($targettypename . 'の削除機能は使用できません。');
	}
	...
	if( $deltype eq 'cemoji' ) {
		...
		&errormsg('typeパラメータが不正です。 on commonPrepForDeleteFile');
		exit;
	}
	...
	if( $plv < $reqLv ) {
		# 削除に必要な権限がなければ拒否
		my $msg = '<p>必要な権限がないため、' . $targettypename . 'の削除はできませんでした。</p><p>※この操作のためには、Lv.' . $reqLv . ' 以上の権限のあるIDが必要です。</p>';
		&showadminpage('NO PERMISSION','',$msg,$backoptions,'');
		exit;
	}

	# ------------------------
	# ファイル格納ディレクトリ (※スラッシュで終わる文字列)
	# ------------------------
	my $placedir = './';
	if( $deltype eq 'cemoji' ) {
		# カスタム絵文字の場合
		...
		my $cedir = &fcts::trim( $setdat{'cemojidir'} ) || 'emoji';		# カスタム絵文字の格納ディレクトリ(前後の空白を除外)
		my $ceservdir = &cemojidircheck($cedir);		# サーバ上のディレクトリ保持用 (相対パス または ファイルシステムの絶対パス)
		$placedir = &fcts::makelastcharslash($ceservdir);
	}

	# 実行して問題なければ、対応ディレクトリ名等の情報をまとめて返す
	return ($placedir, $targettypename, $backoptions);
}
...
sub adminImageTrydels
{
	...
	if( $trydels[0] ne '' ) {
		...
		# 冒頭の確認メッセージの作成
		$msg = qq|<p>画像保存用ディレクトリの中から、以下の $listupItems 個の画像ファイルを削除します。よろしいですか？（削除すると取り消せません。）</p>| . $msg;
...
sub adminDeleteImages
{
	...
	foreach my $onefile ( @trydels ) {
		if( $onefile ne '' ) {
			...
			# サムネイルもあれば併せて消す
			my $tf = &fcts::makethumbnailpath( $of, $subfile{'miniDIR'} );		# サムネイルがあればそのファイルパスを得る、なければ元のファイルパスを得る(＝既に消えている)
			if( -f $tf ) {
				# あれば消す
				unlink( $tf );
				$msg .= '<li>サムネイルも削除しました: ' . &fcts::forsafety($tf) . "</li>\n";
				$count++;
				$thumbcount++;
			}
		}
	...
	my $thumbrep = '';
	if( $thumbcount > 0 ) {
		$thumbrep = '（内 ' . $thumbcount . '個はサムネイル画像）';
	}
	$msg .= '</ul><p>計 ' . $count . '個の画像ファイルを削除しました。' . $thumbrep . '</p>';

	# 画像インデックスを再生成する (削除ファイルの情報は消す設定で)
	my $uiires = &updateImageIndex( "$imagefolder/$subfile{'indexXML'}", 1 );
	if( $uiires == 1 ) {
		$msg .= '<p>画像インデックスを更新しました。</p>';
	}
	else {
		$msg .= '<p>ただし、何らかの要因で画像インデックスファイルを更新できませんでした。</p>';
	}
...
# ADMIN：既存画像のフラグ一括セット
# ---------------------------------
sub adminImgBlukFlagEdit
{
	# 情報受信
	my @targets = $cgi->param('filename');	# 指定されたファイル名一覧
	my $bef_q = $cgi->param('bef_q');		# 直前の画像一覧画面で表示していた検索語
	my $bef_page = $cgi->param('bef_page');	# 直前の画像一覧画面で表示していたページ番号

	# 不正送信の確認
	&fcts::postsecuritycheck('work=imgflagedit');
	...
	if(scalar @targets > 0) {
		...
		# workパラメータから対象フラグを抜き出す
		$cp{'work'} =~ m/imgflagedit-(\w*)/;
		my $flagtype = '';
		my $flagname = '';
		my $resmsg = '';
		if( $1 eq 'nolisted' )	{ $flagtype = 'nolisted';	$flagname = '一覧外';	$resmsg = '、フラグ『一覧外』を付加'; }
		elsif( $1 eq 'nsfw' )	{ $flagtype = 'nsfw';		$flagname = 'NSFW';		$resmsg = '、フラグ『NSFW』を付加'; }
		elsif( $1 eq '_clear' )	{ $flagtype = '-';	$resmsg = '付加されていたフラグをすべて外'; }		# imgflaggingup関数で、削除指定は「-」。
		else {
			# 既定外の値ならエラー
			&errormsg('workパラメータが不正です。 on adminImgBlukFlagEdit');
			exit;
		}

		# 画像インデックスから当該ファイル名を探してフラグを立てる
		my $upcount = &imgflaggingup( $flagtype, @targets );

		# フラグを消した場合は「未使用画像」フラグも消えているので、「未使用画像を探した日時を記録」も消しておく
		if( $flagtype eq '-' ) {
			my @trywrites;
			push(@trywrites,"unusedimgfounddate=");
			&savesettings( @trywrites );
		}

		# 結果報告画面を作る
		$msg = "<p>下記の画像ファイル計 $upcount 個に$resmsgしました。</p><ul><li>" . join('</li><li>',@targets) . '</li></ul>';
		my $backlink = &makeQueryString('mode=admin','work=images',"q=$bef_q","page=$bef_page");
		my $flaglink = &makeQueryString('mode=admin','work=images',"q=$flagtype");
		my $itoplink = &makeQueryString('mode=admin','work=images');

		$msg .= '<p>';
		if(( $bef_q ne '' ) || ( $bef_page > 1 )) {
			# 絞り込み条件が空ではないか、または表示ページが1より大きいなら
			$msg .= qq|<a class="btnlink" href="$backlink">直前に表示していた状態の画像管理ページに戻る</a><br>|;
		}
		if( $flagtype ne '-' ) {
			# フラグの削除ではなければ
			$msg .= qq|<a class="btnlink" href="$flaglink">フラグ「$flagname」の付加された画像の一覧を見る</a><br>|;
		}
		$msg .= qq|<a class="btnlink" href="$itoplink">画像管理TOPへ戻る</a>|;
		$msg .= '</p>';
	}
	else {
		# ファイルが指定されていなければ
		$msg = '<p>画像ファイルが1つも選択されていません。</p>';
	}

	my $css = '';

	&showadminpage('BULK EDIT IMAGE FLAGS','',$msg,'IA',$css);
}

# --------------------------------------
# 下書き・鍵付き・下げる検索リンクを出力
# --------------------------------------
sub makeFilterLinkForManage
{
	my $searchCommand = shift @_ || '';
	my $linklabel = shift @_ || '';
	my $existNum = shift @_ || 0;
...
sub adminManage
{
	...
	my $msg = qq|
		...
		<ul class="sysguide">
			<li><strong>削除</strong>：左端の投稿番号にチェック<input type="checkbox" checked>を入れて「選択した投稿を削除」ボタンを押して下さい。(複数投稿の一括削除も可能)</li>
			<li><strong>編集</strong>：右端の<span class="smallbutton">Edit.00</span>ボタンを押して下さい。</li>
			<li><strong>表示</strong>：投稿内容抜粋欄に見える本文を押せば、その投稿単独ページに移動します。</li>
		</ul>
	| . "\n";
	...
	foreach my $oneclip (@xmldata) {
		...
		if(( $plv >= 7 ) || ( $user eq $permittedid )) {
			...
			if( $showflag ) {
				...
				# フラグ確認
				my $flaglinks = '';
				if( $flags =~ /draft/ )	{ $flaglinks .= '<a href="' . &makeQueryString('mode=admin','work=manage','q=$ps%3Ddraft') . '">下書き</a> '; $lineclass .= 'draft'; $fcdraft++; }
				if( $flags =~ /lock/ )	{ $flaglinks .= '<a href="' . &makeQueryString('mode=admin','work=manage','q=$ps%3Dlock') . '">鍵付き</a> '; $lineclass .= ' lock'; $fclock++; }
				if( $flags =~ /rear/ )	{ $flaglinks .= '<a href="' . &makeQueryString('mode=admin','work=manage','q=$ps%3Drear') . '">下げる</a> '; $lineclass .= ' rear'; $fcrear++; }
	...
	# 追加フィルタの準備 (※当該フィルタの該当件数が1件以上あって、当該フィルタがまだ適用されていない場合に限って出力)
	my $addFiltering = '';
	if(( $fcdraft > 0 )	&& ( index($searchInput,'$ps=draft') == -1 )) {	$addFiltering .= &makeFilterLinkForManage( '$ps=draft', '下書き', $fcdraft, length($searchInput) );	}
	if(( $fclock > 0 )	&& ( index($searchInput,'$ps=lock' ) == -1 )) {	$addFiltering .= &makeFilterLinkForManage( '$ps=lock' , '鍵付き', $fclock , length($searchInput) );	}
	if(( $fcrear > 0 )	&& ( index($searchInput,'$ps=rear' ) == -1 )) {	$addFiltering .= &makeFilterLinkForManage( '$ps=rear' , '下げる', $fcrear , length($searchInput) );	}
	...
	my $headform = qq|
		...
		<div class="ctrlboxCover">
			...
			<div class="searchCover">
				<form action="$cgipath" method="get">
					<span class="acthead">▼$searchTitle：</span><input type="search" name="q" value="$searchInput" placeholder="任意の検索語・カテゴリID・投稿日付等" class="filteringBox" id="filteringBox">
					<input type="submit" value="検索" id="filterSubmit"> $resetFiltering$addFiltering
					<input type="hidden" value="admin" name="mode">
	...
	$msg .= $respagelist . $headform . $restable;
	$msg .= qq|
			<input type="hidden" value="admin" name="mode">
			<input type="hidden" value="trydels" name="work">
			$pagelink
		...
		<p class="noticebox">
			※1ページあたり<b> $postperpageforsyslist </b>件を表示する設定になっています。この設定は、<a href="?mode=admin&amp;work=setting&amp;page=4">システム設定</a>→【<a href="?mode=admin&amp;work=setting&amp;page=4#sysCtrlpanel">管理画面内の表示</a>】から変更できます。
		</p>
	|;

	# CSS
	my $css = q|<style>
		.ctrlboxCover { display:flex; flex-wrap:wrap; gap:0.5em 2em; align-items:end; justify-content:flex-start; margin:1em 0; background-color:#f0fff0; padding:0.75em; border:1px solid #ded; }
		.ctrlbox { display:flex; flex-wrap:wrap; gap:0.5em 0; align-items:end; justify-content:flex-start; }
		.acthead { font-size:0.75em; display:block; text-align:left; }
		.searchCover { min-width:300px; }
		.filteringBox { width: 240px; }
...
sub deletedecos
{
	...
	$str =~ s{\(リンク:(YouTube|VIDEO|Instagram|Insta|CodePen|AppleMusic|Spotify)\)}{($1)}ig;

	$str =~ s/\[[A-Z0-9]:[A-Za-z0-9]+://g;	# 英大文字1文字＋オプションによる装飾記法の開始部分を削除
	$str =~ s/\[[A-Z0-9]://g;				# 英大文字1文字による装飾記法の開始部分を削除
...
sub adminSetting
{
	...
	$setdat{'freelines'} =~ s/<br>/\r\n/g;	# 名前付き1行単位フリースペースの改行変換
	$setdat{'extracss'} =~ s/<br>/\r\n/g;	# 上書きスタイルの改行変換
	$setdat{'easyinputs'} =~ s|<BR />|\r\n|g;	# 何でも簡単入力の改行変換
	...
	my %decoBtnElmnt = ();
	my @decoBtns = ('B','C','D','E','F','H','I','M','Q','S','T','U','1','2','3','4','5','6','7','8','9');
	...
	my @urlcardsizedef = ('','');			$urlcardsizedef[$setdat{'urlcardsizedef'}] = 'checked';
	...
	my @faviconuse = ('','','');			$faviconuse[$setdat{'faviconuse'}] = 'checked';
	my @faviconfor = ('','','');			$faviconfor[$setdat{'faviconfor'}] = 'checked';
	...
	my @showStateBtnStyle = ('','','');		$showStateBtnStyle[$setdat{'showStateBtnStyle'}] = 'checked';
	my @showFuncBtnStyle = ('','','');		$showFuncBtnStyle[$setdat{'showFuncBtnStyle'}] = 'checked';
	my @easyinputWay = ('','');				$easyinputWay[$setdat{'easyinputWay'}] = 'checked';
	...
	$setdat{'galleryimgtolink'}		= &getattributeforcheckbox($setdat{'galleryimgtolink'});
	...
	$setdat{'cemojimaxlimits'}		= &getattributeforcheckbox($setdat{'cemojimaxlimits'});
	...
	$setdat{'urlexpandvideo'}		= &getattributeforcheckbox($setdat{'urlexpandvideo'});
	$setdat{'urlexpandvideolim'}	= &getattributeforcheckbox($setdat{'urlexpandvideolim'});
	...
	$setdat{'allowanylineinsecret'}	= &getattributeforcheckbox($setdat{'allowanylineinsecret'});
	...
	$setdat{'checknowcatbeforehand'} = &getattributeforcheckbox($setdat{'checknowcatbeforehand'});
	...
	$setdat{'imageexpandjsmodule'}	= &getattributeforcheckbox($setdat{'imageexpandjsmodule'});
	$setdat{'autobackup'}			= &getattributeforcheckbox($setdat{'autobackup'});
	$setdat{'loginnoindex'}			= &getattributeforcheckbox($setdat{'loginnoindex'});
	...
	$setdat{'hideenvonerror'}		= &getattributeforcheckbox($setdat{'hideenvonerror'});
	$setdat{'allowservcalcres'}		= &getattributeforcheckbox($setdat{'allowservcalcres'});
	$setdat{'allowhighloadonweb'}	= &getattributeforcheckbox($setdat{'allowhighloadonweb'});
	$setdat{'outputjsconsolelog'}	= &getattributeforcheckbox($setdat{'outputjsconsolelog'});
	...
	$setdat{'showLinkBtnCrd'} = &getattributeforcheckbox($setdat{'showLinkBtnCrd'});
	...
	$setdat{'showDecoBtnOonA'} = &getattributeforcheckbox($setdat{'showDecoBtnOonA'});		$setdat{'showDecoBtnOonQ'} = &getattributeforcheckbox($setdat{'showDecoBtnOonQ'});
	...
	$setdat{'showDecoBtn1onA'} = &getattributeforcheckbox($setdat{'showDecoBtn1onA'});		$setdat{'showDecoBtn1onQ'} = &getattributeforcheckbox($setdat{'showDecoBtn1onQ'});
	$setdat{'showDecoBtn2onA'} = &getattributeforcheckbox($setdat{'showDecoBtn2onA'});		$setdat{'showDecoBtn2onQ'} = &getattributeforcheckbox($setdat{'showDecoBtn2onQ'});
	$setdat{'showDecoBtn3onA'} = &getattributeforcheckbox($setdat{'showDecoBtn3onA'});		$setdat{'showDecoBtn3onQ'} = &getattributeforcheckbox($setdat{'showDecoBtn3onQ'});
	$setdat{'showDecoBtn4onA'} = &getattributeforcheckbox($setdat{'showDecoBtn4onA'});		$setdat{'showDecoBtn4onQ'} = &getattributeforcheckbox($setdat{'showDecoBtn4onQ'});
	$setdat{'showDecoBtn5onA'} = &getattributeforcheckbox($setdat{'showDecoBtn5onA'});		$setdat{'showDecoBtn5onQ'} = &getattributeforcheckbox($setdat{'showDecoBtn5onQ'});
	$setdat{'showDecoBtn6onA'} = &getattributeforcheckbox($setdat{'showDecoBtn6onA'});		$setdat{'showDecoBtn6onQ'} = &getattributeforcheckbox($setdat{'showDecoBtn6onQ'});
	$setdat{'showDecoBtn7onA'} = &getattributeforcheckbox($setdat{'showDecoBtn7onA'});		$setdat{'showDecoBtn7onQ'} = &getattributeforcheckbox($setdat{'showDecoBtn7onQ'});
	$setdat{'showDecoBtn8onA'} = &getattributeforcheckbox($setdat{'showDecoBtn8onA'});		$setdat{'showDecoBtn8onQ'} = &getattributeforcheckbox($setdat{'showDecoBtn8onQ'});
	$setdat{'showDecoBtn9onA'} = &getattributeforcheckbox($setdat{'showDecoBtn9onA'});		$setdat{'showDecoBtn9onQ'} = &getattributeforcheckbox($setdat{'showDecoBtn9onQ'});
	...
	$setdat{'showseckeystate'}		= &getattributeforcheckbox($setdat{'showseckeystate'});

	$setdat{'showFuncBtnSpeech'}	= &getattributeforcheckbox($setdat{'showFuncBtnSpeech'});
	$setdat{'showFuncBtnMaxEdit'}	= &getattributeforcheckbox($setdat{'showFuncBtnMaxEdit'});
	$setdat{'easyinputUse'}			= &getattributeforcheckbox($setdat{'easyinputUse'});
	...
	# LCC関連
	my $hideinfreever = '';
	if(( $setdat{'licencecode'} eq '' ) || ($setdat{'licencecode'} =~ /^\(登録ミス\)/)) {
		# コードの入力がなければオプションは非表示
		$hideinfreever = 'style="display:none;"';
		$setdat{'licencecode'} = '';
	}
	elsif ( &fcts::lcc($setdat{'licencecode'}) != 1 ) {
		# LicenceCodeの文法が正しくなければ挿入しない(ただし確認リンクは表示)
		if($setdat{'licencecode'} =~ /^\[登録ミス\]/) {
			# ミス登録2回目なら再表示
	...
	$setdat{'cemojimaxbytes'} = int( $setdat{'cemojimaxbytes'} / 10.24 ) / 100;				# Bytes→KB (画像1枚あたりの最大サイズ)
	$setdat{'cemojistoragelimit'} = int( $setdat{'cemojistoragelimit'} / 10485.76 ) / 100;	# Bytes→MB (画像保存に使える最大容量)
			...
			<ul class="list">
				<li><input type="checkbox" name="urlautolink" id="urlautolink" value="1" $setdat{'urlautolink'}><label for="urlautolink">URLを自動でリンクにする</label><span class="inputguide">(※1)</span>
					...
					<ul class="list">
						...
						<li><label><input type="checkbox" name="urlparenthesislink" value="1" $setdat{'urlparenthesislink'}>URLを丸括弧で囲む [ラベル](URL) 記法も使用可能にする</label>
							<p class="innersubtitle withseparator">▼カード型リンクでの表示時：</p>
							<ul class="list">
								<li>Twitter Cardの指定がない場合に採用するカードサイズ：
									<ul>
										<li><label><input type="radio" name="urlcardsizedef" value="0" $urlcardsizedef[0]>小画像 <small>(.cardsize-S／summary)</small></label></li>
										<li><label><input type="radio" name="urlcardsizedef" value="1" $urlcardsizedef[1]>大画像 <small>(.cardsize-L／summary_large_image)</small></label></li>
									</ul>
								</li>
							</ul>
						</li>
					...
					<ul class="list">
						...
						<li><input type="checkbox" name="urlexpandyoutube" id="urlexpandyoutube" value="1" $setdat{'urlexpandyoutube'}><label for="urlexpandyoutube">URLの直前に[YouTube]ラベルがあればYouTubeを埋め込む</label>
							<ul class="list">
						...
						<li><input type="checkbox" name="urlexpandvideo" id="urlexpandvideo" value="1" $setdat{'urlexpandvideo'}><label for="urlexpandvideo">URLの直前に[VIDEO]ラベルがあれば動画として埋め込む</label>
							<ul class="subopt">
								<li><label><input type="checkbox" name="urlexpandvideolim" value="1" $setdat{'urlexpandvideolim'}>埋め込むのは、同一ドメイン下にある動画ファイルに限る</label></li>
							</ul>
						</li>
					</ul>
	...
	push(@ctrlset,([
		...
		qq|
			...
			<ul class="list">
				...
				<li><label><input type="checkbox" name="allowanylineinsecret" value="1" $setdat{'allowanylineinsecret'}>本文中の任意の位置に鍵入力フォームを配置可能にする</label> <span class="inputguide">(※8)</span></li>
				<li><label><input type="checkbox" name="allowonepictinsecret" value="1" $setdat{'allowonepictinsecret'}>n枚目の画像を [[ONEPICT:n]] 記法等で表示する</label> <span class="inputguide">(※9)</span></li>
			</ul>
			<p class="inputguide">
				...
				※8：鍵入力フォームを好きな位置に配置したい場合に。鍵入力フォームより後にある文字だけが隠され、前にある文字は何行でも全部が常に表示されます。これは※6～7の設定より優先されます。<br>
				※9：OGPでは1枚目の画像を見せたい場合や、ギャラリーモードやそれに準ずるスキン等で(鍵の入力前でも)指定番目の画像を見せたい場合にはここをONにして下さい。
			</p>
		|
	]));
	...
	push(@ctrlset,([
		...
		'postQuick',
		'QUICKPOSTの表示',
		'uh,usage/#howtouse-catcheck-beforehand',
		'24.5em',
		qq|
			<p>
			...
			<ul>
				<li><label><input type="checkbox" name="checknowcatbeforehand" value="1" $setdat{'checknowcatbeforehand'}>当該カテゴリに最初からチェックを入れておく</label></li>
			</ul>
			<p class="inputguide">
				※何らかのカテゴリに限定表示されている状況では、該当するカテゴリ欄のチェックが最初からONの状態のQUICKPOSTを出力する機能です。(カテゴリボタンが表示される設定の場合のみ)
			</p>
		|
	]));
	...
	push(@ctrlset,([
		...
		'postForm',
		'投稿入力欄の表示と動作',
		'uh,usage/#howtouse-key-ctrl-updown',
		'35em',
		qq|
			<p>
	...
	push(@ctrlset,([
		...
		'postBox',
		'投稿コントロール枠内の設定',
		'uh,usage/#howtouse-key-ctrl-enter',
		'',
		qq|
			<p>
		...
		qq|
			...
			<div class="dbTableCover">
				<table class="decBtnTable standard">
					...
					<tr><td><label><input type="checkbox" name="showDecoBtnOonA" value="1" $setdat{'showDecoBtnOonA'}><span>折り畳む</span><span class="inputguide">(※4)</span></label></td><td><label><input type="checkbox" name="showDecoBtnOonQ" value="1" $setdat{'showDecoBtnOonQ'}><span>折り畳む</span></label></td><td><input type="text" value="$setdat{'decoBtnLabelO'}"  name="decoBtnLabelO" class="shortinput"></td><td><span class="inputguide">(専用の要素)</span></td></tr>
					<tr><td><label><input type="checkbox" name="showDecoBtnFonA" value="1" $setdat{'showDecoBtnFonA'}><span>自由装飾</span><span class="inputguide">(※5)</span></label></td><td><label><input type="checkbox" name="showDecoBtnFonQ" value="1" $setdat{'showDecoBtnFonQ'}><span>自由装飾</span></label></td><td><input type="text" value="$setdat{'decoBtnLabelF'}"  name="decoBtnLabelF" class="shortinput"></td><td>$decoBtnElmnt{'F'}</td></tr>
				</table>
				<div class="addTableBox">
					<div id="showAddTableBtn">
						<input type="button" value="追加の自由装飾ボタンを表示する" onclick="document.getElementById('showAddTableBtn').style.display='none'; document.getElementById('addTable').style.display='table';">
						<p>▲自由な装飾を加えられる汎用ボタンをさらに9個増やせます。<br>設定したい場合は上記のボタンを押して下さい。</p>
					</div>
					<table class="decBtnTable standard" id="addTable">
						<tr><th>新規/編集画面</th><th>QUICKPOST</th><th>ラベル</th><th>出力要素<span class="inputguide">(※2)</span></th></tr>
						<tr><td><label><input type="checkbox" name="showDecoBtn1onA" value="1" $setdat{'showDecoBtn1onA'}><span>汎用1</span><span class="inputguide">(※5)</span></label></td><td><label><input type="checkbox" name="showDecoBtn1onQ" value="1" $setdat{'showDecoBtn1onQ'}><span>汎用1</span></label></td><td><input type="text" value="$setdat{'decoBtnLabel1'}"  name="decoBtnLabel1" class="shortinput"></td><td>$decoBtnElmnt{'1'}</td></tr>
						<tr><td><label><input type="checkbox" name="showDecoBtn2onA" value="1" $setdat{'showDecoBtn2onA'}><span>汎用2</span></label></td><td><label><input type="checkbox" name="showDecoBtn2onQ" value="1" $setdat{'showDecoBtn2onQ'}><span>汎用2</span></label></td><td><input type="text" value="$setdat{'decoBtnLabel2'}"  name="decoBtnLabel2" class="shortinput"></td><td>$decoBtnElmnt{'2'}</td></tr>
						<tr><td><label><input type="checkbox" name="showDecoBtn3onA" value="1" $setdat{'showDecoBtn3onA'}><span>汎用3</span></label></td><td><label><input type="checkbox" name="showDecoBtn3onQ" value="1" $setdat{'showDecoBtn3onQ'}><span>汎用3</span></label></td><td><input type="text" value="$setdat{'decoBtnLabel3'}"  name="decoBtnLabel3" class="shortinput"></td><td>$decoBtnElmnt{'3'}</td></tr>
						<tr><td><label><input type="checkbox" name="showDecoBtn4onA" value="1" $setdat{'showDecoBtn4onA'}><span>汎用4</span></label></td><td><label><input type="checkbox" name="showDecoBtn4onQ" value="1" $setdat{'showDecoBtn4onQ'}><span>汎用4</span></label></td><td><input type="text" value="$setdat{'decoBtnLabel4'}"  name="decoBtnLabel4" class="shortinput"></td><td>$decoBtnElmnt{'4'}</td></tr>
						<tr><td><label><input type="checkbox" name="showDecoBtn5onA" value="1" $setdat{'showDecoBtn5onA'}><span>汎用5</span></label></td><td><label><input type="checkbox" name="showDecoBtn5onQ" value="1" $setdat{'showDecoBtn5onQ'}><span>汎用5</span></label></td><td><input type="text" value="$setdat{'decoBtnLabel5'}"  name="decoBtnLabel5" class="shortinput"></td><td>$decoBtnElmnt{'5'}</td></tr>
						<tr><td><label><input type="checkbox" name="showDecoBtn6onA" value="1" $setdat{'showDecoBtn6onA'}><span>汎用6</span></label></td><td><label><input type="checkbox" name="showDecoBtn6onQ" value="1" $setdat{'showDecoBtn6onQ'}><span>汎用6</span></label></td><td><input type="text" value="$setdat{'decoBtnLabel6'}"  name="decoBtnLabel6" class="shortinput"></td><td>$decoBtnElmnt{'6'}</td></tr>
						<tr><td><label><input type="checkbox" name="showDecoBtn7onA" value="1" $setdat{'showDecoBtn7onA'}><span>汎用7</span></label></td><td><label><input type="checkbox" name="showDecoBtn7onQ" value="1" $setdat{'showDecoBtn7onQ'}><span>汎用7</span></label></td><td><input type="text" value="$setdat{'decoBtnLabel7'}"  name="decoBtnLabel7" class="shortinput"></td><td>$decoBtnElmnt{'7'}</td></tr>
						<tr><td><label><input type="checkbox" name="showDecoBtn8onA" value="1" $setdat{'showDecoBtn8onA'}><span>汎用8</span></label></td><td><label><input type="checkbox" name="showDecoBtn8onQ" value="1" $setdat{'showDecoBtn8onQ'}><span>汎用8</span></label></td><td><input type="text" value="$setdat{'decoBtnLabel8'}"  name="decoBtnLabel8" class="shortinput"></td><td>$decoBtnElmnt{'8'}</td></tr>
						<tr><td><label><input type="checkbox" name="showDecoBtn9onA" value="1" $setdat{'showDecoBtn9onA'}><span>汎用9</span></label></td><td><label><input type="checkbox" name="showDecoBtn9onQ" value="1" $setdat{'showDecoBtn9onQ'}><span>汎用9</span></label></td><td><input type="text" value="$setdat{'decoBtnLabel9'}"  name="decoBtnLabel9" class="shortinput"></td><td>$decoBtnElmnt{'9'}</td></tr>
					</table>
					<script>
						const checkboxes = document.querySelectorAll('#addTable input[type="checkbox"]');	/* 全チェックボックスを取得 */
						const allUnchecked = Array.from(checkboxes).every(checkbox => !checkbox.checked);	/* 全チェックボックスがオフか確認 */
						if(allUnchecked) {
							/* 全オフならtableは初期非表示 */
							...
							/* 全オフでなければボタンを非表示 */
							document.getElementById('showAddTableBtn').style.display = 'none';
						}
					</script>
				</div><!-- /.addTableBox -->
			</div>
			<p class="inputguide">
				...
				※4:details＋summary要素で、折り畳まれた状態で表示します。HTMLの文法上、インライン要素の内側には書けないのでご注意下さい。<br>
				※5:自由装飾や追加の汎用装飾を使うには、事前にCSSの準備が必須です。<a href="$aif{'puburl'}usage/#howtouse-chardecoration-class" $mpconfirm>詳細はこちら</a>。
			</p>
		|
	]));
	...
	push(@ctrlset,([
		...
		qq|
			...
			<table class="decBtnTable standard">
				...
				<tr><td><label><input type="checkbox" name="showLinkBtnCrd" value="1" $setdat{'showLinkBtnCrd'}><span>OGPカードリンク</span><span class="inputguide">(※2)</span></label></td><td><input type="text" value="$setdat{'linkBtnCrdLabel'}"  name="linkBtnCrdLabel" size="12"></td></tr>
			...
			<p class="inputguide">
				...
				※2：リンク先のOGP情報を読み取ってカード型リンクにする機能は、TegUp Ver.2以降が併置されていないと動作しない点にご注意下さい。<br>
	...
	push(@ctrlset,([
		...
		qq|
			...
			<p class="inputguide">
				※カテゴリが1つも存在しない状況では、設定に関係なくボタンは常に非表示になります。<br><br>
				※上記の設定を「ボタンの押下で展開」にしていても、QUICKPOSTの設定区画「カテゴリ限定表示時の特別動作」内にある「当該カテゴリに最初からチェックを入れておく」項目がONに設定されている場合は、カテゴリ限定表示時に限って「最初から展開しておく(常時表示)」の動作になります。
			</p>
		|
	]));
	...
	push(@ctrlset,([
		...
		qq|
			...
			<ul>
				<li>
					<label><input type="radio" name="showStateBtnStyle" value="0" $showStateBtnStyle[0] id="showStateBtnStyle0">ボタン</label>「<input type="text" value="$setdat{'statebuttonlabel'}"  name="statebuttonlabel" class="shortinput">」<label for="showStateBtnStyle0">の押下で展開</label><br>
					<label><input type="radio" name="showStateBtnStyle" value="1" $showStateBtnStyle[1]>最初から展開しておく(常時表示)</label><br>
					<label><input type="radio" name="showStateBtnStyle" value="2" $showStateBtnStyle[2]>一切表示しない(常時非表示)</label><br>
				</li>
			</ul>
			<p class="withseparator">
				▼表示する公開状態の選択：
			...
			<table class="decBtnTable standard">
				<tr><th>公開状態の種類</th><th>ラベル</th></tr>
				<tr><td><label><input type="checkbox" name="showFuncBtnStaytop" value="1" $setdat{'showFuncBtnStaytop'}><span>先頭に固定</span></label> <span class="inputguide">(※1)</span></td><td><input type="text" value="$setdat{'funcBtnStaytopLabel'}"  name="funcBtnStaytopLabel" size="12"></td></tr>
				<tr><td><label><input type="checkbox" name="showFuncBtnDraft" value="1" $setdat{'showFuncBtnDraft'}><span>下書き(非公開)</span></label> <span class="inputguide">(※2)</span></td><td><input type="text" value="$setdat{'funcBtnDraftLabel'}"  name="funcBtnDraftLabel" size="12"></td></tr>
				<tr><td rowspan="3"><label><input type="checkbox" name="showFuncBtnSecret" value="1" $setdat{'showFuncBtnSecret'}><span>鍵付き</span></label> <span class="inputguide">(※2)</span></td><td><input type="text" value="$setdat{'funcBtnSecretLabel'}"  name="funcBtnSecretLabel" size="12"></td></tr>
				<tr><td><label style="display:flex;align-items:center;"><span style="font-size:0.6em; line-height:1.1; width:3.6em;">個別鍵ボタン</span><input type="text" value="$setdat{'funcBtnSecIndLabel'}"  name="funcBtnSecIndLabel" size="6"></labe></td></tr>
				<tr><td><label style="display:flex;align-items:center;"><span style="font-size:0.6em; line-height:1.1; width:3.6em;">鍵入力窓位置</span><input type="text" value="$setdat{'funcBtnSecFormLabel'}"  name="funcBtnSecFormLabel" size="6"></labe></td></tr>
				<tr><td><label><input type="checkbox" name="showFuncBtnRear" value="1" $setdat{'showFuncBtnRear'}><span>下げる(一覧外)</span></label> <span class="inputguide">(※2,3)</span></td><td><input type="text" value="$setdat{'funcBtnRearLabel'}"  name="funcBtnRearLabel" size="12"></td></tr>
			</table>
			<p class="inputguide">
				※1：管理者権限のないユーザでログインしている場合では、設定に関係なく常に非表示です。<br>
				※2：ゲスト権限では表示されません。個別鍵・鍵入力窓位置ボタンは、それぞれの機能が使用可能な設定の場合にのみ表示されます。<br>
			...
			<ul>
				<li><label><input type="checkbox" name="showseckeystate" value="1" $setdat{'showseckeystate'}>鍵入力窓の挿入前に現在設定を案内</label> <span class="inputguide">(※4)</span></li>
			</ul>
			<p class="inputguide">
				※4：鍵入力窓位置ボタンの押下時に、現在の鍵付き投稿の表示設定を案内するダイアログを表示します。<br>
			</p>
		|
	]));
	...
	push(@ctrlset,([
		...
		'postBtnFunc',
		'機能ボタンの表示設定',
		'uh,usage/#howtouse-editmax;uh,usage/#howtouse-easyinput',
		'24em',
		qq|
			<p>
				▼機能ボタンの表示：<br>
			...
			<table class="decBtnTable standard">
				<tr><th>機能の種類</th><th>ラベル</th></tr>
				<tr><td rowspan="2"><label><input type="checkbox" name="showFuncBtnSpeech" value="1" $setdat{'showFuncBtnSpeech'}><span>読み上げ</span></label></td><td><input type="text" value="$setdat{'funcBtnSpeechLabel'}"  name="funcBtnSpeechLabel" size="12"></td></tr>
				<tr><td><label style="display:flex;align-items:center;"><span style="font-size:0.6em; line-height:1.1; width:3.6em;">止めるボタン</span><input type="text" value="$setdat{'funcBtnSpStopLabel'}"  name="funcBtnSpStopLabel" size="6"></labe></td></tr>

				<tr><td><label><input type="checkbox" name="showFuncBtnMaxEdit" value="1" $setdat{'showFuncBtnMaxEdit'}><span>編集域の最大化</span></label></td><td><input type="text" value="$setdat{'funcBtnMaxEditLabel'}"  name="funcBtnMaxEditLabel" size="12"></td></tr>
			</table>
			<p class="withseparator">
				▼何でも簡単入力ボタン機能：
			</p>
			<ul>
				<li><input type="checkbox" name="easyinputUse" id="easyinputUse" value="1" $setdat{'easyinputUse'}><label for="easyinputUse">何でも簡単入力ボタン機能を使う</label>
					<ul class="list">
						<li>1行1件で入力内容を列挙： <span class="inputguide">(※1)</span>
							<ul>
								<li class="eiBox"><textarea name="easyinputs">$setdat{'easyinputs'}</textarea></li>
								<li>
									投稿欄下部での表示形態：<br>
									<label><input type="radio" name="easyinputWay" value="0" $easyinputWay[0]>各個を独立したボタンとして列挙する</label><br>
									<label><input type="radio" name="easyinputWay" value="1" $easyinputWay[1]>プルダウン(セレクトボックス)にまとめる</label><br>
								</li>
							</ul>
						</li>
					</ul>
			...
			<p class="inputguide">
				※1：ここに列挙した内容が1行ずつ専用ボタンになり、押すとその文字が編集領域に入力されます。&lt;br&gt;と書くと改行を挿入できます。半角コロン記号を使って「ラベル:入力文字列」の形式で書くと、ボタンのラベルを別指定できます。カスタム絵文字記法など、入力文字列自体に半角コロン記号を含めたい場合は、適当なラベルを最初に付けて下さい。<br>
			</p>
		|
	]));
	...
	push(@ctrlset,([
		...
		qq|
			<p>
				◆名前付き1行単位フリースペース入力欄: <span class="inputguide">※$allowhtmlmsg。1件1行「name:中身」の形式でいくつでも記述できます。</span><br>
				<textarea name="freelines" wrap="off" cols="45" rows="5" class="fsp100 mid" placeholder="1件1行で「名称:中身」の記法でいくつでも列挙できます。">$setdat{'freelines'}</textarea><br>
			</p>
			<p class="inputguide">
				※上記の入力欄に記述した内容は、スキンファイル内に <strong>[[FREELINE:～]]</strong> のように記述した位置に挿入されます。
				例えば「 sample:テスト 」のように書けば <strong>[[FREELINE:sample]]</strong> と記述した位置に「テスト」が出力されます。
				名称は、大文字・小文字を区別します（sample・Sample・SAMPLEは異なる名称だと認識されます）。
			</p>
		|
	]));
	...
	push(@ctrlset,([
		...
		qq|
			...
			<ul class="list">
				<li><input type="checkbox" name="galleryoutput" id="galleryoutput" value="1" $setdat{'galleryoutput'}><label for="galleryoutput">ギャラリー表示機能を使う</label> <span class="inputguide">(※1)</span>
					<ul class="list">
						...
						<li><label><input type="checkbox" name="galleryimgtolink" value="1" $setdat{'galleryimgtolink'}>画像を(原寸画像への)リンクにする</label> <span class="inputguide">(※5)</span></li>
					</ul>
			...
			<p class="inputguide">
				...
				※3：挿入の仕様は『ページの表示』→【ページの表示／全体】での設定が使われます。<br>
				※4：状況表示部分に件数を表示し、2ページ目以降ではページ番号も表示します。<br>
				※5：原寸画像へのリンクにする場合、リンク時の仕様は『ページの表示』→【投稿本文の表示／画像】→[画像の表示]で設定された各設定が使われます。<br>
			...
			<ul>
				...
				<li><input type="radio" name="isuselightbox" id="isuselightbox0" value="2" $isuselightbox[2]><label for="isuselightbox0">他のスクリプトを使う</label> <span class="inputguide">(※3,4)</span>
					<ul class="list sublist">
						...
						<li style="list-style-type:none;margin-top:0.33em;"><label><input type="checkbox" name="imageexpandjsmodule" value="1" $setdat{'imageexpandjsmodule'}>JavaScriptをモジュールとして読み込む <span class="inputguide">(※5)</span></label><br>
					</ul>
				</li>
			</ul>
			<p class="inputguide">
				...
				※4：2つ以上のファイルを読み込みたい場合は、<strong class="important">半角空白記号で区切って列挙</strong>できます。個数に上限はありません。なお、jQueryは（スキンの書き方次第では）独自に読み込まれますので重複して読み込ませないようご注意下さい。<br>
				※5：JavaScriptを読み込むscript要素に「type=&quot;module&quot;」を付加します。ESM(モジュール形式)に非対応なスクリプトに付加すると動作しませんのでご注意下さい。
			</p>
		|
	]));
	...
	push(@ctrlset,([
		...
		qq|
			...
			<p class="inputguide">
				※ディレクトリ名だけを指定すれば、てがろぐCGI設置位置のサブディレクトリだと解釈されます。※URLでの指定はできません。<br>
				※「/」で始まるPATHを指定すると、Document Rootを基準にした絶対パスだと解釈されます(※ファイルシステムのRootではなく)。同一サーバ内に設置された他のてがろぐCGIと共有したい場合等に。<br>
				※存在しないディレクトリを指定すれば、カスタム絵文字を無効にできます。
			</p>
			<p class="withseparator">
				▼カスタム絵文字用画像の保存容量
			</p>
			<ul class="list">
				<li><input type="checkbox" name="cemojimaxlimits" id="cemojimaxlimits" value="1" $setdat{'cemojimaxlimits'}><label for="cemojimaxlimits">アップロードできる容量に上限を設ける</label><br>
					<span class="inputguide">※以下の設定は、上限を設ける場合にのみ有効（単位に注意）</span>
					<ul class="list">
						<li><label>画像1枚あたりの最大サイズ：<input type="text" value="$setdat{'cemojimaxbytes'}" name="cemojimaxbytes" class="halfinput"><b class="unit unitKB">KB</b></label> <span class="inputguide">(標準：100KB)</span></li>
						<li><label>画像保存に使える最大容量：<input type="text" value="$setdat{'cemojistoragelimit'}" name="cemojistoragelimit" class="halfinput"><b class="unit unitMB">MB</b></label> <span class="inputguide">(標準：50MB)</span></li>
					</ul>
				</li>
			</ul>
			<p class="inputguide">
				※設定を変更しても、既にUPされたカスタム絵文字画像は消えません。<br>
			</p>
		|
	]));
	...
	push(@ctrlset,([
		...
		qq|
			...
			<ul>
				<li><label><input type="radio" name="faviconuse" value="0" $faviconuse[0]>指定しない (サイト側の設定に任せる)</label> <span class="inputguide">(※1)</span></li>
				<li>	   <input type="radio" name="faviconuse" value="1" $faviconuse[1] id="faviconuse1"><label for="faviconuse1">独自に指定する</label>
					<ul class="list sublist">
						<li><label>Favicon URL等：<input type="text" value="$setdat{'faviconurl'}" name="faviconurl"></label> <span class="inputguide">(※2)</span></li>
					</ul>
				</li>
				<li><label><input type="radio" name="faviconuse" value="2" $faviconuse[2]>内蔵アイコン <img src="| . DEFAULTFAVICON . qq|" style="vertical-align:middle; width:32px; height:32px;"> を使う</label></li>
			</ul>
			<p class="withseparator">
				▼Faviconの適用先
			</p>
			<ul>
				<li><label><input type="radio" name="faviconfor" value="0" $faviconfor[0]>管理画面内のみ</label></li>
				<li><label><input type="radio" name="faviconfor" value="1" $faviconfor[1]>管理画面とスキン <span class="inputguide">(※3)</span></li>
			</ul>
			<p class="inputguide">
				※1：Webサーバにfavicon.ico等があればそれが使われる標準状態です。<br>
				※2：URLのほか「/」で始まる絶対パスや「../」等で始まる相対パスでも指定可能。ただし、WAF環境下では相対パスを書くと403エラーが出る可能性があります。<br>
				※3：スキンには [[FAVICON]] の記述位置に出力されます。この記述がないスキンでは、どこにも出力されません。
			</p>
		|
	]));
	...
	push(@ctrlset,([
		...
		qq|
			...
			<ul class="list">
				<li><label><input type="text" value="$setdat{'conpanedistinction'}" name="conpanedistinction" class="halfinput"></label> <span class="inputguide">(標準:なし) ※てがろぐを複数併用する場合の識別アイコン等に。</span></li>
			</ul>
			<p class="withseparator">
				▼ログインフォームへの追加情報：<br>
			</p>
			<ul class="list">
				<li><label>上部追加メッセージ：<input type="text" value="$setdat{'loginformgid'}" name="loginformgid" class="longinput"></label> <span class="inputguide">(標準:なし)</span></li>
				<li><label>下部追加メッセージ：<input type="text" value="$setdat{'loginformmsg'}" name="loginformmsg" class="longinput"></label> <span class="inputguide">(標準:なし)</span></li>
				<li><label><input type="checkbox" name="loginnoindex" value="1" $setdat{'loginnoindex'}>meta要素(robots noindex)を追加</label>
			</ul>
			<p class="withseparator">
			...
			<ul class="list">
				...
				<li><label>画像管理画面で1ページに表示する画像枚数：<input type="text" value="$setdat{'imageperpage'}" name="imageperpage" size="3" onblur="if(parseInt(this.value, 10) > 2000) { alert('「画像管理画面で1ページに表示する画像枚数」として極端に大きな値を指定すると、画像一覧画面を表示しようとする際に動作が遅くなる（待ち時間が長くなる）可能性がありますのでご注意下さい。値は2000未満をお勧め致します。'); }">枚</label> <span class="inputguide">(標準15)</span>
					<ul class="list">
						<li><label>絞り込み時に1ページに表示する画像枚数：<input type="text" value="$setdat{'imagesearchperpage'}" name="imagesearchperpage" size="3">枚</label> <span class="inputguide">(標準500)</span>
					</ul>
				</li>
			</ul>
			<p class="inputguide">
		...
		qq|
			...
			<div id="highsets">
				...
				<ul>
					...
					<li><label><input type="checkbox" name="hideenvonerror" value="1" $setdat{'hideenvonerror'}>エラー発生時の参考情報の表示を省略する</label></li>
					<li><label><input type="checkbox" name="allowservcalcres" value="1" $setdat{'allowservcalcres'}>サーバ側の計算資源を使う方の機能も表に出す</label></li>
					<li><label><input type="checkbox" name="allowhighloadonweb" value="1" $setdat{'allowhighloadonweb'}>Localhost以外でも高負荷処理の実行を許可する</label> <span class="inputguide">(※5)</span></li>
					<li><label><input type="checkbox" name="outputjsconsolelog" value="1" $setdat{'outputjsconsolelog'}>OGP取得時のリクエストログをConsoleに出力する</label> <span class="inputguide">(※6)</span></li>
					<li>連携コード：<input type="text" value="$setdat{'coopcode'}" name="coopcode" placeholder=""> <span class="inputguide">(※7)</span></li>
				</ul>
				...
				<p class="inputguide">
					...
					※5：インターネット上の共用サーバで稼働している場合は、よほど大丈夫だとの確信がない限り、絶対にONにしないで下さい。<br>
					※6：ブラウザの開発者ツールのコンソールに出力。TegUpへのトークンが含まれるので、常用せずに見たい際にだけONにして下さい。<br>
					※7：何らかの公式サービス等で、個人情報を登録することなく所有権を確認するために使われる予定ですが、今のところ何にも使っていません。^^;
				</p>
				<p class="withseparator">
					▼動作形態：<br>
				</p>
				<ul>
					<li>
						...
						<ul class="list">
							...
							<li>初期画面に表示するメッセージ：<input type="text" value="$setdat{'privopguidemsg'}" name="privopguidemsg" placeholder="" class="stdlinput"> <span class="inputguide">(※9)</span></li>
						</ul>
					</li>
				</ul>
				<p class="inputguide">
					...
					※9：$allowhtmlmsg。空欄にするとメッセージは何も表示されなくなります。<br>
				</p>
			</div>
		|
	]));
...
$tab4html
	...
	my $css = '<style>
		...
		.noticeElm { color:#e88; }
		.eiBox {}
		.eiBox textarea { font-size:1em; height:7.5em; width:min(21em,100%); overflow-x: auto; overflow-y: auto; white-space: pre; word-wrap: normal; }
		/* ▼セパレータ付きBOX */
		...
		/* ▼装飾ボタン選択 */
		.dbTableCover { display:flex; flex-wrap:wrap; gap:1em; align-items:start; }
		.decBtnTable input:checked + span { color: purple; font-weight: bold; }
		#showAddTableBtn { border:1px solid green; padding:1em; background-color:#ffe; }
		#showAddTableBtn p { margin:1.5em 0 0 0; font-size:0.8em; line-height:1.2; max-width:400px; }
		/* ▼フリスペ編集 */
		#fs_fspMultiSpace { display:block; }
		.fsp100 { width: 100%; height: 15em; box-sizing:border-box; }
		.fsp100.mid { height: 12em; }
		.fsp100::placeholder { color:#aaa; }
		...
		@media (max-width: 599px) {
			...
			.decBtnTable th { font-size:0.67rem; word-break: break-all; }
		}
...
sub adminTrychangeset
{
	...
	my %trySetdat = (
		...
		galleryimgtolink	=> $cgi->param('galleryimgtolink'	)	 || 0 ,
		...
		allowanylineinsecret	=> $cgi->param('allowanylineinsecret') || 0 ,
		...
		imagesearchperpage	=> $cgi->param('imagesearchperpage'	) || 15 , 	# 0は許容できない値
		...
		imageexpandjsmodule	=> $cgi->param('imageexpandjsmodule') || 0 ,
		...
		cemojimaxlimits		=> $cgi->param('cemojimaxlimits'	) || 0 ,
		cemojimaxbytes		=> $cgi->param('cemojimaxbytes'		) || 20 ,
		cemojistoragelimit	=> $cgi->param('cemojistoragelimit'	) || 1 ,
		...
		urlcardsizedef		=> $cgi->param('urlcardsizedef'	) || 0 ,
		...
		urlexpandvideo		=> $cgi->param('urlexpandvideo'		) || 0 ,
		urlexpandvideolim	=> $cgi->param('urlexpandvideolim'	) || 0 ,
		...
		checknowcatbeforehand	=> $cgi->param('checknowcatbeforehand'	) || 0 ,
		...
		showDecoBtnOonA	=> $cgi->param('showDecoBtnOonA') || 0 ,	showDecoBtnOonQ	=> $cgi->param('showDecoBtnOonQ') || 0 ,	decoBtnLabelO	=> $cgi->param('decoBtnLabelO') || '折畳' ,
		...
		showDecoBtn1onA	=> $cgi->param('showDecoBtn1onA') || 0 ,	showDecoBtn1onQ	=> $cgi->param('showDecoBtn1onQ') || 0 ,	decoBtnLabel1	=> $cgi->param('decoBtnLabel1') || '①' ,	decoBtnElmnt1	=> $cgi->param('decoBtnElmnt1') || 'span',	,
		showDecoBtn2onA	=> $cgi->param('showDecoBtn2onA') || 0 ,	showDecoBtn2onQ	=> $cgi->param('showDecoBtn2onQ') || 0 ,	decoBtnLabel2	=> $cgi->param('decoBtnLabel2') || '②' ,	decoBtnElmnt2	=> $cgi->param('decoBtnElmnt2') || 'span',	,
		showDecoBtn3onA	=> $cgi->param('showDecoBtn3onA') || 0 ,	showDecoBtn3onQ	=> $cgi->param('showDecoBtn3onQ') || 0 ,	decoBtnLabel3	=> $cgi->param('decoBtnLabel3') || '③' ,	decoBtnElmnt3	=> $cgi->param('decoBtnElmnt3') || 'span',	,
		showDecoBtn4onA	=> $cgi->param('showDecoBtn4onA') || 0 ,	showDecoBtn4onQ	=> $cgi->param('showDecoBtn4onQ') || 0 ,	decoBtnLabel4	=> $cgi->param('decoBtnLabel4') || '④' ,	decoBtnElmnt4	=> $cgi->param('decoBtnElmnt4') || 'span',	,
		showDecoBtn5onA	=> $cgi->param('showDecoBtn5onA') || 0 ,	showDecoBtn5onQ	=> $cgi->param('showDecoBtn5onQ') || 0 ,	decoBtnLabel5	=> $cgi->param('decoBtnLabel5') || '⑤' ,	decoBtnElmnt5	=> $cgi->param('decoBtnElmnt5') || 'span',	,
		showDecoBtn6onA	=> $cgi->param('showDecoBtn6onA') || 0 ,	showDecoBtn6onQ	=> $cgi->param('showDecoBtn6onQ') || 0 ,	decoBtnLabel6	=> $cgi->param('decoBtnLabel6') || '⑥' ,	decoBtnElmnt6	=> $cgi->param('decoBtnElmnt6') || 'span',	,
		showDecoBtn7onA	=> $cgi->param('showDecoBtn7onA') || 0 ,	showDecoBtn7onQ	=> $cgi->param('showDecoBtn7onQ') || 0 ,	decoBtnLabel7	=> $cgi->param('decoBtnLabel7') || '⑦' ,	decoBtnElmnt7	=> $cgi->param('decoBtnElmnt7') || 'span',	,
		showDecoBtn8onA	=> $cgi->param('showDecoBtn8onA') || 0 ,	showDecoBtn8onQ	=> $cgi->param('showDecoBtn8onQ') || 0 ,	decoBtnLabel8	=> $cgi->param('decoBtnLabel8') || '⑧' ,	decoBtnElmnt8	=> $cgi->param('decoBtnElmnt8') || 'span',	,
		showDecoBtn9onA	=> $cgi->param('showDecoBtn9onA') || 0 ,	showDecoBtn9onQ	=> $cgi->param('showDecoBtn9onQ') || 0 ,	decoBtnLabel9	=> $cgi->param('decoBtnLabel9') || '⑨' ,	decoBtnElmnt9	=> $cgi->param('decoBtnElmnt9') || 'span',	,
		...
		showLinkBtnCrd => $cgi->param('showLinkBtnCrd') || 0 ,		linkBtnCrdLabel => $cgi->param('linkBtnCrdLabel') || 'OGPカード' ,
		...
		statebuttonlabel	=> $cgi->param('statebuttonlabel'	) || '公開状態' ,
		...
		showStateBtnStyle	=> $cgi->param('showStateBtnStyle')		|| 0 ,
		...
		showFuncBtnStaytop	=> $cgi->param('showFuncBtnStaytop')	|| 0 ,		funcBtnStaytopLabel	=> $cgi->param('funcBtnStaytopLabel')	|| '先頭に固定' ,
		showFuncBtnDraft	=> $cgi->param('showFuncBtnDraft')		|| 0 ,		funcBtnDraftLabel	=> $cgi->param('funcBtnDraftLabel')		|| '下書き(非公開)' ,
		showFuncBtnSecret	=> $cgi->param('showFuncBtnSecret')		|| 0 ,		funcBtnSecretLabel	=> $cgi->param('funcBtnSecretLabel')	|| '鍵付き' ,
																				funcBtnSecIndLabel	=> $cgi->param('funcBtnSecIndLabel')	|| '個別鍵' ,
																				funcBtnSecFormLabel	=> $cgi->param('funcBtnSecFormLabel')	|| '鍵入力位置' ,
		showFuncBtnRear		=> $cgi->param('showFuncBtnRear')		|| 0 ,		funcBtnRearLabel	=> $cgi->param('funcBtnRearLabel')		|| '下げる(一覧外)' ,
		showseckeystate		=> $cgi->param('showseckeystate')		|| 0 ,

		showFuncBtnSpeech	=> $cgi->param('showFuncBtnSpeech')		|| 0 ,		funcBtnSpeechLabel	=> $cgi->param('funcBtnSpeechLabel')	|| '読み上げ' ,
																				funcBtnSpStopLabel	=> $cgi->param('funcBtnSpStopLabel')	|| '止' ,
		showFuncBtnMaxEdit	=> $cgi->param('showFuncBtnMaxEdit')	|| 0 ,		funcBtnMaxEditLabel	=> $cgi->param('funcBtnMaxEditLabel')	|| '編集最大' ,

		easyinputUse		=> $cgi->param('easyinputUse'		) || 0 ,
		easyinputs			=> $cgi->param('easyinputs'			) || '' ,
		easyinputWay		=> $cgi->param('easyinputWay'		) || 0 ,
		...
		privopguidemsg		=> $cgi->param('privopguidemsg'		) || '' ,
		...
		faviconuse			=> $cgi->param('faviconuse'			) || 0 ,
		faviconurl			=> $cgi->param('faviconurl'			) || '' ,
		faviconfor			=> $cgi->param('faviconfor'			) || 0 ,
		...
		loginformgid		=> $cgi->param('loginformgid'		) || '' ,
		loginnoindex		=> $cgi->param('loginnoindex'		) || 0 ,
		...
		hideenvonerror		=> $cgi->param('hideenvonerror'		) || 0 ,
		allowservcalcres	=> $cgi->param('allowservcalcres'	) || 0 ,
		allowhighloadonweb	=> $cgi->param('allowhighloadonweb'	) || 0 ,
		outputjsconsolelog	=> $cgi->param('outputjsconsolelog'	) || 0 ,
	...
	my @nominus = ('entryperpage','rssentries','galleryentries','imagemaxwidth','imagemaxheight','longurlcutter','hashtagcut','usericonsizew','usericonsizeh','newsignhours','elapsenotationtime','hashtagBtnListupMax',
		...
		'imagemaxbytes','imagefilelimit','imagestoragelimit','cemojimaxbytes','cemojistoragelimit','datelimitreeditdays','backupfilehold','loginlocktime','loginlockminutes','sessiontimenum','postperpageforsyslist');	# 対象群
	...
	$trySetdat{'cemojimaxbytes'} = int($trySetdat{'cemojimaxbytes'} * 1024);				# KB→Bytes (画像1枚あたりの最大サイズ)
	$trySetdat{'cemojistoragelimit'} = int($trySetdat{'cemojistoragelimit'} * 1048576);		# MB→Bytes (画像保存に使える最大容量)
	...
	$trySetdat{'freelines'} =~ s/\r\n|\r|\n/<br>/g;

	# EXTRACSSの保存前処理
	$trySetdat{'extracss'} =~ s/^(\s|\t|\r|\n)+$//;	# 先頭から終わりまでに、空白とタブと改行しか存在しないなら全部消す。
	$trySetdat{'extracss'} =~ s/\r\n|\r|\n/<br>/g;	# 改行を変換（※データは1行にする必要がある）

	# 何でも簡単入力の改行を変換（※データは1行にする必要がある）
	$trySetdat{'easyinputs'} =~ s|\r\n|<BR />|g;
	$trySetdat{'easyinputs'} =~ s{\r|\n}{<BR />}g;		# 改行がCR+LFではない場合も
	$trySetdat{'easyinputs'} =~ s|(<BR />)+|<BR />|g;	# 空行を取り除く(＝連続する改行は1つにする)
...
sub adminRecount
{
	...
	if(( $target eq 'all' ) || ( $target eq 'latestlist' )) {
		...
		$res .= '最新投稿日時：' . $setdat{'latestpostdate'} . "\n";
		$res .= '</div>';
		$donecount++;
	}
	# カテゴリ該当数の再集計
	if(( $target eq 'all' ) || ( $target eq 'cat' )) {
		...
		$res .= '<h2>▼カテゴリ集計:</h2>' . "\n";
		$res .= &categorycounter() . "\n";
		$res .= '<p><a href="' . &makeQueryString('mode=admin','work=categories') . '" class="btnlink">カテゴリ管理へ戻る</a></p>';
		$res .= q|<p class="noticebox">※上記には、削除済みのカテゴリIDでもリストアップされる可能性があります。<br>カテゴリを削除しても、投稿とカテゴリIDとの紐付け情報は消えないためです。ここには、各投稿が属しているすべてのカテゴリIDが列挙されています。従って、既に削除済みのカテゴリでも（そのカテゴリIDと紐付いている投稿が存在する限りは）上記にはリストアップされ続けます。<br>そのため、一度削除したカテゴリでも、同じカテゴリIDを使ってカテゴリを新規作成することで、そのカテゴリを復活させられます。</p>|;
		$res .= '</div>';
		$donecount++;
	}
	# ハッシュタグ該当数の再集計
...
# ADMIN：外部ツールへの移動
# -------------------------
sub adminExternalTool
{
	my $tool = shift @_ || '';	# ツール名
	my $msg = '';

	if( $tool eq 'tegaclap' ) {
		if( -f $subfile{'clapPHP'} ) {
			# てがくらっぷがあれば移動する
			my $path = &fcts::joinwithslash( $cgifulldir, $subfile{'clapPHP'} );
			print "Location: $path\n\n\n";
			exit;
		}
		else {
			# なければ
			$msg = '<p>てがくらっぷ(tegaclap.php)が既定の位置に見つかりませんでした。既定の位置に設置されていない場合、ここから移動することはできません。ただし、てがくらっぷは単独でも動作可能ですから、てがくらっぷの設置場所に直接アクセスする方法で利用可能です。てがろぐのアドオンとして利用したい場合は、てがろぐ側が求める既定位置に設置して下さい。</p>'
		}
	}

	my $css = '<style>
	</style>';
	&showadminpage('FAILED TO START','',$msg,'CA',$css);
}
...
sub listupBackupfiles
{
	...
	foreach my $of (@bupfiles) {
		...
		if( -f $fp ) {
			...
			# ダウンロードをてがろぐ経由(octet-stream)にするリンクを作成 (※ここで実在パスを生成できるにもかかわらずあえて本物のファイルパスを与えないのは、パラメータを弄って任意のファイルを選択されるような事態を避けるため)
			my $bupdate = '';
			if( $of =~ /(\d{8})/ ) {
				# バックアップファイル名から日付(数字8文字)部分だけを抽出
				$bupdate = $1;
			}
			my $dlLink = &makeQueryString('mode=getbackup','work=oldxml',"bupdate=$bupdate");
			$res .= qq|<tr><td>$count</td><td class="filelink"><a href="$dlLink" download>$of</a></td><td>$mtime</td><td>$fsu</td></tr>|;
			$count++;
		}
	}
...
sub adminExport
{
	...
	my $msg = qq|
		...
		<p class="noticebox">
			※てがろぐを<strong class="important">引っ越したい</strong>（＝てがろぐCGIの設置位置を移動したい）場合に、このエクスポート機能を使う必要は<strong class="important">ありません</strong>。
			てがろぐを引っ越すには、FTPでサーバ上にある全ファイル（※サブディレクトリも含めた全ファイル）をダウンロードし、引っ越し先のサーバに丸ごとアップロードすれば良いだけです。
			詳しくは、ヘルプドキュメントの「<a href="$aif{'puburl'}setup/#serverchange">他のサーバへ引っ越したい場合の操作</a>」項目をご覧下さい。
		</p>
	|;
	my $css = '<style>
...
sub getPulldownDateList
{
	...
	return '<select name="date" class="select-date">' .  &fcts::albaa( $dateopts ) . '</select>';
}
...
sub getPulldownHashtagList
{
	...
	return '<select name="tag" class="select-tag"><option value="">全ハッシュタグ</option>' .  &fcts::albaa( $htagopts ) . '</select>';
}
...
sub getPulldownCatList
{
	...
	return '<select name="cat" class="select-cat"><option value="">全カテゴリ</option>' .  &fcts::albaa( $catopts ) . '<option value="-">' . $otherlabel . '</option></select>';
}
...
sub modeGetbackupfile
{
	...
	elsif( $buptarget eq 'oldxml' ) {
		# 過去のXMLの場合は、対象ファイル名の日付部分(数字だけ)をパラメータから得る
		my $bupdate = $cgi->param('bupdate') || '';
		# そこからファイル名を作る
		if( $bupdate =~ m/^\d{8}$/ ) {
			# 数字8文字の場合のみ実行
			$targetfilename = &fcts::makelastcharslash( $autobackupto ) . 'backup.' . $bupdate . '.tegalog.xml';
			# そのファイルの実在を確認
			if( ! -f $targetfilename ) {
				# 存在しない場合はエラー終了
				&showadminpage('UNKNOWN BACKUP DATE','',"<p>指定の日付のバックアップファイルは見つかりませんでした。</p>",'BA');
				exit;
			}
		}
	}
...
sub downloadBackupfile
{
	...
	# ダウンロード時の保存用ファイル名を作る
	my $dlFileName = '';
	if( index($targetfilename, $autobackupto) == 0) {
		# バックアップ専用ディレクトリ内のファイルなら、そのファイル名のままダウンロードさせれば良い。
		$dlFileName = $targetfilename;
		# バックアップ専用ディレクトリ名が含まれているので、それは（ダウンロード用ファイル名に含めると困るので）削除する
		$dlFileName =~ s|^.*/||;
		# そのままだと最新日付のxmlとファイル名が重複してしまうので、oldの接頭辞を加える
		$dlFileName = 'old-' . $dlFileName;
	}
	else {
		# バックアップ専用ディレクトリのファイルではないなら、ダウンロード時専用の保存用ファイル名を生成する
		my $tfn4safe = $targetfilename;
		$tfn4safe =~ s/\//_/g;	# ファイル名中のスラッシュをアンダーバーに変換する
		$dlFileName = 'backup.' . &fcts::getNowDateForFileName() . '.' . $tfn4safe;		# backup.日付.元名称(+元拡張子) の形式にする
	}
...
# ワンタイムトークン発行		引数:なし
# ----------------------		返値:トークン文字列
sub onetimeTokenPublish
{
	my $token = &fcts::getrandstr(10);
	my $pubdate = time;

	# 保存
	my @trywrites;
	push(@trywrites,"onetimetoken=$token,$pubdate");	# onetimetoken=[トークン文字列],[発行エポック時] の形式で記録
...
# ----------------------
# ワンタイムトークン確認		引数1:トークン文字列、引数2:結果の返し方(0=結果コードだけを返す、1=エラーがあれば既製エラー画面を表示してCGIを終わる)
# ----------------------		返値:一致結果コード(1=一致、0=不一致、-1=トークン未発行)
sub onetimeTokenCheck
{
	my $trytoken = shift @_ || '';
	my $restype = shift @_ || 0;

	# 記録されているトークンを得る
	my ($rectoken, $pubtime) = split(/,/,$setdat{'onetimetoken'});		# トークン文字列,発行エポック時

	# 共通エラーメッセージ
	my $cem = '<div class="noticebox"><p>このエラーは、</p><ul><li>何らかの処理中にブラウザのリロード(再読込)ボタンを押してしまった</li><li>何らかの処理を開始するリンクを2回以上連続で押してしまった</li><li>時間のかかる処理を（ブラウザの）複数のタブを使って同時並行で実行してしまった</li><li>画面が自動で遷移した後に、ブラウザの「戻る」ボタンを押してしまった結果、処理が再実行されてしまった</ul><p>……のような場合にも発生する可能性があります。</p></div><p>&#10145; これ以後の処理はキャンセルされていますので、最初の画面に戻って操作をやり直して下さい。</p>';

	# 一致を確認
	if( $trytoken eq $rectoken ) {
		# 一致していたら
		return 1;
	}
	elsif( $rectoken eq '' ) {
		...
		if( $restype == 0 ) {
			...
			# 既製エラーを表示する場合
			my $err .= '<p><b>ワンタイムトークンが発行されていません。</b></p><p>※システム側が自動生成したフォーム以外からの送信は受け付けられません。（自作フォームからの送信はできません。）</p>' . $cem;
			&showadminpage('NO TOKEN','',$err,'CA','');
			exit;
		}
	}
	else {
		...
		if( $restype == 0 ) {
			...
			# 既製エラーを表示する場合
			my $err .= '<p><b>トークンが一致しませんでした。</b>実行中の処理は拒否されました。</p><p>※偽装フォーム等を使った不正なリクエストからシステムを守るために発行されるトークンが一致しませんでした。</p>' . $cem;
			&showadminpage('TOKEN MISMATCH','',$err,'CA','');
			exit;
		}
	}

	return -2;	# この行は実行されないハズ
}

# ----------------------
# ワンタイムトークン削除		引数:なし
# ----------------------		返値:なし
sub onetimeTokenClear
{
	# 保存
	my @trywrites;
	push(@trywrites,"onetimetoken=");	# onetimetoken項目を値なしで記録
...
sub makepostform
{
	...
	my $stateinputset = '';		# 公開状態ボタン関連ソース格納用
	...
	if( $setdat{'urlautolink'} == 1 && $setdat{'showLinkBtnStyle'} != 2 ) {
			...
			function insLinkUNIQUERAND(kind){
				...
				if( kind == 'U' ) {
					...
					if( txtSel.match(/^https?:\\/\\//) ) {
						/* URLが選択されているなら、それがリンク先だと解釈する */
						uis = txtSel;
						txtSel = '';
					}
					if( uis == '' ) {
						uis = prompt('OGPを読み取ってカード型リンクとして表示したいURLを入力して下さい：');
					}
					if( !uis ) { tArea.focus(); return; }
					if( !uis.match(/^https?:\\/\\//) ) {
						alert('http(s)://から始まるURL以外は指定できません。'); tArea.focus(); return;
					}
					else {
						uis += ' ';
					}
		...
		if(  $setdat{'showLinkBtnCrd'} == 1 )											{	$linkbuttonset .= q|<input type="button" value="| . &fcts::forsafetybutand($setdat{'linkBtnCrdLabel'}) . q|" onclick="insLinkUNIQUERAND('C');" title="OGP情報を出すカード型リンク">|;	}
	...
	if( $setdat{'allowdecorate'} == 1 && $setdat{'showDecoBtnStyle'} != 2 ) {
		...
		<script>
			function seldecoUNIQUERAND(decoSign,decoColor,decoName,befThrough){
				...
				if( decoName ) {
					if( decoSign == 'F' ) {
						decoName = prompt(( decoName + '入力して下さい：'),decoColor);
					...
					else if(( decoSign == 'L' ) \|\| ( decoSign == 'H' ) \|\| ( decoSign == 'O' ) \|\| (/\\d/.test(decoSign))) {
						if(decoName == '*') { decoName = '追加class'; }
						var pmsg = decoName + 'が必要なら入力して下さい。不要なら空欄のままOKを押して下さい。';
						if( decoSign == 'L' ) { pmsg += '(' + decoName + 'の詳細は公式マニュアルをご覧下さい。)'; }
						decoName = prompt(pmsg,decoColor);
						if( decoName === null ) { tArea.focus(); return; }
						if(( txtSel.length == 0 ) && ( decoSign == 'L' )) { txtSel = '$listinputsample'; }
		...
		if( ( $isNPOST && $setdat{'showDecoBtnHonA'} == 1 ) || ( $isQUICK && $setdat{'showDecoBtnHonQ'} == 1  ) ) { $decobuttonset .= q|<input type="button" class="decoBtnH" value="| . &fcts::forsafetybutand( $setdat{'decoBtnLabelH'} ) . q|"  onclick="seldecoUNIQUERAND('H','','ボタンラベル');" title="隠す(Hide)">|; }
		if( ( $isNPOST && $setdat{'showDecoBtnOonA'} == 1 ) || ( $isQUICK && $setdat{'showDecoBtnOonQ'} == 1  ) ) { $decobuttonset .= q|<input type="button" class="decoBtnO" value="| . &fcts::forsafetybutand( $setdat{'decoBtnLabelO'} ) . q|"  onclick="seldecoUNIQUERAND('O','','サマリー(ラベル)');" title="折り畳む(fOld)">|; }
		if( ( $isNPOST && $setdat{'showDecoBtnFonA'} == 1 ) || ( $isQUICK && $setdat{'showDecoBtnFonQ'} == 1  ) ) { $decobuttonset .= q|<input type="button" class="decoBtnF" value="| . &fcts::forsafetybutand( $setdat{'decoBtnLabelF'} ) . q|"  onclick="seldecoUNIQUERAND('F','','適用するclass名を半角で');" title="自由装飾(Free)">|; }

		if( ( $isNPOST && $setdat{'showDecoBtn1onA'} == 1 ) || ( $isQUICK && $setdat{'showDecoBtn1onQ'} == 1  ) ) { $decobuttonset .= q|<input type="button" class="decoBtn1" value="| . &fcts::forsafetybutand( $setdat{'decoBtnLabel1'} ) . q|"  onclick="seldecoUNIQUERAND('1','','*');" title="汎用装飾1：| . $setdat{'decoBtnElmnt1'} . q|">|; }
		if( ( $isNPOST && $setdat{'showDecoBtn2onA'} == 1 ) || ( $isQUICK && $setdat{'showDecoBtn2onQ'} == 1  ) ) { $decobuttonset .= q|<input type="button" class="decoBtn2" value="| . &fcts::forsafetybutand( $setdat{'decoBtnLabel2'} ) . q|"  onclick="seldecoUNIQUERAND('2','','*');" title="汎用装飾2：| . $setdat{'decoBtnElmnt2'} . q|">|; }
		if( ( $isNPOST && $setdat{'showDecoBtn3onA'} == 1 ) || ( $isQUICK && $setdat{'showDecoBtn3onQ'} == 1  ) ) { $decobuttonset .= q|<input type="button" class="decoBtn3" value="| . &fcts::forsafetybutand( $setdat{'decoBtnLabel3'} ) . q|"  onclick="seldecoUNIQUERAND('3','','*');" title="汎用装飾3：| . $setdat{'decoBtnElmnt3'} . q|">|; }
		if( ( $isNPOST && $setdat{'showDecoBtn4onA'} == 1 ) || ( $isQUICK && $setdat{'showDecoBtn4onQ'} == 1  ) ) { $decobuttonset .= q|<input type="button" class="decoBtn4" value="| . &fcts::forsafetybutand( $setdat{'decoBtnLabel4'} ) . q|"  onclick="seldecoUNIQUERAND('4','','*');" title="汎用装飾4：| . $setdat{'decoBtnElmnt4'} . q|">|; }
		if( ( $isNPOST && $setdat{'showDecoBtn5onA'} == 1 ) || ( $isQUICK && $setdat{'showDecoBtn5onQ'} == 1  ) ) { $decobuttonset .= q|<input type="button" class="decoBtn5" value="| . &fcts::forsafetybutand( $setdat{'decoBtnLabel5'} ) . q|"  onclick="seldecoUNIQUERAND('5','','*');" title="汎用装飾5：| . $setdat{'decoBtnElmnt5'} . q|">|; }
		if( ( $isNPOST && $setdat{'showDecoBtn6onA'} == 1 ) || ( $isQUICK && $setdat{'showDecoBtn6onQ'} == 1  ) ) { $decobuttonset .= q|<input type="button" class="decoBtn6" value="| . &fcts::forsafetybutand( $setdat{'decoBtnLabel6'} ) . q|"  onclick="seldecoUNIQUERAND('6','','*');" title="汎用装飾6：| . $setdat{'decoBtnElmnt6'} . q|">|; }
		if( ( $isNPOST && $setdat{'showDecoBtn7onA'} == 1 ) || ( $isQUICK && $setdat{'showDecoBtn7onQ'} == 1  ) ) { $decobuttonset .= q|<input type="button" class="decoBtn7" value="| . &fcts::forsafetybutand( $setdat{'decoBtnLabel7'} ) . q|"  onclick="seldecoUNIQUERAND('7','','*');" title="汎用装飾7：| . $setdat{'decoBtnElmnt7'} . q|">|; }
		if( ( $isNPOST && $setdat{'showDecoBtn8onA'} == 1 ) || ( $isQUICK && $setdat{'showDecoBtn8onQ'} == 1  ) ) { $decobuttonset .= q|<input type="button" class="decoBtn8" value="| . &fcts::forsafetybutand( $setdat{'decoBtnLabel8'} ) . q|"  onclick="seldecoUNIQUERAND('8','','*');" title="汎用装飾8：| . $setdat{'decoBtnElmnt8'} . q|">|; }
		if( ( $isNPOST && $setdat{'showDecoBtn9onA'} == 1 ) || ( $isQUICK && $setdat{'showDecoBtn9onQ'} == 1  ) ) { $decobuttonset .= q|<input type="button" class="decoBtn9" value="| . &fcts::forsafetybutand( $setdat{'decoBtnLabel9'} ) . q|"  onclick="seldecoUNIQUERAND('9','','*');" title="汎用装飾9：| . $setdat{'decoBtnElmnt9'} . q|">|; }
	...
	if( $setdat{'showCategoryBtnStyle'} != 2 ) {
		...
		# QUICKPOSTで、限定表示中のカテゴリに自動でチェックを入れておく設定の場合
		if(( $isQUICK ) && ( $setdat{'checknowcatbeforehand'} )) {
			# 限定中のカテゴリを配列に入れておく
			@prevcats = split(/,/, $cp{'cat'});
			if(scalar(@prevcats) > 0) {
				# カテゴリが1個以上ある場合は、カテゴリ選択ボタンの表示形態を一時的に(強制的に)1にする (※この行が実行されているなら、値は2(不使用)ではない。)
				$setdat{'showCategoryBtnStyle'} = 1;
			}
		}
...
# 			$categoryinputset .= q|<span class="helpbutton" onclick="alert('カテゴリの追加や編集は、管理画面の「カテゴリ管理」でできます。');">？</span>|;
	...
	if( $setdat{'showFuncBtnStyle'} != 2 ) {
		...
		<script>
			...
			function editmaxUNIQUERAND() {
				var covElm = document.getElementById('postformUNIQUERAND');
				var editElm = document.getElementById('tegalogpostUNIQUERAND');

				/* 2回目以降でも正しく動作するよう、一旦設定値をリセットしてから高さを計算する */
			...
			function easyInputUNIQUERAND(easystr){
				if( !easystr ) { return; }
				var tArea	= document.getElementById('tegalogpostUNIQUERAND');
				var tValue	= tArea.value;
				var staPos	= parseInt(tArea.selectionStart, 10);
		...
		# 何でも簡単入力ボタン機能がONならリストアップする
		my $easyinputHtml = '';
		if( $setdat{'easyinputUse'} == 1 ) {
			# 設定データを1件ずつ分解する
			my @easys = split(/<BR \/>/, $setdat{'easyinputs'});
			# 1件ずつ出力
			foreach my $one ( @easys ) {
				# ラベルが指定されていれば分離する
				my $eLabel = $one;
				my $eInput = $one;
				my $eTitle = '';
				if( index($one, ':') >= 1 ) {
					# 2文字目以降に半角コロン記号があるなら「ラベル指定あり」として分離する
					my @LaIn = split(/:/, $one, 2);		# コロン記号が複数あっても2分割しかしない
					if(( $LaIn[0] ne '' ) && ( $LaIn[1] ne '' )) {
						# ラベルと中身のどちらも空ではなければ、分離結果を採用する
						$eLabel = $LaIn[0];
						$eInput = $LaIn[1];
					}
				}
				# ラベルの調整
				$eLabel =~ s/<br>/ /g;	# 改行指示は半角スペースに置き換える(表示上の話)
				$eTitle = &fcts::mbSubstr($eLabel,18,'…');	# title属性値には18文字まで出力する
				$eLabel = &fcts::mbSubstr($eLabel,6,'…');	# 長すぎる場合には省略する
				# 安全化
				$eLabel = &fcts::forsafety($eLabel);
				$eInput = &fcts::forsafety($eInput);

				if( $setdat{'easyinputWay'} == 1 ) {
					# セレクトボックスなら
					$easyinputHtml .= qq|<option value="$eInput">$eLabel</option>|;
				}
				else {
					# ボタンなら (※製作メモ:注:onclick属性値に書かれたJavaScriptに直接変数の中身を書くと円記号や＆記号のエスケープ問題の面倒を見るのが大変になるので、別の属性値をJSから参照する方法を採用している。)
					$easyinputHtml .= qq|<input type="button" value="$eLabel" onclick="easyInputUNIQUERAND(this.getAttribute('data-input'));" data-input="$eInput" title="$eTitleを入力">|;
				}
			}
			# セレクトボックスの場合だけ、前後に必要要素を加える
			if( $setdat{'easyinputWay'} == 1 ) {
				$easyinputHtml = qq|<select class="nandemoEasyInput" style="max-width: 150px;" id="nandemoeiUNIQUERAND">$easyinputHtml</select>|;
				$easyinputHtml .= qq|<input type="button" class="neiIns" value="挿入"  onclick="easyInputUNIQUERAND( document.getElementById('nandemoeiUNIQUERAND').value );" title="選択した項目を挿入">|;
			}
		}
		...
		if( $setdat{'showFuncBtnSpeech'} == 1 ) {
			$btncount++;
			$funcinputset .= q|<input type="button" value="| . &fcts::forsafetybutand($setdat{'funcBtnSpeechLabel'}) . q|" onclick="comSpeechUNIQUERAND();" title="入力内容を合成音声で読み上げる">|;
			$funcinputset .= q|<input type="button" value="| . &fcts::forsafetybutand($setdat{'funcBtnSpStopLabel'}) . q|" onclick="speechSynthesis.cancel();" title="読み上げを止める">|;
		}
		if( $setdat{'showFuncBtnMaxEdit'} == 1 ) {
			$btncount++;
			$funcinputset .= q|<input type="button" value="| . &fcts::forsafetybutand($setdat{'funcBtnMaxEditLabel'}) . q|" onclick="editmaxUNIQUERAND();" title="編集領域を最大化する">|;
		}
		if( $setdat{'easyinputUse'} == 1 ) {
			$btncount++;
			$funcinputset .= $easyinputHtml;
		}
	...
	# ▼公開状態ボタン関連UI
	if( $setdat{'showStateBtnStyle'} != 2 ) {
		# 公開状態ボタンの表示がOFFでなければ出力する
		my $keyformConfirm = 'true';
		if(( $setdat{'allowanylineinsecret'} == 1 ) && ( $setdat{'secretkeystatus'} >= 1 )) {
			# 鍵入力位置指定ボタンを表示する場合、かつ、鍵が有効に設定されている場合は
			if( $setdat{'showseckeystate'} == 1 ) {
				# 案内を表示する設定なら、設定状況を調べてConfirm用メッセージを作成しておく
				my $kfcmS = '';
				my $kfcmA = '　';
				my $kfcmB = '　';
				my $kfcmC = '　';
				if(( $setdat{'allow2ndlineinsecret'} == 1 ) && ( $setdat{'allowonelineinsecret'} == 1 )) {
					# 2行目と1行目の両方にチェックが入っているなら、2行目まで見せる (※2行目がONでも1行目がOFFなら全文を見せないので)
					$kfcmC = '→'; $kfcmS = '(C)';
				}
				elsif(( $setdat{'allow2ndlineinsecret'} == 0 ) && ( $setdat{'allowonelineinsecret'} == 1 )) {
					# 2行目はOFFだが1行目がONなら、1行目だけ見せる
					$kfcmB = '→'; $kfcmS = '(B)';
				}
				else {
					# それ以外なら全文を見せない
				...
				$keyformConfirm = 'confirm(\'現在の鍵付き投稿の表示は、下記' . $kfcmS . 'の設定になっています。\n\n' .
					$kfcmA . '(A) 全文を見せない\n' .
					$kfcmB . '(B) 本文の1行目だけは常に見せる\n' .
					$kfcmC . '(C) 本文の2行目までを常に見せる\n\n' .
					'上記の設定に関わらず、本文中に [SYS:KEYFORM] を書くと、その位置に鍵の入力窓を表示し、そこ以前の本文は常に見せ、それ以降だけを鍵で隠せます。\n\n' .
					'現在のカーソル位置に [SYS:KEYFORM] を挿入するなら OK を押して下さい。\n上記の設定' . $kfcmS . 'の動作で良いなら何も挿入不要なのでキャンセルして下さい。\')';
			}
		}
		# JavaScriptの出力を作る
		$stateinputset = &fcts::tooneline( qq|
		<script>
			function showStateSetUNIQUERAND() {
				document.getElementById('stateDoorUNIQUERAND').style.display = 'none';
				document.getElementById('stateSetUNIQUERAND').style.display = 'inline';
			}
			...
			function insKeyFormUNIQUERAND(){
				...
				var placed = tValue.lastIndexOf("[SYS:KEYFORM]");
				if (placed !== -1) {
					tArea.setSelectionRange(placed, placed + 13);
					alert('既に鍵入力フォームの表示位置（＝鍵を掛ける開始位置）が設定されているようです。変更するには、本文中の記述を直接編集して下さい。');
				}
				else {
					var uis = $keyformConfirm;
					if( uis ) {
						newText = '[SYS:KEYFORM]';
		...
		foreach my $oneflag ( reverse(@allflags) ) {
			if( $oneflag eq 'draft' ) {
				...
				$idForm = q|<span id="draftsign" class="flagguide" onclick="alert('本番掲載するには[公開状態]→[下書き]チェックをOFFにして下さい。投稿時刻を現在日時にして先頭に投稿したい場合は、[新規に投稿]ラジオボタンを選択した上で[元投稿を削除]にチェックを入れて下さい。');">下書き</span>| . $idForm;
			}
			...
			if( $oneflag eq 'lock' ) {
				...
				$idForm = q|<span id="locksign" class="flagguide" onclick="alert('この投稿は鍵付き(パスワード保護状態)です。鍵を外すには[公開状態]→[鍵付き]チェックをOFFにして下さい。');">&#128274;</span>| . $idForm;
			}
			if( $oneflag eq 'rear' ) {
				# 下げるフラグが立っていれば
				$checkedfuncrear = ' checked';		# チェックボックスをON
				$idForm = q|<span id="rearsign" class="flagguide" onclick="alert('この投稿は下げられています。通常の条件で掲載するには[公開状態]→[下げる]チェックをOFFにして下さい。');">下げ中</span>| . $idForm;
			}
		}

		# ボタンを表示 (※ユーザ権限も同時に確認)
		my $btncount = 0;	# 内訳個数の確認用
		my $btnlabel = &fcts::forsafetybutand( $setdat{'statebuttonlabel'} );	# ボタンのラベル
		$stateinputset .= qq|<span class="decoBtns funcUIs">|;
		if( $setdat{'showStateBtnStyle'} == 0 ) { $stateinputset .= qq|<span id="stateDoorUNIQUERAND"><input type="button" class="stateDoorUNIQUERAND" value="$btnlabel" onclick="showStateSetUNIQUERAND();" title="公開状態セットを表示"></span><span id="stateSetUNIQUERAND" style="display:none;">|; }	# 非表示から動的に表示する場合(1/2)
		if(( $setdat{'showFuncBtnStaytop'} == 1 ) && ( $userlevel >= 9 )) { $btncount++; $stateinputset .= qq|<span class="catChecks"><label title="固定順序は管理画面の[設定]→[ページ設定]から"><input type="checkbox" name="fixed" value="1"$checkedfuncfixed>| . &fcts::forsafetybutand($setdat{'funcBtnStaytopLabel'}) . q|</label></span>|; }
		if(( $setdat{'showFuncBtnDraft'} == 1 ) && ( $userlevel >= 3 )) { $btncount++; $stateinputset .= qq|<span class="catChecks"><label><input type="checkbox" name="flag" value="draft"$checkedfuncdraft>| . &fcts::forsafetybutand($setdat{'funcBtnDraftLabel'}) . q|</label></span>|; }
		if(( $setdat{'showFuncBtnSecret'} == 1 ) && ( $setdat{'secretkeystatus'} >= 1 ) && ( $userlevel >= 3 )) {
			# 個別鍵指定ボタンを表示するかどうかを先に判断
			my $individuallockbtn = '';
			if( $setdat{'secretkeystatus'} == 2 ) {
				$individuallockbtn .= q|<input type="button" value="| . &fcts::forsafetybutand($setdat{'funcBtnSecIndLabel'}) . q|" onclick="insIndKeyUNIQUERAND();" title="個別鍵を設定する">|;
			}
			# 鍵入力位置指定ボタンを表示するかどうかを先に判断
			if( $setdat{'secretkeystatus'} >= 1 ) {
				$individuallockbtn .= q|<input type="button" value="| . &fcts::forsafetybutand($setdat{'funcBtnSecFormLabel'}) . q|" onclick="insKeyFormUNIQUERAND();" title="鍵入力フォームの表示位置(鍵を掛ける開始位置)を指定する">|;
			}
			my $keyformplacebtn = '';
			# チェックとボタンを出力
			$btncount++; $stateinputset .= qq|<span class="catChecks"><label><input type="checkbox" name="flag" value="lock" id="flaglockUNIQUERAND"$checkedfunclock>| . &fcts::forsafetybutand($setdat{'funcBtnSecretLabel'}) . qq|</label>$individuallockbtn$keyformplacebtn</span>|;
		}
		if(( $setdat{'showFuncBtnRear'} == 1 ) && ( $userlevel >= 3 )) { $btncount++; $stateinputset .= qq|<span class="catChecks"><label><input type="checkbox" name="flag" value="rear"$checkedfuncrear>| . &fcts::forsafetybutand($setdat{'funcBtnRearLabel'}) . q|</label></span>|; }
		if( $setdat{'showStateBtnStyle'} == 0 ) { $stateinputset .= qq|</span><!-- /#funcSet -->|; }	# 非表示から動的に表示する場合(2/2)
		$stateinputset .= qq|</span><!-- /.decoBtns -->|;
	...
	if( $isNPOST ) {
		...
		if( $postid ne '' ) {
			# 編集の場合は、メッセージを追加する
			$addforedit = '（投稿を削除したい場合は「この投稿を削除」ボタンを押して下さい。）';
		}
...
<script>
	function emptyPostCheck() {
		...
		var imginput = document.getElementById('newImgUpUNIQUERAND');
		if((textarea.value === '') && (imginput.files.length === 0)) {
			/* 本文が空で、かつ、ファイルが1つも指定されていない場合のみ */
			alert('空欄での投稿はできません。投稿本文を何か入力するか、または投稿画像を指定して下さい。| . $addforedit . q|');
			return false;
		}
		return true;
	}
	...
	my $postformhtml = qq|
		...
		<form action="$cgipath" method="post" class="postform" id="postformUNIQUERAND" enctype="multipart/form-data"$noemptypostSet>
			$idForm $dateForm
			$textarea
...
sub datadatecounter
{
	...
	if( $oldestY >= 0 && $newestY >= 0 ) {
		...
		my $dExpandsHtml = qq|<div class="datelimitlist">|;	# 日付折り畳みリストのHTMLソース
		...
		while( $cnowY >= $oldestY ) {
			...
			if(( defined($yearmonth[$cnowY][0]) ) && ( $yearmonth[$cnowY][0] > 0 )) {
				...
				if( $setdat{'datelistShowYear'} != 0 ) {
					...
					$dExpandsHtml	.= qq|<details class="datelimit-year"><summary>$outputY<span class="unit nen">年</span></summary><div class="datelimit-expand"><div class="datelimit-yearhead"><a href="?date=$outputY" class="datelistlink $selectedsign">$outputY<span class="unit nen">年</span></a><span class="num">($yearmonth[$cnowY][0])</span></div><ul class="datelimitsublist">|;
				}
				# さらに月チェック
				my $countStartM;
				my $countStepM;
				...
				for (1 .. 12) {
					...
					if(( defined($yearmonth[$cnowY][$cnowM]) ) && ( $yearmonth[$cnowY][$cnowM] > 0 )) {
						...
						$dExpandsHtml	.= qq|<li class="datelimit-month"><a href="?date=$outputY/$outputP" class="datelistlink $selectedsign"><span class="year">$outputY<span class="unit nen">年</span></span><span class="month">$outputM<span class="unit gatsu">月</span></span></a><span class="num">($yearmonth[$cnowY][$cnowM])</span></li>|;
					}
					# 次のループで使う月を計算
					$cnowM += $countStepM;
				}
				$dListupHtml	.= qq|</ul></li>| if( $setdat{'datelistShowYear'} != 0 );
				$dExpandsHtml	.= qq|</ul></div></details>|;
			}
			$cnowY--;
		}
		...
		$setdat{'dateexpandshtml'}	= $dExpandsHtml;
	}
	...
	if( $tempuse == 0 ) {
		...
		push( @trywrites, "dateexpandshtml=" . $setdat{'dateexpandshtml'} );	# ※
		&savesettings( @trywrites );
	}
...
sub getlatestpostdate
{
	...
	# 逆順で表示されている場合は、配列をひっくり返す
	if( $cp{'order'} eq 'reverse' ) {
		@targets = reverse(@targets);
	}

	# 全投稿を先頭からループ
	foreach my $oneclip (@targets) {

		# 分解
# 		my $id		= &fcts::forsafety( &fcts::getcontent($oneclip,'id') );
		my $date	= &fcts::forsafety( &fcts::getcontent($oneclip,'date') );
# 		my $user	= &fcts::forsafety( &fcts::getcontent($oneclip,'user') );
# 		my $cats	= &fcts::forsafety( &fcts::getcontent($oneclip,'cat') );
# 		my $comment	= &fcts::forsafety( &fcts::getcontent($oneclip,'comment') );
# 		my $uname   = &fcts::forsafety( &fcts::getUserDetail($user,2) ) || &fcts::forsafety($setdat{'unknownusername'});
		my $flags	= &fcts::forsafety( &fcts::getcontent($oneclip,'flag') );
		my $flagtf	= &fcts::getcontent($oneclip,'topfixed');	# 先頭固定の場合だけに付加されるフラグ(＝ 1 or 空文字列 )
		...
		# カウントしない条件
		if(( $flags =~ m/draft/ ) || ( $flagtf )) {
			# 下書き・先頭固定だったら (※「下げる」はカウント対象)
			next;
		}
		elsif(( $flags =~ m/scheduled/ ) && ( $setdat{'futuredateway'} == 1 )) {
			# 予約投稿だったら、予約投稿フラグが立っていて、かつ、未来の日付を予約投稿扱いにする場合で、予約時刻に達していなければカウントしない
		...
		# この時点の投稿日時が「最新投稿の日時」なので、それを得て終わる
		$ret = $date;
		last;
	}
...
# ------------------ #
# 最新投稿日時を記録 #		※何度も調べないようにキャッシュしておく
# ------------------ #
sub reclatestpostdate
{
	my $latestpostdate	= shift @_ || return;	# 引数が空なら何もしない

	# 生成結果を保存
	$setdat{'latestpostdate'} = $latestpostdate;

	# 設定ファイルに書き込み
	my @trywrites;
	push( @trywrites, "latestpostdate=" . $setdat{'latestpostdate'} );
	&savesettings( @trywrites );
...
# -------------------- #
# 新着系情報の一括更新 #	※新着リストと、最新投稿日時の情報を再生成して、設定ファイルに保存する一括処理
# -------------------- #
sub updateLatestInfos
{
	# 1. 新着リストの再生成
	my $latestlisthtml = &makelatestlist( $setdat{'latestlistup'} , $setdat{'latestlistparts'} , $setdat{'latesttitlecut'} );

	# 2. 最新投稿日時を得る (全投稿対象)
	my $latestpostdate = &getlatestpostdate();
...
sub makeHashtaglist
{
	my $type = shift @_ || '';		# LIST|PULL
	my $lim = shift @_ || 0;		# 個数制限
	my $js = shift @_ || '';		# :JS オプション
	my $linkmode = shift @_ || '';	# GALLERY|PICTS|SITEMAP|RSS|KEEPMODE

	# パラメータの妥当性を確認
	if( $lim !~ m/^\d+$/ ) {
		# 数字以外の文字がある場合はエラー
		return '<b>個数指定の値が不正です。値は正の整数で記述して下さい。スキンの記述を再確認して下さい。</b>';
	}
	$lim = $lim + 0;

	# ハッシュタグリスト用パーツを生成
	my ($hashtagliststring, $hashtagpullstring1, $hashtagpullstring2a, $hashtagpullstring2b, $hashtagpullstring3, $hashtagpullstring4a, $hashtagpullstring4b) = &makeHashtaglistParts($lim);

	# オプションに合わせて出力
	my $ret = '';
	if( $type eq 'LIST' ) {
		# リスト出力なら
		$ret = $hashtagliststring;
	}
	elsif( $type eq 'PULL' ) {
		# プルダウンメニューで
		if( $js eq '' ) {
			# JSオプションなしなら
			$ret = "$hashtagpullstring1$hashtagpullstring2a$hashtagpullstring3$hashtagpullstring4a";
		}
		elsif( $js eq ':JS' ) {
			# JSオプションありなら
			$ret = "$hashtagpullstring1$hashtagpullstring2b$hashtagpullstring3$hashtagpullstring4b";
		}
		else {
			return '<b>ハッシュタグリストのオプション種類を示す文字列が正しくありません。スキンの記述を再確認して下さい。</b>';
		}
	}
	else {
		return '<b>ハッシュタグリストの種類を示す文字列が正しくありません。スキンの記述を再確認して下さい。</b>';
	}

	# 指定モード用リンクの調整出力
	if( $linkmode ne '' ) {
		# 何か指定されている場合は調整用関数に渡す
		$ret = &linkadjustbymode($linkmode,$ret);
	}
...
# --------------------------------
# ハッシュタグリスト(パーツ)の生成		引数：リストに含める「最低出現個数」の値
...
sub makeHashtaglistParts
{
	...
	if( $outputCount == 0 ) {
		...
		if( $lim >= 2 ) {
			# 最低出現個数が2以上なら
			$hashtagliststring  .= '<li class="notexist">使用回数が' . &fcts::forsafety($lim) . '回を超えるハッシュタグはありません。</li>';
		}
		elsif( $setdat{'hashtagcup'} == 1 ) {
			# ハッシュタグが集計される設定なら
...
sub passfront
{
	...
	# ログインフォームの上下に表示する独自メッセージ
	my $GidForLoginScreen = '';
	my $MsgForLoginScreen = '';
	if( $setdat{'loginformgid'} ne '' ) {
		# あれば表示
		$GidForLoginScreen = '<div class="upperUserGid">' . &fcts::forsafety( $setdat{'loginformgid'} ) . '</div>';
	}
	if( $setdat{'loginformmsg'} ne '' ) {
		# あれば表示
		$MsgForLoginScreen = '<div class="lowerUserMsg">' . &fcts::forsafety( $setdat{'loginformmsg'} ) . '</div>';
	}
	...
	# canonical追加
	$css .= qq|\n\t<link rel="canonical" href="$cgifullurl?mode=admin">|;
	if(( $setdat{'rbni'} == 1 ) || ( $setdat{'loginnoindex'} == 1 ) || ( $setdat{'signhider'} == 1 )) { $css .= "\n\t" . &fcts::rbni(); }
...
sub showadminpage
{
	...
	foreach my $flag (split //, $fflags) {
		...
		elsif( $flag eq 'E' )	{ $footer .= '<a href="' . &makeQueryString('mode=admin','work=cemoji')		. '">カスタム絵文字リストに戻る</a>'; }
...
# -----------------------
# Favicon用link要素の出力		引数:指定がない場合に採用するデフォルトFavicon-href値
# -----------------------		返値:link要素 (または空要素)
sub outputFaviconLink
{
	my $deffavicon = shift @_ || '';
	my $ret = '';

	if(( $setdat{'faviconuse'} == 1 ) && ( $setdat{'faviconurl'} ne '' )) {
		# 独自アイコン指定の場合(＋URLの指定がある場合)
		$ret .= '<link rel="icon" href="' . &fcts::trim( &fcts::forsafety($setdat{'faviconurl'}) ) . '">';
	}
	elsif( $setdat{'faviconuse'} == 2 ) {
		# てがろぐアイコン指定の場合
		$ret .= '<link rel="icon" href="' . DEFAULTFAVICON . '">';
	}

	# ユーザ側の指定がない場合で、デフォルト指定があるならそれを採用する
	if(( $ret eq '' ) && ( $deffavicon ne '' )) {
		$ret .= '<link rel="icon" href="' . &fcts::forsafety($deffavicon) . '">';
	}
...
sub errormsg
{
	...
	if( !$setdat{'hideenvonerror'} ) {
		...
		$envs = '<div class="hideenv"><b>【より詳細な参考情報の表示方法】</b><br>※<a href="?mode=admin">管理画面</a>にアクセスできる場合は、[設定]→[システム設定]タブ→【<a href="?mode=admin&work=setting&page=4#sysAdvanced">高度な設定</a>】→「エラー発生時の参考情報の表示を省略する」項目をOFFにしてから再度この画面を再読込してみて下さい。すると、ここ（今読んでいるこの文章のある部分）に環境情報がリストアップされます。問い合わせの際には、その情報も併せてお送り頂けると話が早くなる可能性があります。</div>';
	}
	...
	<head>
		...
		<style>
			...
			.hideenv { background-color:#eee; margin:1em 0; padding:0.5em; }
		</style>
	</head>
	<body>
		<h1>&#9940; てがろぐ 動作エラー</h1>
