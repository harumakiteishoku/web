...
# ================================================================== #
#  てがろぐ本体お手軽アップデータ＆補助 TegUp Ver 2.1.0　[tegup.php] #
# ================================================================== #
#  Copyright (C) Fumihiro Nishimura.(Nishishi) 2024-2025.　　　　　　#
# 　　　 　　　 　　　 　　　 　　　 　　　 　　　 　　　 　　　　　 #
...
#  　が使用可能である必要があります。 　　　 　　　 　　　 　　　 　 #
#  ※てがろぐ側の動作を補助する機能としても使われますので、アップデ  #
#  　ート機能を使わない場合でも、TegUpの設置が必要な場合があります。 #
# 　　　 　　　 　　　 　　　 　　　 　　　 　　　 　　　 　　　　　 #
#  https://www.nishishi.com/ 　　　 　　　 　　　 　　  [2025/04/24] #
# ================================================================== #
...
// ------------------------------
//
// TegUp Ver.2 以降では、TegUpの設定情報は tegup.ini ファイルに書く仕様になりました。
// もし、tegup.php と同じディレクトリに tegup.ini が存在する場合は、そちらを書き換えることで設定できます。
//
// もし tenup.ini が存在しない場合は、下記の初期値を直接書き換えることで設定を変更できますが、次にTegUpを起動させた際には自動的に tegup.ini ファイルが生成されてそこに設定が書き出されます。
// それ以後は、tegaup.ini 側の設定だけが参照されるため、以下の初期値を再変更しても無視されますのでご注意下さい。
//
// ※既に tegup.ini ファイルが存在する場合は、下記の初期値を書き換えても一切反映されません。(tegup.iniに書かれている設定で上書きされるためです。)
//                                            ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// ----------------------------

// 初期値●てがろぐ本体ファイル名 (※修正の際は引用符を消さないようご注意下さい)
$tegalogFileName = 'tegalog.cgi';

// 初期値●このツールの使用を許可するユーザ権限 (0～9)
$requiredUserLv = 9;

// 初期値●バージョンアップ作業の直前に実行するバックアップの対象ファイル
// 　　0 : バックアップしない (非推奨)
// 　　1 : てがろぐ本体2ファイルとTegUp本体の計3ファイルだけをバックアップ (標準)
// 　　2 : 上記に加えて、パスワード・セッションID保存ファイルと、設定ファイル、データファイルも含めた6ファイルをバックアップ
$backupTargets = 1;

// 初期値●作業用として一時的に作成する仮ディレクトリのパーミッション (※8進数で指定するため先頭に 0 が必須)
$tempDirPermission = 0755;

// 初期値●作業ログをファイル(tegup.log)にも出力するかどうか
// 　　0 : 出力しない (標準)
// 　　1 : 出力する
$outputLogToFile = 0;

// 初期値●OGPキャッシュデータ保存用ファイル名
$ogpCacheFile = 'ogp-cache.ini';
...
$AppWORKTITLE = 'てがろぐ本体お手軽アップデータ＆補助ツール ';
$AppSEP = '';
$AppVERSION = '2.1.0';
$AppSTATUS = '';
$AppBODY = '';
...
$staleCacheTime = time() - 2678400;	// 1ヶ月前(＝2,678,400秒 (60*60*24*31))のエポック秒

// ――――――――――――――――――――――――――――
// ▼TegUpデータファイルの読み取り(or初回生成)
// ――――――――――――――――――――――――――――
$tegupINI = 'tegup.ini';
if( is_readable($tegupINI) ) {
	// tegup.iniがあれば読む
	$config = [];
	$lines = file($tegupINI, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
	foreach ($lines as $line) {
		$line = trim($line);
		if((strpos($line, '/') === 0) || (strpos($line, '=') === false)) {
			// コメント行(先頭がスラッシュか、または内部に「=」を含まない行)はスキップ
			continue;
		}
		list($key, $value) = explode('=', $line, 2);	// KEY=VALUE に分解(=は最初の1つのみで分割)
		$config[trim($key)] = trim($value);				// KEYとVALUE共にトリムして配列に追加
	}

	// 得られたINIをグローバル変数側に反映 (得られなかった設定値は初期値のまま)
	if( isset($config['tegalogFileName'])	&& ( strlen($config['tegalogFileName']) > 0		)) {	$tegalogFileName	=	$config['tegalogFileName'];		}
	if( isset($config['requiredUserLv'])	&& ( strlen($config['requiredUserLv']) > 0		)) {	$requiredUserLv		=	$config['requiredUserLv'];		}
	if( isset($config['backupTargets'])		&& ( strlen($config['backupTargets']) > 0		)) {	$backupTargets		=	$config['backupTargets'];		}
	if( isset($config['tempDirPermission'])	&& ( strlen($config['tempDirPermission']) > 0	)) {	$tempDirPermission	=	intval($config['tempDirPermission'], 8);	}	// 0755のような文字列を8進数の数値として代入する
	if( isset($config['outputLogToFile'])	&& ( strlen($config['outputLogToFile']) > 0		)) {	$outputLogToFile	=	$config['outputLogToFile'];		}
	if( isset($config['ogpCacheFile'])		&& ( strlen($config['ogpCacheFile']) > 0		)) {	$ogpCacheFile		=	$config['ogpCacheFile'];		}
}
else {
	// tegup.iniがなければ初期値をベースにして作る
	$iniStr = "
// ======================================
// TegUp 設定ファイル (TegUp Ver.2以降用)
// ======================================
// ※TegUpの動作設定を変更したい場合は、このファイルの中身を直接編集して下さい。その際は、「=」記号の右側だけを書き換えて下さい。
// このファイルが存在する限り、tegup.php本体に直接記述された初期値は無視されます。
// もし、すべてを初期値に戻したい場合は、このファイル(tegup.ini)を削除してから、TegUpを1度実行して下さい。すると、初期状態のtegup.iniが再生成されます。
// ======================================

// ▼てがろぐ本体ファイル名 (※デフォルトでは tegalog.cgi ですが、変更している場合は変更後のファイル名を指定して下さい。引用符は不要。)
tegalogFileName=$tegalogFileName
...
tempDirPermission=" . sprintf('%04o', $tempDirPermission) . "
...
outputLogToFile=$outputLogToFile

// ▼OGPキャッシュデータ保存用ファイル名
ogpCacheFile=$ogpCacheFile


// --- --- --- * これ以下にはプログラム側が必要に応じて書き込みますので何も書いたり編集したりしないで下さい * --- --- ---
";

	$iniStr .= 'FOV=' . $AppVERSION . "\n";
	$iniStr .= 'AGD=' . time() . "\n";

	// ファイル出力
	file_put_contents( $tegupINI, $iniStr );
}
...
if( !is_numeric($requiredUserLv) ) { errorEnd('<p>設定項目 requiredUserLv には、0～9 の範囲の整数のみを指定して下さい。</p>'); }
elseif( $requiredUserLv < 0 ) { $requiredUserLv = 0; }
elseif( $requiredUserLv > 9 ) { $requiredUserLv = 9; }

if( !is_numeric($backupTargets) ) { errorEnd('<p>設定項目 backupTargets には、0～2 の範囲の整数のみを指定して下さい。</p>'); }
elseif( $backupTargets < 0 ) { $backupTargets = 0; }
elseif( $backupTargets > 2 ) { $backupTargets = 2; }

if( !is_numeric($tempDirPermission) || $tempDirPermission <= 1 ) { errorEnd('<p>設定項目 tempDirPermission には、パーミッション値を8進数の数値で指定して下さい。(0755など)</p>'); }

if( !is_numeric($outputLogToFile) ) { errorEnd('<p>設定項目 outputLogToFile には、0または1 の整数のみを指定して下さい。</p>'); }
elseif( $outputLogToFile < 0 ) { $outputLogToFile = 0; }
elseif( $outputLogToFile > 1 ) { $outputLogToFile = 1; }

if(!preg_match('/^[-._a-zA-Z0-9]+$/', $ogpCacheFile)) { errorEnd('<p>設定項目 ogpCacheFile には、ファイル名として、半角英数字と「.」「-」「_」だけが使用可能です。</p>'); }
...
// ――――――――――――――――――――――――――――
if( isset($_GET["token"]) ) {
	// 連携トークンが与えられている場合は、てがろぐ補助機能だけを実行する
	$tsfret = tegalogSubFunc();

	// エラーがある場合にだけ出力
	$errs = errorMsg();
	if( $errs != '' ) {
		if( !$tsfret ) {
			echo '<!DOCTYPE html><html><head><meta charset="utf-8"><title>TegUp：てがろぐ補助エラー</title></head><body><div style="padding:0.5em; background:darkorange; color:white; border-radius:0.5em;">TegUp：てがろぐ補助機能 ERROR</div>' . "\n";
		}
		else {
			echo "\n\n";
		}
...
if( $loadedSets == -2 ) {
	...
	if( $tegalogFileName == 'tegalog.cgi' ) {
		...
		errorEnd('<p>このディレクトリには、<strong>tegalog.cgi</strong> ファイルが見つかりませんでした。</p><p>もし、tegalog.cgi のファイル名を変更して使っているなら、そのファイル名をデータファイル ' . $tegupINI . 'にある設定欄に記載してから、再度実行して下さい。</p>');
	}
	else {
		// 変更されている場合
...
// デモモードの場合は実行を阻止
if( $demoMode !== 0 ) {
	$AppBODY .= '<p style="background:#eee; padding:0.5em;">このツールは、<b style="color:crimson;">デモモード</b>で稼働中です。アップデート処理は実行されません。</p>';
	taskLog('デモモードで稼働しています。メイン処理は実行しませんでした。');
	$readyToNextStep = false;
}
...
// ▼てがろぐ補助処理
// ========================================================
function tegalogSubFunc()
{
	// URLを受け取る
	$fetchUrl = '';
	if( isset($_GET["fetch"]) ) {
		// fetchパラメータがあれば
		if( !isValidUrl($_GET["fetch"]) ) {
			// URLとして妥当でなければエラー
			errorMsg("指定のURL「" . htmlsafe($_GET["fetch"]) . "」は、URLとして妥当ではありません。");
			return false;
		}
		$fetchUrl = $_GET["fetch"];
	}
	else {
		errorMsg("fetchパラメータがありません。");
		return false;
	}

	// キャッシュされていれば(有効期限が切れていない限りは)キャッシュから返し、なければ新たに取得する
	$fetchedData = '';
	$successFlag = false;
	$cacheData = getOGPInfoFromCache($fetchUrl);
	if( $cacheData['validity'] == true ) {
		// キャッシュ有効期限内なら、そのままそのキャッシュ文字列を返す。
		$fetchedData = $cacheData['json'];
	}
	else {
		// --------------------------------------------------------------------------------------------------
		// キャッシュがないなら、トークンの正当性を確認してから、情報を取得して抽出結果をJSON形式にまとめる。
		// --------------------------------------------------------------------------------------------------

		// トークンの正しさを確認して、正しくない場合は終了する
		if( isValidTegalogToken( $_GET["token"] ) === false ) {
			// 正しくない場合 (※エラーメッセージは関数側で出力されているのでここではただ終わる)
			return false;
		}
		...
		$waitsec = 1;
		if( isset($_GET["order"]) ) {
			// orderパラメータがあれば、それを元に待ち時間を決める (JavaScriptからの複数リクエストはほぼ同時にくる前提)
			...
			elseif( $order <= 6 ) {
				// 2～6番目なら
				$waitsec = $order - 1;	// 1～5秒待つ
			}
			else {
				// それ以降なら (だいたいキャッシュされているケースも多いだろうから)
				$waitsec = (int)($order / 2) + 3;	// 6秒以上待つ (7th:6s、8-9th:7s、10-11st:8s、12-13rd:9s、14-15th:10s)
			}
		}
		// 待機
		if( $waitsec > 0 ) {
		...
		// OGP情報を取得に行く
		$nowFetchedData = fetchOGPInfo($fetchUrl);
		if( $nowFetchedData === false ) {
			// 取得失敗した場合
			if( $cacheData['json'] != '' ) {
				// 過去のキャッシュがある場合はそれを使う
				$fetchedData = $cacheData['json'];
				...
				// 過去のキャッシュがないならエラーを返す
				errorMsg("指定URLから情報を取得できませんでした。");
				return false;
			}
		}
		else {
			// 取得成功ならそれを返す
			$fetchedData = $nowFetchedData;
			$successFlag = true;
		}
	}
	...
	// JSONデータを返す
	// ----------------
	header('Content-Type: application/json; charset=utf-8');	// JSON用のヘッダ
	echo $fetchedData;

	// OGP情報を新たに取得した場合で、パラメータに order と total がある場合
	if( $successFlag && isset($_GET["order"]) && isset($_GET["total"]) ) {
		if( $_GET["order"] == $_GET["total"] ) {
			// orderとtotalが同値(＝今回が一連の処理で最後のリクエスト)なら
			if(mt_rand(1, 3) === 1) {
				// 3分の1の確率で、キャッシュを整頓する (リクエストが集中した場合の負荷軽減のため、実行回数を1/3に留める)
				tidyupOGPInfoCache();
			}
		}
	}
...
{
	// まず filter_var でURLの基本的な妥当性を確認
	if(false === filter_var($url, FILTER_VALIDATE_URL)) {
		return false;
	}
	// URL が http か https で始まっているかを確認
...
// --------------------------------------
// Local Hostで稼働しているかどうかを確認		返値: LocalHostならtrue、それ以外ならfalse
// --------------------------------------
function isUnderLocalhost()
{
...
// --------------------------------------------
// てがろぐ連携トークンとしての正しさを確認する
// --------------------------------------------
function isValidTegalogToken($trytoken, $filename = 'psif.cgi')
{
	$isvalid = false;

	if( ! $trytoken ) {
		// トークンの指定がない場合
		errorMsg("トークンが渡されませんでした。(この処理のためには有効なトークンが必要です。)");
		return false;
	}

	// for DEBUG:トークン文字列が「local」の場合は、LocalHostで稼働している場合に限って無条件で許可
	if(( $trytoken == 'local' ) && ( isUnderLocalhost() )) {
		return true;
	}
	...
	if($tokenFile) {
		// ファイルが開けたら、中を走査
		while(($line = fgets($tokenFile)) !== false) {
			// 指定のトークンを含む行を探す
			$tokenhead = 'TOKEN=';
			if((strpos($line, $tokenhead) !== false) && (strpos($line, $trytoken) !== false)) {
				// あれば、有効期限を抜き出す
				if( preg_match('/=(\d+),/', $line, $matches) ) {
					// 有効期限を過ぎていないかどうかを確認する
					if( $matches[1] >= time() ) {
						// まだ有効期間内なら正規のトークンとして扱う
						$isvalid = true;
					}
					else {
						$diffsec = time() - $matches[1];
						errorMsg("トークンの有効期限が過ぎています。( $diffsec )");
					}
				}
				else {
					errorMsg("トークンの有効期限が得られませんでした。");
				}
				break;
			}
		}
		if( !$isvalid ) {
			errorMsg("トークンが正しくありません。");
		}
		fclose($tokenFile); // ファイルを閉じる
	}
	else {
		// ファイルを開けなかったら
		errorMsg("ローカルのてがろぐセッション維持ファイルを開けませんでした。");
...
// ------------------------------------------------
// 指定URLのOGP情報をキャッシュファイルから読み込む
// ------------------------------------------------
function getOGPInfoFromCache($targeturl)
{
	global $ogpCacheFile, $staleCacheTime;		// キャッシュファイル名, キャッシュが古すぎると判断する日時

	// キャッシュの中身を連想配列に読み込む (※キャッシュファイルは最初は存在しないので、読めなくてもエラーにはしない)
	$ogpCache = [];
	if(is_readable($ogpCacheFile) && (($handle = fopen($ogpCacheFile, 'r')) !== false)) {
		while(($line = fgets($handle)) !== false) {
			// 行を「\t=\t」で分割
			$parts = explode("\t=\t", trim($line), 2);
			if(count($parts) === 2) {
				$ogpCache[$parts[0]] = $parts[1]; // 連想配列に格納
			}
		}
		fclose($handle);
	}
	...
	// 指定URLが既にあるかどうかを確認する
	if(isset($ogpCache[$targeturl])) {
		// キャッシュされているなら値を得る
		$cacheValue = $ogpCache[$targeturl];
		// 有効期限とJSONを分離
		list($expiryTime, $ret['json']) = explode("\t", $cacheValue, 2);
		// 有効期限が過ぎていないか確認
		if( $expiryTime >= time() ) {
			// 過ぎていない
			$ret['validity'] = true;
			...
			// 有効期限が過ぎている場合で、さらに1ヶ月以上古い場合は、キャッシュデータを整頓する処理をしておく
			// ここの処理は、JavaScript側から「最終リクエスト」の判断情報が来なくなってしまった場合のフェールセーフであって、メインではない。
			// ※注：たまたま当該データが1ヶ月以上参照されていなかった場合にしか実行されない。(1ヶ月以上古いデータがあっても、当該データが1ヶ月未満で追記されている場合は、古いデータはファイル内には残り続ける。連想配列にした時点で古いデータは見えなくなるため。なので、0.1％の確率で実行する。)
			if(( $expiryTime < $staleCacheTime ) || (mt_rand(1, 1000) === 1)) {
				// 古すぎる場合はキャッシュ整理処理を呼ぶ (※そうでなくても0.1％の確率で実行する)
				tidyupOGPInfoCache();
			}
		}
	}

	// 有効期限確認結果(validity)と、JSON文字列(json)を連想配列で返す。(※有効期限が過ぎていても、キャッシュJSONがあるならそれも返す。)
	return $ret;
}

// ---------------------------------------
// OGPキャッシュから古すぎるものを排除する
// ---------------------------------------
function tidyupOGPInfoCache()
{
	global $ogpCacheFile, $staleCacheTime;		// キャッシュファイル名, キャッシュが古すぎると判断する日時

	// キャッシュファイルが存在する場合にだけ実行する
	if(!is_readable($ogpCacheFile)) {
		// なければ何もせずに終わる
		return false;
	}

	// キャッシュの中身を連想配列に読み込む (※この処理だけで、同じキー(＝URL)のデータは最後の値1つに統合される。
	$ogpCache = [];
	if(($handle = fopen($ogpCacheFile, 'r')) !== false) {
		if(flock($handle, LOCK_SH)) { // 共有ロックを取得 :他のプロセスが書き込めなくなる(読み込みは可)
			while(($line = fgets($handle)) !== false) {
				// 行を「\t=\t」で分割
				$parts = explode("\t=\t", trim($line), 2);
				if(count($parts) === 2) {
			...
			errorMsg("OGPキャッシュファイルのロックに失敗したため整理処理はできませんでした。");
			fclose($handle);
			return false;
		}
		fclose($handle);
	...
	// 記録用に整形しつつ、古すぎるデータを排除する
	$output = '';
	foreach ($ogpCache as $key => $value) {
		// データの古さを確認する
		$dat = explode("\t", trim($value), 2);
		$expireTime = intval($dat[0]);	// データの先頭にある有効期限(エポック秒)を数値で得る
		if( $expireTime < $staleCacheTime ) {
			// キャッシュ期限が、1ヶ月前よりも古い場合は、記録しない
			continue;
		}
		// 記録用に整形する
		$output .= $key . "\t=\t" . $value . "\n";
	...
	// ファイルに出力
	$fpcret = file_put_contents($ogpCacheFile, $output, LOCK_EX);
	if( $fpcret === false ) {
		// ファイルに書けなかったらエラー
		errorMsg("OGPキャッシュファイルの整頓結果を書き込めませんでした。");
		return false;
	}
...
// ---------------------------------
// 指定URLのHTTPヘッダから情報を得る		引数:URL
// ---------------------------------		返値:コード・名称・Content-Typeの連想配列
function fetchHeadersInfo($url)
{
	$headers = @get_headers($url, 1);	// 無効なURLに対する警告を抑制(@)してヘッダを取得
	...
	if(preg_match('/\s(\d{3})\s(.+)/', $headers[0], $matches)) {
		$hinfo['code'] = $matches[1];
		$hinfo['name'] = $matches[2];
	}

	// Content-Type の取得
	$hinfo['ctype'] = isset($headers['Content-Type']) ? $headers['Content-Type'] : '';

	return $hinfo;
}

// ----------------------------------------------------
// 指定URLからOGP情報を得てキャッシュファイルに記録する
// ----------------------------------------------------
function fetchOGPInfo($pageUrl)
{
	global $ogpCacheFile;		// キャッシュファイル名
	$expiryBaseTime = 129600;	// キャッシュ期限(ベース):36時間
	...
	// HTMLソースの取得
	$pageHtml = @file_get_contents($pageUrl, false, $context);
	if($pageHtml === false) {
		// 読めなかった場合は、HTTPステータスを取得して概要文として記録しておく (たとえエラーURLでも同じサーバに何度も読みに行かないように)
		$errMsg = '';
		$hinfo = fetchHeadersInfo($pageUrl);
		if( $hinfo['code'] === false ) {
			// アクセスすらできなかった場合は、与えられたURLがおかしい
			$errMsg = '指定URLへのアクセスに失敗しました。';
		}
		else {
			// アクセスはできた場合はHTTPステータスを提示しておく
			$errMsg = $hinfo['code'] . ' ' . $hinfo['name'];
		}

		// エラーメッセージを概要文に設定
		$ogDescription = '(' . htmlsafe($errMsg) . ')';

		// キャッシュ期限を短めにする (8時間)
		$expiryBaseTime = 28800;
	}
	else {
		// 読めた場合は中身を解析する

		// ---------
		// DOMDocumentが文字コードを誤解するのを防ぐための前処理
		// 1. 既存のCharset値の抽出
		$charset = null;
		if(preg_match('/<meta\s+charset\s*=\s*["\']?([^"\'>\s]+)["\']?\s*\/?>/i', $pageHtml, $matches)) {
			$charset = $matches[1];
		}
		else {
			$encoding = mb_detect_encoding($pageHtml, ['UTF-8', 'SJIS', 'EUC-JP', 'ISO-2022-JP', 'ISO-8859-1', 'ASCII'], true);
			if( $encoding === false ) {
				// 判別できなかった場合はとりあえずUTF-8とする
				$charset = 'UTF-8';
			}
			elseif( $encoding && strtoupper($encoding) == 'SJIS' ) {
				$charset = 'Shift_JIS';
			}
			else {
				$charset = $encoding;
		...
		if($charset !== null) {
			...
			errorMsg("文字コード強制挿入用のタグ生成に失敗しました。");
		}
		// 3. それをhead要素の最初に挿入
		if(preg_match('/<head\b[^>]*>/i', $pageHtml, $matches, PREG_OFFSET_CAPTURE)) {
			$pos = $matches[0][1] + strlen($matches[0][0]);
			$pageHtml = substr_replace($pageHtml, $tags, $pos, 0);
		}
		// 4. head要素がない場合は html要素の最初に挿入
		elseif(preg_match('/<html\b[^>]*>/i', $pageHtml, $matches, PREG_OFFSET_CAPTURE)) {
			$pos = $matches[0][1] + strlen($matches[0][0]);
			$pageHtml = substr_replace($pageHtml, $tags, $pos, 0);
		}
		else {
			errorMsg("対象データには、html要素もhead要素もありませんでした。");
		}

		// ---------
		// HTML解析の準備：DOMDocumentとXPathを使用
		...
		if($nodes->length > 0) {
			$ogTitle = trim($nodes->item(0)->nodeValue);
		}
		else {
		...
		// ▽OGP:タイトルの取得 (※TEXT抽出版)
		if( $ogTitle === '' ) {
			// DOM解析に失敗したらテキスト抽出を試す
			$ogTitle = extractMetaContent($pageHtml, '/<meta\s+property="og:title"\s+content="([^"]+)"/i');
			if(!$ogTitle) {
				$ogTitle = extractMetaContent($pageHtml, '/<title>(.*?)<\/title>/is');
			}
		}
		...
		if($nodes->length > 0) {
			$ogDescription = trim($nodes->item(0)->nodeValue);
		}
		else {
		...
		if( $ogDescription === '' ) {
			// DOM解析に失敗したらテキスト抽出を試す
			$ogDescription = extractMetaContent($pageHtml, '/<meta\s+property="og:description"\s+content="([^"]+)"/i');
			if(!$ogDescription) {
				$ogDescription = extractMetaContent($pageHtml, '/<meta\s+name="description"\s+content="([^"]+)"/i');
			}
		}
		...
		if( $ogImage === '' ) {
			// DOM解析に失敗したらテキスト抽出を試す
			$ogImage = extractMetaContent($pageHtml, '/<meta\s+property="og:image"\s+content="([^"]+)"/i');
		}
		// OGP画像が相対パスで書かれていそうな場合の簡易対処
		if(( $ogImage !== '' ) && (!preg_match('/^https?:\/\//', $ogImage))) {
			$parsedUrl = parse_url($pageUrl);
			$domain = $parsedUrl['scheme'] . '://' . $parsedUrl['host'];
			if(substr($ogImage, 0, 1) === '/') {
		...
		// ▼Twitter Cardの取得 (※DOM解析版)
		$nodes = $xpath->query('//meta[@name="twitter:card"]/@content');
		if($nodes->length > 0) {
			$cardSize = trim($nodes->item(0)->nodeValue);
		}
		// ▽Twitter Cardの取得 (※TEXT抽出版)
		if( $cardSize === '' ) {
			// DOM解析に失敗したらテキスト抽出を試す
			$cardSize = extractMetaContent($pageHtml, '/<meta\s+name="twitter:card"\s+content="([^"]+)"/i');
		}

		// ---------
		// OGPデータが得られなかった場合
		if(($ogTitle === '') && ($ogDescription === '') && ($ogImage === '')) {
			// 読めなかった場合は、Content-Typeを取得して判断
			$hinfo = fetchHeadersInfo($pageUrl);
			if( $hinfo['ctype'] === 'text/html' ) {
				// 「text/html」なら、何らかのトラブルの可能性も考慮してキャッシュ期限を短めにする (8時間)
				$expiryBaseTime = 28800;
			}
			else {
				// 他の場合は、そもそもHTMLを返すURLではない可能性があるため、概要文にそれを出し、キャッシュ期限は長めにする (7日間)
				$ogDescription = 'Content-Type: ' . htmlsafe($hinfo['ctype']) . ' (リンク先はWebページではない可能性があります。この情報は7日間キャッシュされます。)';
				$expiryBaseTime = 604800;
			}
		}
	}
	...
	// JSONデータの作成
	$ogData = [
		'title' => replaceSafeForJson($ogTitle),
		'description' => replaceSafeForJson($ogDescription),
		'image' => replaceSafeForJson($ogImage),
		'card' => replaceSafeForJson($cardSize),
		'recorded_date' => date("Y-m-d H:i:s")
	];
	$jsonLine = json_encode($ogData, JSON_UNESCAPED_UNICODE);
	$jsonLine = preg_replace("/[\r\n\t]+/", '', $jsonLine);		// 改行とタブが含まれていても全部消す
	$jsonLine = strip_tags($jsonLine);							// HTMLタグも消す

	// キャッシュ期限 (ベース＋12時間＝36～48時間後)
	$expiryTime = time() + $expiryBaseTime + rand(0, 43200);

	// 記録データの作成
	$addRecord = $pageUrl . "\t=\t" . $expiryTime . "\t" . $jsonLine . "\n";

	// ファイルに追記
	$fpcret = file_put_contents($ogpCacheFile, $addRecord, FILE_APPEND | LOCK_EX);
	if( $fpcret === false ) {
		// ファイルに書けなかったらエラー
		errorMsg("OGPデータをキャッシュファイルに書き込めませんでした。");
		return false;
	}
...
// JSONデータに本来なら含めて良いのだがJavaScriptで処理する際に排除しておく方が良さそうな記号を全角化する
function replaceSafeForJson($string) {
