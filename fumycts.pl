...
# ================================================================== #
#  Fumy Versatile Control Set Ver 2.23.0 (UTF-8)        [fumycts.pl] #
# ================================================================== #
...
my $vernum = 2023000;	# 2.23.0
#            ^  ^  ^

# ------------------------------------------	引数：0=全部読む／1=PWだけを読む／2:SESSIONだけを読む／3:FAILUREだけを読む／4:TOKENだけを読む	※行頭の種別「PASS=／SESSION=／FAILURE=／TOKEN=」は出力しない
...
sub loadpwsifile
{
	...
	elsif( $kind == 4 ) { return @retToken; }
	else {
		return ( \@retPass, \@retSession, \@retFailure, \@retToken );
	}
}

# ------------------------------------------
# パスワード・セッションID格納ファイルへ出力	引数1：1=PWだけを更新／2:SESSIONだけを更新／3:FAILUREだけを更新／4:TOKENだけを更新　引数2：更新用配列
# ------------------------------------------
sub savepwsifile
{
	...
	foreach my $one (@tokens) {
		$one = 'TOKEN=' . $one;
	}

	# 出力用の全データ列を作成
	if( $kind == 1 ) {		push( @ret, (@tryarray,	@sessions,	@failures,	@tokens		));	}	# PASSだけを更新
	elsif( $kind == 2 ) {	push( @ret, (@passes,	@tryarray,	@failures,	@tokens		));	}	# SESSIONだけを更新
	elsif( $kind == 3 ) {	push( @ret, (@passes,	@sessions,	@tryarray,	@tokens		));	}	# FAILUREだけを更新
	elsif( $kind == 4 ) {	push( @ret, (@passes,	@sessions,	@failures,	@tryarray	));	}	# TOKENだけを更新
	else {	push( @ret, (@passes, @sessions, @failures, @tokens, @tryarray));			}	# エラー(何も更新しない)
...
# -------------------------- #
# ◆トークンの発行/記録/削除 #	記録形式： TOKEN=有効期限,トークン文字列[改行]
# -------------------------- #
sub maketoken
{
	my $expireSec = shift @_ || 180;	# トークンの有効期限(デフォルト:3分)

	# パスワード・セッションID記録ファイルの読み込み
	my @tokens = &loadpwsifile(4);

	# トークンの生成
	my $tokenstr = 'tk' . &getrandstr(18);		# 単純に18文字のランダムな英数字をトークンとする

	# トークンのタイムアウト時刻の生成
	my $timeouttime = time + $expireSec;

	# 今回のトークンを記録
	my @ret;
	push( @ret, "$timeouttime,$tokenstr" );

	# 無効になったトークンの削除
	foreach my $onesid ( @tokens ) {
		if( $onesid =~ m/^(\d+),.+/ ) {
			# トークンの有効期限を得て調べる
			if( $1 >= time ) {
				# 有効期限が来ていなければ記録継続
...
sub tidyDat
{
	...
	foreach my $oneinfo (@itemlist) {
		@{$retdat[$i]} = split(/<>/,$oneinfo); # 1件内データを分離	（※$retdat[$i] には自動的に配列リファレンスが割り当てられるため、@retdat には配列リファレンスのリストが格納される。そのため、関数から返した先でも2次元配列のように扱える。※この書き方だと要素数が入るだけで配列にならない）
		$i++;
	}
...
# ----------------------------------------
# ◆OSV：旧独自区切りデータをFumyxmlに変換		引数1：「<>と<,>で区切られた生データ」、引数2以降：Fumyxmlで使う要素名群(必要なだけ列挙)
# ----------------------------------------		返値：Fumyxml文字列
sub datToFumyxml
{
	my $rawDat = shift @_ || '';
	...
	my $fumyXml = '<?fumyxml version="1.0" ?>';

	my @itemlist = split(/<,>/,$rawDat);	# 1件単位で分離
	foreach my $oneitem (@itemlist) {
		my @oneArray = split(/<>/,$oneitem);	# 1件内データを分離
		my $newOneXml = '';
		my $i = 0;
		foreach my $onerec (@oneArray) {
			$newOneXml .= '<' . $newElements[$i] . '>' . $onerec . '</' . $newElements[$i] . '>';		# 引数で指定された要素名を使ってマークアップ
			$i++;
		}
		# 1件分のデータに整形して追加
		$fumyXml .= '<datum>' . $newOneXml . '</datum>';
	}
...
# ----------------------
# ◆OSV：Fumyxmlかどうか	引数：調べる文字列
# ----------------------	返値：YES=1/NO=0
sub isFumyxml
{
	my $checkStr = shift @_ || '';

	if(index($checkStr, '<?fumyxml') == 0) {
		return 1;
	}
	return 0;
}
...
# ------------------------
# ◆OSV：Fumyxmlの分解格納		引数：「<?fumyxml version="1.0" ?><datum>～</datum>」系統のデータ(文字列)
# ------------------------		返値：「1件の全タグを連想配列にしたハッシュ」へのリファレンスを格納した配列
sub expandFumyxml
{
	my $rawXml = shift @_ || '';
	my @retdat = ();
	...
	# 先頭の宣言を除外
# 	$rawXml =~ s/^<?fumyxml version="[\d\.]+" ?>//;

	# 1件ずつ分割
	my @xmlArray = $rawXml =~ /<datum>(.*?)<\/datum>/g;

	foreach my $entry (@xmlArray) {
		my %hash;
...
# ----------------------------------------------
# ◆OSV：Fumyxmlの1件データから任意要素の値を得る		引数1：得たい要素名、引数2：「<datum>～</datum>」1つの文字列
# -----------------------------------------------		返値：引数で指定された文字列に含まれる任意要素の値
sub getOneValueInDatum
{
	my $requesttag = shift @_ || &errormsg('getOneValueInDatum：要素名の指定がない');
	my $targetdatum = shift @_ || '';

	# まず外側のdatumタグを除外する (注:この処理がないと datum要素を1つ取得して終わりになる！)
	if( $targetdatum =~ /<datum>(.*?)<\/datum>/ ) {
		$targetdatum = $1;
	}

	# タグをパースしてハッシュに格納
	my %hash;
	while( $targetdatum =~ /<([\w_:-]+)>(.*?)<\/\1>/g ) {
		$hash{$1} = $2;
	}

	# 指定のキーの値を返す
	my $gotItem = $hash{$requesttag};
	if( !defined $gotItem ) {
		# 指定のキーが存在しない場合は空文字列を返す
		return '';
	}
	return $gotItem;
}
...
# -------------------------------
# ◆OSV：Fumyxmlの1件データを作成		引数：要素名と値の連想配列リファレンス
# -------------------------------		返値：「<datum>～</datum>」1つの文字列
sub makeFumyxmlOneDatum
{
	my $datumRef = shift @_ || &errormsg('makeFumyxmlOneDatum：連想配列のリファレンスが必須');
	my $ret = '';

	# 1項目ずつマークアップする(※掲載順序を固定するためソートする)
	foreach my $key (sort keys %{$datumRef}) {
		$ret .= "<$key>" . $datumRef->{$key} . "</$key>";
	}

	return '<datum>' . $ret .'</datum>';
}

# --------------------------------
# ◆OSV：Fumyxmlの完全データを作成		引数：複数の「<datum>～</datum>」の配列 (※中身の正しさは確認しないので注意)
# --------------------------------		返値：Fumyxmlの1行文字列
sub makeCompleteFumyxml
{
	my $fumyXml = '<?fumyxml version="1.0" ?>';

	foreach my $oneDatum (@_) {
		$fumyXml .= $oneDatum;
	}
...
# -------------------------
# ◆UID：ユーザリストを表示		引数1：カテゴリのリンクに追加する文字列(必要なら)、引数2：フラグ、引数3：ログイン中のユーザID、引数4：限定表示中のユーザID
# -------------------------
sub getCustomUserList
{
	my $addlinkstr	= shift @_ || '';
	my $uflags		= shift @_ || '';
	my $loggedinUID	= shift @_ || '';
	my $showingUID	= shift @_ || '';

	# フラグの値チェック
	if( $uflags !~ m/[I|N|C|L|T]/ ) {
		# 英字フラグのどれも含まれていないならデフォルト値にする
		$uflags = '<N>';	# デフォルトは「リンク（名前）」
	}

	my $retListHtml = '<ul class="userlist">';

	# 全ユーザをループ
	for my $i ( 0 .. $#userdata ) {
		my $uid   = $userdata[$i][0];	# I: User ID
		my $ulv   = $userdata[$i][1];	# L: 権限
		my $uname = $userdata[$i][2];	# N: 表示名
		my $utext = $userdata[$i][3];	# T: 紹介文
		my $uicon = $userdata[$i][4];	# C: アイコン
		...
		# 要素C：アイコンがあれば表示HTMLを作る
		my $uPartC = '';
		if( $uicon ne '' ) {
			$uPartC = '<span class="oneusericon"><img src="' . &forsafety($uicon) . '" alt="' . &forsafety($uname) . '" class="usericon-' . &forsafety($uid) . '"></span>';
		}

		# リンク用文字列を作る
		my $linkhref = '?userid=' . &forsafety($uid);
		if( $addlinkstr ne '' ) {
			# 追加があれば加える
		...
		# 状況に応じた追加classの準備
		my $addclas = '';
		if( $uid eq $loggedinUID ) { $addclas .= ' nowloggedin'; }
		if( $uid eq $showingUID  ) { $addclas .= ' nowshowing'; }

		# フラグから出力ソースを作成
		my $usource .= '';
		foreach my $flag (split //, $uflags) {
			# フラグがあるだけループ
			if(    $flag eq 'I' )	{ $usource .= '<span class="oneuserid">' . &forsafety($uid) . '</span>'; }
			elsif( $flag eq 'L' )	{ $usource .= '<span class="oneuserlevel">' . &forsafety($ulv) . '</span>'; }
			elsif( $flag eq 'N' )	{ $usource .= '<span class="oneusername">' . &forsafety($uname) . '</span>'; }
			elsif( $flag eq 'T' )	{ $usource .= '<span class="oneusertext">' . &forsafety($utext) . '</span>'; }
			elsif( $flag eq 'C' )	{ $usource .= $uPartC; }
			elsif( $flag eq 'B' )	{ $usource .= '<br>'; }
			elsif( $flag eq '<' )	{
				my $suid = &forsafety($uid);
				$usource .= qq|<a href="$linkhref" class="oneuserlink userid-$suid$addclas">|;
			}
			elsif( $flag eq '>' )	{ $usource .= '</a>'; }
			elsif( $flag eq '"' )	{ $usource .= '&quot;'; }
			elsif( $flag eq "'" )	{ $usource .= '&apos;'; }
			else { $usource .= $flag; }
		}

		# 生成したソースを追記
		$retListHtml .= qq|\n<li class="oneuser">$usource</li>|;
	}
...
# -----------------------------------	引数1：ID、引数2：連想配列のリファレンス
# ◆CAT：カテゴリ情報を更新/追加/削除	引数2は、値が「＾維」だったら前の値を維持。順序が「DEL」だったら削除。
# -----------------------------------	返値：カテゴリ情報をデータファイルに記録するための1行文字列　→Fumyxml形式の完全な1行データ
sub makeLineForCategoryDat
{
	my $tryid		= shift @_ || '';
	my $catOptRef	= shift @_ || &errormsg('makeLineForCategoryDat：カテゴリ情報の連想配列リファレンスが必須');

	# …………………………………………………………
	# Step.0：カテゴリ情報(連想配列内)の値チェック
	# …………………………………………………………
	# ▼親カテゴリ
	if( $catOptRef->{'cat:parent'} eq '' ) {
		# 親カテゴリがない場合（＝自分が最上階層の場合）は「-」を記録する仕様
		$catOptRef->{'cat:parent'} = '-';
	}
	# ▼該当件数
	if( $catOptRef->{'cat:posts'} eq '' ) {
		# 何もなければ0を記録
		$catOptRef->{'cat:posts'} = 0;
	}
	# ▼順序指定
	if( $catOptRef->{'cat:order'} eq '' ) {
		# 順序指定がない場合は1と解釈
		$catOptRef->{'cat:order'} = 1;
	}

	# ………………………………………………………………………………………………………
	# Step.1：まずは追加/編集/削除：カテゴリ生データを全ループして更新(or追加/削除)
	# ………………………………………………………………………………………………………
	my @newids;	# とりあえず追加/編集/削除する配列
	my @orders;	# 掲載順序の記録用
	my $wflag = 0;
	for my $i ( 0 .. $#catdata ) {
		if( $catdata[$i]{'cat:id'} eq $tryid ) {
			# 指定のIDを発見したら
			if( $catOptRef->{'cat:order'} ne 'DEL' ) {
				# 削除フラグがなければ更新

				# 各値が「＾維」だったら以前の値を維持する
				if( $catOptRef->{'cat:name'}    eq '＾維' ) { $catOptRef->{'cat:name'}    = $catdata[$i]{'cat:name'}; }
				if( $catOptRef->{'cat:dscrpt'}  eq '＾維' ) { $catOptRef->{'cat:dscrpt'}  = $catdata[$i]{'cat:dscrpt'} || ''; }		# 値がなければ空文字
				if( $catOptRef->{'cat:parent'}  eq '＾維' ) { $catOptRef->{'cat:parent'}  = $catdata[$i]{'cat:parent'}; }
				if( $catOptRef->{'cat:posts'}   eq '＾維' ) { $catOptRef->{'cat:posts'}   = $catdata[$i]{'cat:posts'}; }
				if( $catOptRef->{'cat:order'}   eq '＾維' ) { $catOptRef->{'cat:order'}   = $catdata[$i]{'cat:order'}; }
				if( $catOptRef->{'cat:iconsrc'} eq '＾維' ) { $catOptRef->{'cat:iconsrc'} = $catdata[$i]{'cat:iconsrc'} || ''; }	# 値がなければ空文字

				# 順序の全角数字対策
				$catOptRef->{'cat:order'} = &nozenkaku($catOptRef->{'cat:order'});

				# Fumyxml用の1件文字列を生成して @newids に記録
				$catOptRef->{'cat:id'} = $tryid;
				my $oneDatum = &makeFumyxmlOneDatum( $catOptRef );		# 注:引数には連想配列のリファレンスが必要だが $catOptRef は元々リファレンスなのでそのまま渡す。
				push(@newids, $oneDatum);	# 1行Fumyxml文字列を追加
				push(@orders, $catOptRef->{'cat:order'});	# 掲載順序も同時に記録
			}
			$wflag = 1;		# 作業済みフラグを立てる
		}
		else {
			# 別のIDだったらそのまま追加
			$catdata[$i]{'cat:order'}	= &nozenkaku($catdata[$i]{'cat:order'});
			$catdata[$i]{'cat:name'}	= $catdata[$i]{'cat:name'} || '';		# 値がなければ空文字
			$catdata[$i]{'cat:iconsrc'}	= $catdata[$i]{'cat:iconsrc'} || '';	# 値がなければ空文字

			# Fumyxml用の1件文字列を生成して @newids に記録
			my $oneDatum = &makeFumyxmlOneDatum( $catdata[$i] );
			push(@newids, $oneDatum);
			push(@orders, $catdata[$i]{'cat:order'} );
		}
	}
	if( $wflag == 0 ) {
		# まだ何もしていなければ新規追加なので(とりあえず)末尾に追加する (※該当個数は「未集計」にする)
		$catOptRef->{'cat:order'} = &nozenkaku($catOptRef->{'cat:order'});
		$catOptRef->{'cat:posts'} = '未集計';
		my $oneDatum = &makeFumyxmlOneDatum( $catOptRef );		# 注:引数には連想配列のリファレンスが必要だが $catOptRef は元々リファレンスなのでそのまま渡す。
		push(@newids, $oneDatum);
		push(@orders, $catOptRef->{'cat:order'});
	}

	# ………………
	# 削除の後処理：削除されたIDを親カテゴリに持つIDを最上階層に引き上げておく
	# ………………
	if( $catOptRef->{'cat:order'} eq 'DEL' ) {
		for my $i ( 0 .. $#newids ) {
			$newids[$i] =~ s|<cat:parent>$tryid</cat:parent>|<cat:parent>-</cat:parent>|;		# 親カテゴリIDを、最上階層「-」に修正する
		}
	}
	...
	$constructCatTree = sub {
		...
		for my $i ( 0 .. $#sortedids ) {
			...
			if( &getOneValueInDatum('cat:parent',$sortedids[$i]) eq $parentCatId ) {	# 親カテゴリ名の一致を確認
				# 親要素が指定カテゴリだったら記録
				push(@layeringids,$sortedids[$i]);
				# いま抜き出したカテゴリ名を得る
				my $onecat = &getOneValueInDatum('cat:id',$sortedids[$i]);
				# 記録済みの元データは消しておく
				$sortedids[$i] = '';
	...
	# カテゴリ情報をデータファイルに記録するための1行文字列(Fumyxml)を生成して返す	※カテゴリが1件もない場合は、Fumyxmlの宣言だけが入っている
	return &makeCompleteFumyxml(@layeringids);
}

# -------------------------------------------
# ◆CAT：カテゴリの該当件数だけをアップデート	引数：「カテゴリID：該当個数」形式の連想配列のリファレンス
# -------------------------------------------	返値：処理内容の報告文字列 (※返値は捨てても問題ない。この関数では @catdata を直接書き換える)
sub updateCatApplyPostsNum
{
	my $catApplyPostsRef	= shift @_ || &errormsg('updateCatApplyPostsNum：カテゴリ該当個数の連想配列リファレンスが必須');
	my $res = '<ul>';

	# 最初に全カテゴリの該当件数を0にリセットしておく
	for my $i ( 0 .. $#catdata ) {
		$catdata[$i]{'cat:posts'} = 0;
	}

	while( my ($catid, $apnum) = each %{$catApplyPostsRef} ) {
		# catid = カテゴリID
		# apnum = 該当件数(数値)
		for my $i ( 0 .. $#catdata ) {
			if( $catdata[$i]{'cat:id'} eq $catid ) {
				# IDが一致したらそこの該当件数を書き換える
				$catdata[$i]{'cat:posts'} = $apnum;
				# 結果表示用文字列を作る
				$res .= '<li>カテゴリID：' . $catid . ' ：' . $apnum . '</li>';
			}
		}
    }
...
# ---------------------------------------------
# ◆CAT：現在のカテゴリ情報を保存できる文字列化		返値：$setdat{'categorylist'}に格納できる「Fumyxmlの完全データ」
# ---------------------------------------------
sub getCompFumyxmlForCats
{
	my @catDatumElements = ();

	# 全カテゴリをループして datum 要素化する
	for my $i ( 0 .. $#catdata ) {
		push( @catDatumElements, &makeFumyxmlOneDatum( $catdata[$i] ) );
	}

	# それをFumyxml完全データ化して返す
	return &makeCompleteFumyxml( @catDatumElements );
}
...
sub getCategoryTree
{
	...
	$makeCatTreeStructure = sub {
		...
		for my $i ( 0 .. $#catdata ) {
			...
			my $catid		= $catdata[$i]{'cat:id'};
			my $catname		= $catdata[$i]{'cat:name'};
			my $catdscrpt	= $catdata[$i]{'cat:dscrpt'};
			my $parentcat	= $catdata[$i]{'cat:parent'} || '-';
			my $catposts	= $catdata[$i]{'cat:posts'} || '0';
			my $catorder	= $catdata[$i]{'cat:order'};
			my $caticonurl	= $catdata[$i]{'cat:iconsrc'} || '';
...
sub getCategoryTable
{
	...
	for my $i ( 0 .. $#catdata ) {
		...
		if( $linkhref ne '' ) {
			# リンク文字列を作る
			$ltag1 = qq|<a href="$linkhref&amp;catid=| . &forsafety($catdata[$i]{'cat:id'}) . qq|">|;
			$ltag2 = '</a>';
		}

		my $catid		= &forsafety( $catdata[$i]{'cat:id'} );
		my $catname		= &forsafety( $catdata[$i]{'cat:name'} );
		my $catdscrpt	= &mbSubstr( &forsafety( $catdata[$i]{'cat:dscrpt'} ), 15, '…');	# 最長15文字だけを掲載
		my $parentcat	= &forsafety( $catdata[$i]{'cat:parent'} ) || '-';
		my $catposts	= &forsafety( $catdata[$i]{'cat:posts'} ) || '0';
		my $catorder	= &forsafety( $catdata[$i]{'cat:order'} );
		my $caticonsrc	= &forsafety( $catdata[$i]{'cat:iconsrc'} );
...
# -------------------------------
# ◆CAT：カテゴリIDから情報を得る	引数1：カテゴリID、引数2：得たいタグ名 (※下位互換性のため数値指定も可：1=表示名,2=概要文,3=親カテゴリID,4=該当個数,5=掲載順序,6=アイコンURL)
# -------------------------------	返値：得たいデータ（該当なしの場合は「空文字列」を返す）
sub getCategoryDetail
{
	my $searchid = shift @_ || &errormsg('getCategoryDetail：カテゴリIDの指定がない');
	my $detailname = shift @_ || '';

	# 下位互換性のための数値指定をタグ名に変換
	if( $detailname eq '1' ) { $detailname = 'cat:name'; }			# 1=表示名
	elsif( $detailname eq '2' ) { $detailname = 'cat:dscrpt'; }		# 2=概要文
	elsif( $detailname eq '3' ) { $detailname = 'cat:parent'; }		# 3=親カテゴリID
	elsif( $detailname eq '4' ) { $detailname = 'cat:posts'; }		# 4=該当個数
	elsif( $detailname eq '5' ) { $detailname = 'cat:order'; }		# 5=掲載順序
	elsif( $detailname eq '6' ) { $detailname = 'cat:iconsrc'; }	# 6=アイコンURL
	elsif( $detailname eq '' ) { &errormsg('getCategoryDetail：得たいタグ名または情報番号の指定がない'); }

	# カテゴリデータ内から指定のIDを見つける
	for my $i ( 0 .. $#catdata ) {
		if( $catdata[$i]{'cat:id'} eq $searchid ) {
			# 見つかったら、指定のデータを返す
			my $gotItem = $catdata[$i]{$detailname};
			if( !defined $gotItem ) {
				# 指定のキーが存在しない場合は空文字列を返す
				return '';
			}
...
sub getCategoryLastOrder
{
	...
	# 全ユーザをループ
	for my $i ( 0 .. $#catdata ) {
		if( $max < $catdata[$i]{'cat:order'} ) {
			$max = $catdata[$i]{'cat:order'};
		}
	}
...
sub getCategoryList
{
	...
	for my $i ( 0 .. $#catdata ) {
		if( $type == 1 ) {
			push( @ret, "$catdata[$i]{'cat:id'}<>$catdata[$i]{'cat:name'}" );
		}
		elsif( $type == 2 ) {
			push( @ret, "$catdata[$i]{'cat:id'}<>$catdata[$i]{'cat:name'}<>$catdata[$i]{'cat:posts'}" );
		}
		else {
			push( @ret, "$catdata[$i]{'cat:id'}" );
		}
	}
...
sub getepochtime
{
	my $str = shift @_ || return 0;		# &errormsg('Datetime string Required.');	→引数がない場合でもエラーにはせずに0を返すよう仕様変更 v2.22.1
...
# --------------------------------------------
# 指定ファイルのタイムスタンプ(更新日時)を得る		引数:ファイル名
# --------------------------------------------		返値:更新日時(エポック秒)	※ファイルがなければ -1 を返す。
sub getModTimeStamp
{
	my $file = shift @_ ||  &errormsg('ERROR:getTimeStamp, No FileName.');
...
# -------------------------------------		※扱えるのは文字列データのみ (ネスト・配列・数値は扱えないので注意)
# ◆GEO：連想配列の中身をJSON形式で返す		引数：連想配列のリファレンス
# -------------------------------------		返値：文字列
sub outputJSON
{
	my $hashRef = shift @_ || &errormsg('outputJSON: HashRef Required');

	my $ret = "{";
	my $first = 1;
	foreach my $key (sort keys %{$hashRef}) {
		$ret .= "," unless $first;	# 最初の要素以外はカンマを追加
		$ret .= '"' . escape_json($key) . '":"' . escape_json($hashRef->{$key}) . '"';
		$first = 0;
	}
	$ret .= "}\n";
...
# ------------------------------------
# ◆GEO：連想配列の中身をINI形式で返す		引数：連想配列のリファレンス
# ------------------------------------		返値：文字列
sub outputINI
{
	my $hashRef = shift @_ || &errormsg('outputINI: HashRef Required');

	my $ret = '';
	foreach my $key (sort keys %{$hashRef}) {
		$ret .= &escape_ini($key) . '=' . &escape_ini($hashRef->{$key}) . "\n";
	}
...
# ------------------------------------
# ◆GEO：連想配列の中身をTAG形式で返す		引数：連想配列のリファレンス
# ------------------------------------		返値：文字列
sub outputTAG
{
	my $hashRef = shift @_ || &errormsg('outputTAG: HashRef Required');

	my $ret = '';
	foreach my $key (sort keys %{$hashRef}) {
		$ret .= '<' . &escape_tag($key) . '>' . &escape_tag($hashRef->{$key}) . '</' . &escape_tag($key) . ">\n";
	}
...
# -------------------------------------
# ◆GEO：連想配列の中身をYAML形式で返す		引数：連想配列のリファレンス
# -------------------------------------		返値：文字列
sub outputYAML
{
	my $hashRef = shift @_ || &errormsg('outputYAML: HashRef Required');

	my $ret = '';
	foreach my $key (sort keys %{$hashRef}) {
		$ret .= &escape_yaml($key) . ': ' . &escape_yaml($hashRef->{$key}) . "\n";
	}
...
# ------------------------
# ◆指定範囲の数値に収める		引数1：現在値、引数2：最小値、引数3：最大値
# ------------------------		返値：現在値が最小値～最大値の範囲内なら現在値、範囲外なら最大か最小の最も近い方の値
sub inrange
{
	my $num  = shift @_ || 0;
	my $min  = shift @_ || 1;
	my $max  = shift @_ || 10000;
...
# ------------------------------
sub escape_json {
	my $str = shift @_;		# 値が0でも空文字列にしない
	$str =~ s/\\/\\\\/g;	# バックスラッシュをエスケープ
	$str =~ s/"/\\"/g;		# ダブルクォートをエスケープ
	$str =~ s/\n/\\n/g;		# 改行をエスケープ
	$str =~ s/\r/\\r/g;		# キャリッジリターンをエスケープ
	$str =~ s/\t/\\t/g;		# タブをエスケープ
	return $str;
}
...
sub escape_yaml {
	...
	if( index($str,':') >= 0 ) {
		# コロンが含まれている場合
		if(( index($str,'"') == -1 ) && ( index($str,"'") == -1 )) {
			# 引用符は含まれていない場合→ダブルクォーテーションで囲む
			return '"' . $str . '"';
		}
		elsif(( index($str,'"') >= 0 ) && ( index($str,"'") == -1 )) {
			# 引用符：ダブルも含まれているがシングルはない場合→シングルで囲む
			return "'$str'";
		}
		elsif(( index($str,'"') == -1 ) && ( index($str,"'") >= 0 )) {
			# 引用符：ダブルはないがシングルが含まれている場合→ダブルで囲む
			return '"' . $str . '"';
		}
		else {
			# 両種類の引用符も含まれている場合は諦めて何もしない(無加工で返す)
		}
	}
...
# --------------------------------------------------------
# ◆複数のカンマで区切られた文字列に、さらに項目を追加する		引数1：追加したい項目(文字列)、引数2：元のカンマ区切り文字列
# --------------------------------------------------------
sub commaseparateadd
{
	my $add = shift @_ || '';	# 追加する項目
	my $old = shift @_ || '';	# 元のカンマ区切り文字列

	if( $add eq '' ) {
		# 追加項目が空文字列なら、何もせず元の文字列を返す
		return $old;
	}

	my @oitems = split(/,/,$old);		# 既存項目を分解
	push( @oitems, $add );				# 指定項目を追加
	my $newcsv = join(',', @oitems);	# カンマで区切って文字列化
...
# ---------------------------------
# ◆localhostで動作しているかどうか
# ---------------------------------		返値:YES=1,NO=0
sub isRunOnLocalhost
{
	if(($ENV{'SERVER_NAME'} eq 'localhost') || ($ENV{'REMOTE_ADDR'} =~ /^127\..+/ )) {
		return 1;
	}
	return 0;
}
...
sub errormsg
{
	...
	if(( $showenv == 1 ) || ( -f './showenv.txt' )) {
		...
		$envs = '<div class="hideenv"><b>【より詳細な参考情報の表示方法】</b><br>以下のどちらかの方法を実施してから再度この画面を再読込してみて下さい。すると、ここ（今読んでいるこの文章のある部分）に「環境情報を表示」というボタンが表示されます。それを押すと、お使いのサーバの環境情報がリストアップされます。問い合わせの際には、その情報も併せてお送り頂けると話が早くなる可能性があります。<ul><li>〔選択肢A〕tegalog.cgiと同じディレクトリに showenv.txt というファイル名でファイルを置く（中身は読みませんので空で構いません）。</li><li>〔選択肢B〕fumycts.plソースの21行目付近にある <code>my $showenv = 0;</code> の記述を <code>my $showenv = <strong class="important">1</strong>;</code> に変更する。</li></ul>※どちらか一方だけで充分です。</div>';
	}
	...
	<head>
		...
		<style>
			...
			code { border:1px solid #ccc; background-color:snow; padding:2px 0.25em; }
		</style>
	</head>
	<body>
